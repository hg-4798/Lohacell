<?php
    ...
    // 전체 댓글 수
    $sql    = "select count(*) from tblbasket_event_comment  ";
    $row    = pmysql_fetch_object(pmysql_query($sql));
    $total_comment_count = $row->count;

    $sql    = "select * from tblbasket_event_comment  order by idx desc ";
    ...
    while ($row = pmysql_fetch_array($result)) {
        $reg_date = date("Y/m/d H:i:s", strtotime($row['date']) );
            ...
            $review_html .= '
                    <li>
                        <div class="box">
                            ...
                            <span class="id">' . setIDEncryp($row['id']) . '</span>
                        </div>
                        <p>' . $comment . ' ';
    ...
    if ( $isMobile ) {
        include($Dir.TempletDir."promotion/mobile/promotion_detail_6_TEM001.php");
    } else {
?>
