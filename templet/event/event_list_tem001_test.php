			...
			?>
				...
				<li>
					...
					<?}else{?>
						...
						<a href="http://nasign.ajashop.co.kr/board/board.php?pagetype=view&num=<?=$row->num?>&board=event" target="_self">
					<?}?>
					<? if(is_file($Dir.DataDir."shopimages/board/event/".$row->vstorefilename)){ ?>
							<img style="width: 275px; height: 150px;" src="<?=$Dir.DataDir."shopimages/board/event/".$row->vstorefilename?>" alt="" />
					<? } ?>
						</a></p>
						<dl>
							<dt><?=$row->title?></dt>
