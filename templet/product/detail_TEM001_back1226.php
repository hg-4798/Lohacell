...
<div class="detail_header_warp">
		        ...
		        <p class="product_name"><?=$_pdata->productname?></p>
					...
					?>
						<dl class="product_info_price">
						<dt>판매가격 </dt><dd>:</dd>
						<dd><font id="idx_consumer"><?if($_pdata->consumerprice){ echo number_format($_pdata->consumerprice)." → ";}?></font><font class="sale_price" id="idx_price"><?=number_format(getProductSalePrice($_pdata->sellprice,$dc_data[price]))?>원</font></dd>
...
<span><img src="<?=$Dir?>image/detail/d_all_btn.gif"  alt="제품상세"></span>
	...
	<div class="product_caution">
		...
		<div class="description_qna_list">
		    <ul>
				<li><img src="<?=$Dir?>image/detail_guide.gif"  alt="주의사항"></li>
...
<?=(strlen($dicker)>0)?"return;\n":"";?>
	...
	if(temp==1) {
		...
		document.all["idx_consumer"].innerHTML = document.form1.consumer.value+" → ";
		}else{
			if(consumer[0]!='0'){
