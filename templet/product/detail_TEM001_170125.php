		...
		<div class="layer-content">
			...
			<form name="frm_storesearch" id="frm_storesearch" onSubmit="return false;">
				...
				<div class="sorting-size">
					<ul class="clear">
						<li><label><input type="radio" class="CLS_sizechk" id="size_85" name="sizechk" ids="85" value="85" onclick="storeSearchChkForm('size');"><span>85</span></label></li>
			...
			<div class="map-box CLS_stock_store_list hide">
				...
				<div class="map-area">
					...
					<script>
						...
						var icon_x = 20;
						var icon_y = 30;
						var trueChk= false;	//2016-10-05 libe90 디폴트 체크용 변수추가
						var chks= 'N';	//2016-10-05 libe90 디폴트 체크용 변수추가
						...
						function addResult(i, beach) {
							var rdate_div= $('.CLS_HiddenRdate').html();	//2016-10-05 libe90 방문예정일레이어 위치이동용 변수추가
							...
							if(global_delivery_type == '1' && beach[14]=='Y') {	//2016-10-05 libe90 매장픽업가능매장만 선택되도록 작업
								altchk='javascript:mapSelectStore(\'' + i + '\', \'' + beach[1] + '\', \'' + beach[2] + '\', \'' + beach[12] + '\', \'' + beach[0] + '\', \'' + beach[5] + '\');" onClick="javascript:$(\'.CLS_stock_store_num\').removeClass(\'on\');$(this).find(\'.CLS_stock_store_num\').addClass(\'on\');';
								if(trueChk==false){
									trueChk=true;
									chks='Y';
								}else{
							...
							}else if(global_delivery_type == '2' && beach[16]=='Y') {	//2016-10-05 libe90 바로발송 가능매장일때만 선택되도록 작업
								altchk = 'javascript:mapSelectStore(\'' + i + '\', \'' + beach[1] + '\', \'' + beach[2] + '\', \'' + beach[12] + '\', \'' + beach[0] + '\', \'' + beach[5] + '\');" onClick="javascript:$(\'.CLS_stock_store_num\').removeClass(\'on\');$(this).find(\'.CLS_stock_store_num\').addClass(\'on\');';
								if(trueChk==false){
									trueChk=true;
									chks='Y';
							...
							}else{	//2016-10-05 libe90 선택불가처리
								altchk='javascript:alert(\'선택가능한 매장이 아닙니다.\')'
								chks='N';
							}
							contentString += '	<a href="'+altchk+'">';
							contentString += '		<span class="CLS_stock_store_num';
							if (chks=='Y') contentString += ' on';	//2016-10-05 libe90 기본체크값 설정
							...
							contentString += '	</a>';
							contentString += "<div class='CLS_HiddenRdate' id='ID_HiddenRdate_"+i+"'>"+rdate_div+"</div>";	//2016-10-05 libe90 방문수령일 레이어 셋팅
							contentString += '</li>';

							li.innerHTML = contentString;
							results.appendChild(li);
							$('#ID_HiddenRdate_'+i).hide();	//2016-10-05 libe90 방문수령일 히든
							if (chks=='Y'){
								$('#ID_HiddenRdate_'+i).show();	//2016-10-05 libe90 가능매장일경우 첫번째만 방문수령일 보이기
							}
							if (chks=='Y') {	//2016-10-05 libe90 가능매장일경우 초기값 선택
								mapSelectStore(i, beach[1], beach[2], beach[12], beach[0], beach[5]);
							}
						}
						...
						function mapSelectStore(i, x, y, storecode, storename, storeaddr) {
							...
							$('.CLS_HiddenRdate').hide();	//2016-10-05 libe90 기존수령일 정보 히든
							$('#ID_HiddenRdate_'+i).show();	//2016-10-05 libe90 해당 수령일 정보 노출
							$('.CLS_select_reservation_date').val('');	//2016-10-05 libe90 기존수령일값 초기화
...
console.log(data);
						...
						function custonSort(a, b) {	//2016-10-06 libe90 매장정렬용 함수
							if(a.sortData == b.sortData){ return 0} return  a.sortData > b.sortData ? 1 : -1;
						}
			...
			<input type = 'hidden' class = 'CLS_select_reservation_date'> <?//2016-10-05 libe90 수령일지정용 히든값 추가?>
			<input type = 'hidden' class = 'CLS_select_storename'>
			<input type = 'hidden' class = 'CLS_select_storeaddr'>
			<!--<div class='CLS_HiddenRdate' id='ID_HiddenRdate' style = 'display:none;'><?//2016-10-05 libe90 수령일 위치이동용 div?>-->
				<div class="form-box CLS_reservation_date" style = 'display:none;'>
					<div class="form-box">
						<div class="my-comp-select" style="width:150px;">
							<select class="required_reservation_date" label="날짜" onchange="$('.CLS_select_reservation_date').val($(this).val())">	<?//2016-10-05 libe90 수령일선택시 히든값에 선택값 셋팅?>
	...
	<div class="layer-inner w800">
		...
		<div class="layer-content">
			...
			<form name='reviewForm' id='reviewForm' method='POST' action='' >
                ...
                <input type="hidden" name="op_idx" id="op_idx" value="<?=$review_order_idx?>" />
	...
	<div class="layer-inner w800">
		...
		<div class="layer-content">
			...
			<table class="th_left util">
				...
				<tr>
					<th scope="row"><label for="qna-content">내용</label><span class="required">*</span></th>
					<td colspan="3">
				...
				<tr>
					<th scope="row"><label for="qna-pwd">비밀번호</label><span class="required">*</span></th>
			...
			<form name='prForm' id='prForm' method='POST' action="<?=$Dir.FrontDir?>basket.php" >
					...
					<ul class="image-list">
						...
						if ( $multi_img=="Y" && $yesimage[0] ) {
							...
							foreach( $yesimage as $mImgKey=>$mImgVal ){
								$multiImg = getProductImage2($imagepath_multi, $mImgVal);
								if($multiImg){
				...
				<div class="hero-info">
					...
					<div class="price-wrap">
						<?if($_pdata->consumerprice != $_pdata->sellprice){ ?>
							<dl class="price">
								<dt>소비자가</dt><?//2016-10-06 libe90 정차장님요청으로 수정?>
								<dd><del><?=number_format( $_pdata->consumerprice )?></del></dd>
							</dl>
							<dl class="sale-price">
								<dt>판매가</dt><?//2016-10-06 libe90 정차장님요청으로 수정?>
								<dd>
						...
						<?if ($coupon_use['code'] !='' && $coupon_use['btn_yn']=='Y') {?>
							<dl class="coupon">
								...
								<dd>
									...
									<a href="javascript:;" class="CLS_coupon_download ml-5" data-coupon='<?=urlencode("COUPON|".$coupon_use['type']."|".$coupon_use['code'])?>'><img src="../static/img/btn/btn_coupon_download.gif" alt="쿠폰다운로드"></a>
						...
						<?if($_ShopInfo->getStaffYn() == 'Y' && $_pdata->hotdealyn=='N'){?>
							<dl class="staff">
								<dt>임직원가</dt>
								<dd>
									<strong><?=number_format($staff_use['price']) ?> (<?=$staff_use['per'] ?>%)</strong>
								</dd>
							</dl>
						<?}?>
						<!-- <?if($_pdata->consumerprice != $_pdata->sellprice){ ?>
				...
				-->
					...
					<style>
						...
						.hero-info-option td:first-child {font-family:ngb; font-size:1.3rem; text-align:center;}
						.hero-info-option div.qty{position:relative; text-align:right; width:122px;}
						...
						.hero-info-option .btn_option_delete_type {float:left;bottom:3px; right:0; margin:0px auto; text-align:center;}
						.hero-info-option .btn_option_delete_type .btn-type1 {min-width:18px;}
						...
						.hero-info-option .btn_option_delete_type a.addOptionStore {margin:0 3px 3px; width:90px; height:35px; line-height:35px; background:#a6a6a6;}
					...
					<div class="hero-info-option">
						<table style="width:360px;" cellpadding="0" cellspacing="0" border="0" align="center">
							<colgroup><col style="width:55px"><col style="width:122px"><col style="width:93px"><col style="width:63px"><col style="width:auto"></colgroup>
					...
					<div class="opt-add">
						<ul>
							<li>Style: <?=$_pdata->prodcode?></li>
							<li>Color: <?=$_pdata->colorcode?></li>
                    ...
                    // <br>태그 제거
                    $arrList_content = array("/<br\/>/", "/<br>/");
                    $_pdata_prcontent_tmp = trim(preg_replace($arrList_content, "", $_pdata_prcontent));
                    if ( empty($_pdata_prcontent_tmp) ) {
                    } else {
?>
						<dl>
							<dt>DETAILS -</dt>
...
<?
                        if ( strpos($_pdata_prcontent,"table>")!=false || strpos($_pdata_prcontent,"TABLE>")!=false)
                            echo "<pre>".$_pdata_prcontent."</pre>";
                        else if(strpos($_pdata_prcontent,"</")!=false)
                            echo nl2br($_pdata_prcontent);
                        else if(strpos($_pdata_prcontent,"img")!=false || strpos($_pdata_prcontent,"IMG")!=false)
                            echo nl2br($_pdata_prcontent);
                        else
                            echo str_replace(" ","&nbsp;",nl2br($_pdata_prcontent));
                    }
                    ?>
					</div>
					<!-- // [D] 옵션값 추가-->
...
?>
		...
		<section class="goods-detail-info">
			...
			<div class="cont-info">
				<dl>
					<dt><?=$jungbo_option[1]?></dt>
					<dd>
						<?=$jungbo_val[1]?>
					</dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[2]?></dt>
					<dd><?=$jungbo_val[2]?></dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[3]?></dt>
					<dd><?=$jungbo_val[3]?></dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[4]?></dt>
					<dd><?=$jungbo_val[4]?></dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[5]?></dt>
					<dd><?=$jungbo_val[5]?></dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[6]?></dt>
					<dd>
						<?=$jungbo_val[6]?>
					</dd>
				</dl>
				<dl>
					<dt><?=$jungbo_option[7]?></dt>
					<dd>
						<?=$jungbo_val[7]?>
					</dd>
				</dl>
		...
		<section class="goods-detail-info">
			...
			<div class="cont-info">
				...
				<dl>
					<dt>품질보증기준</dt>
					<dd>
						품질보증기간: 구입 후 6개월 유통 중 손상되었거나 품질에 이상이 있는 제품에 한하여 소비자피해보상규정 에 의거 보상하여 드립니다. <br>단, 제품에 부착되어 있는 사용방법 및 취급 시 주의사항에 따라 제품을 관리해 주시기 바랍니다.
					</dd>
				</dl>
				<dl>
					<dt>취급시주의사항</dt>
					<dd>
						신발 뒤꿈치를 꺾어 신지 마십시오. 신발에 기름이 접촉하지 않도록 해 주십시오. 불이나 난방기구 근처에는 절대 두지 마십시오. <br>천연 가죽이나 천은 물기 및 마찰에 의해 색깔이 변할 가능성이 있습니다.
					</dd>
				</dl>
			</div>
		</section>
