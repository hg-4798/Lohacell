...
var objImgSrc = document.getElementById("objImg").getAttribute("lsrc");
$("#objImg").zoomImage({ zoomUrl:objImgSrc, bigBorderColor:"#000000", bigBorderWidth:5 });
*/
$(function(){
	// thumb_list objImg
	$(".small_thumb li").click(function(){
		$("#objImg").attr("src",$(this).find("img").attr("src"));
		$("#objImg").attr("lsrc",$(this).find("img").attr("src"));

		$("#objImg").trigger('destroy'); // 객체 초기화
		objImgSrc = document.getElementById("objImg").getAttribute("lsrc");
		$("#objImg").zoomImage({ zoomUrl:objImgSrc, bigBorderColor:"#000000", bigBorderWidth:5 });
	});
});
...
function primage_view(img,type) {
	...
	url = "../front/primage_view.php?scroll="+sc+"&image="+img;
					...
					<input type='hidden' value='<?=$priceDcPercent?>' id = 'ID_priceDcPercent'>
						<script>
							...
							function item_ea_up(conIdx,idx){
								...
								var itemPrice = Number($("#itemPrice-"+conIdx).attr("alt"));// + Number($("#optionPrice-"+conIdx).val());
							...
							/* 키로 입력할 경우     
							$(document).on("keydown", ".amount2", function(e){
							...
							*/
							$(function(){
								$("#option1").change(function(){
									...
									val1 = Number(val1)-1;
									var itemTotalPrice = Number(optpriceArr_[val1]); 
									...
									appendHtml += "			<input type=hidden id='optionPrice-"+controlIdx_+"' class='optionPrice' value='"+Number(optpriceArr_[val1])+"' >";
							...
							function setTotalPrice(){
								var totaltemp = 0; // 옵션별 누적 금액
								var totalea = 0; 
								var itemPrice = 0; // 옵션 가격
								var amount2Count = 0;
								var mainItemPrice = Number($("#ID_sellprice").val())*Number($("#quantity").val());
								$(".amount2").each(function(index){
									...
									var tmp_price = itemPrice;
									itemPrice = Number($("#optionPrice-"+itemIdx).val());
									...
									var noOptionPrice = mainItemPrice;
									var totaltemp2 = Number(totaltemp)+Number(noOptionPrice);
									$("#result_total_price").html("<span class='price_d'><strong>"+jsSetComa(totaltemp2)+"원</strong></span>");
									$("#ID_goodsprice").val(totaltemp2);
								...
								if (amount2Count == 0){
									...
									var tmp_price = Number(tmp_price)*Number(tmp);
									//tmp_price = Number(tmp_price) + Number(mainItemPrice);
									$("#ID_goodsprice").val(tmp_price);
									$("#result_total_price").html("<span class='price_d'><strong>"+jsSetComa(tmp_price)+"원</strong></span>");
									totalea = Number(totalea) + Number(tmp);
									setDeliPrice(totaltemp,totalea);
								}
							}   
