...
?>
		<li>
			<?=$ahref_def?>
				<img src="<?=$Dir.$_pdata->maximage?>" lsrc="<?=$Dir.$_pdata->maximage?>" alt="" style="width:78px;height:78px"/>
			</a></li>
<?php } else { ?>
...
?>
		...
		<?php 	if(is_file($Dir.DataDir."shopimages/product/".$_pdata->maximage)) { 	?>
					<p class="big_thumb"><img id="zoom" lsrc="<?=$Dir.DataDir."shopimages/product/".$_pdata->maximage?>" src="<?=$Dir.DataDir."shopimages/product/".$_pdata->maximage?>"
						name="primg" alt=""  /></p>
		<?php	}else { 	?>
...
<!--// 줌 이미지 스크립트 -->
				...
				<? } ?>
							...
							<div class="priceLayer" id="priceLayer">
									...
									<ul class="article">
										...
										<!--<li>
											...
											<span class="detail">- 2,850원</span>
										</li>-->
									</ul>
									<ul class="article total">
										<li>
											<span class="label">최종 결제금액</span>
											<span class="detail"><strong class="strong"><?=number_format($SellpriceValue-$coumoney)?>원</strong></span>
										</li>
									</ul>
									<ul class="list-a">
					...
					function item_ea_up(conIdx,idx){
						if(!change_quantityOpt("up")) return;
					...
					function item_ea_dn(conIdx,idx){
						if(!change_quantityOpt("dn")) return;
						var goodsprice = $("#ID_goodsprice").val();
						var count_ = $("#quantityea-"+conIdx).val();
						var itemPrice = $("#itemTotalPrice-"+conIdx).val();
						count_ = Number(count_)-1;
					...
					<?php if($_pdata->reserve>0){ ?>
						...
						$("#ID_displyReserv").html(comma((tmp-itemQuantity)*tmp_reserve)+" point");
					<?php } ?>
					...
					$(function(){
						$("#option1").change(function(){
							...
							appendHtml += "	<span class='opt_name'>-<?=mb_substr($_pdata->productname,0,25,'EUC-KR')?>,"+option1Arr_[val1]+"</span> <span class='price' id='itemPrice-"+controlIdx_+"' alt="+itemTotalPrice+">"+jsSetComa(itemTotalPrice)+"원</span>";
							appendHtml += "</div>";
						...
						//$('#option2 option').bind('click', function(){
							...
							appendHtml += "<li id='items-"+controlIdx_+"'>";
							appendHtml += "<div class='item_info_area'>";
							appendHtml += "	<span class='opt_name'>-<?=mb_substr($_pdata->productname,0,25,'EUC-KR')?>,"+option1Arr_[val1]+","+option2Arr_[val2]+"</span> <span class='price' id='itemPrice-"+controlIdx_+"' alt="+itemTotalPrice+">"+jsSetComa(itemTotalPrice)+"원</span>";
							appendHtml += "</div>";
							appendHtml += "<div class='item_editer_area'>";
							appendHtml += "		<div style='float:left;'>";
							appendHtml += "			<input type=text id='quantityea-"+controlIdx_+"' value='1' class='amount2' size = '2' readonly>";
