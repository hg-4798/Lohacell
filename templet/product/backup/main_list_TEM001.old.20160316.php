...
while ( $row = pmysql_fetch_array($result) ) {
    if ( $idx == 0 ) {
        $brand_list_html .= '<div class="list">';
        $brand_list_html .= '<div class="js-scroll scroll-wrap4">';
		$brand_list_html .= '<ul class="">';
    } elseif ( $idx % $countPerList == 0 ) {
        $brand_list_html .= '</ul>';
        $brand_list_html .= '</div>';
        $brand_list_html .= '</div>';
        $brand_list_html .= '<div class="list">';
    ...
    $brand_list_html .= '<li><a href="/front/brand_detail.php?bridx=' . $row['bridx'] . '">' . $row['brandname'] . '</a></li>';
    $idx++;
}
$brand_list_html .= '</ul>';
$brand_list_html .= '</div>';
...
$brand_list_html = '';
$brand_list_html .= '<div class="list brand-all">';
$brand_list_html .= '<div class="js-scroll scroll-wrap4">';
$brand_list_html .= '<ul class="brand-cate-all">';
...
$today_pick_html = '';
while ($row = pmysql_fetch_array($result) ) {
    $prod_sql  = "SELECT productcode, productname, sellprice, consumerprice, brand, maximage, minimage, tinyimage, over_minimage, mdcomment, review_cnt, icon ";
...
$prod_sql  = "SELECT productcode, productname, sellprice, consumerprice, brand, maximage, minimage, tinyimage, mdcomment, over_minimage, review_cnt, icon ";
...
while ( $row = pmysql_fetch_array($result) ) {
    ...
    if ( $arrProd[0] ) {
        ...
        while ( $review_row = pmysql_fetch_array($review_result) ) {
            ...
            // 등록일
            $reg_date = substr($review_row[date], 0, 3) . "." . substr($review_row[date], 4, 2) . "." . substr($review_row[date], 6, 2);
...
$sql .= "WHERE code_a = '" . $cate_code . "' AND code_b <> '000' AND code_c = '000' AND group_code !='NO' AND display_list is NULL ";
...
foreach ( $arrSecondCategory as $key => $arrData ) {
    ...
    $sql .= "AND code_c <> '000' AND code_d = '000' AND group_code !='NO' AND display_list is NULL ";
    $sql .= "ORDER BY cate_sort ASC ";
    $result = pmysql_query($sql);
    ...
    $item_list_html .= ' 
                <div class="list">
                        <h3><a href="../front/productlist.php?code=' . $secondCateCode . '">' . $code_b_name . '</a></h3>
                        <div class="js-scroll scroll-wrap' . $item_idx . '">
                            <ul class="" >';
    ...
    while ( $row = pmysql_fetch_array($result) ) {
        if ( $row['code_c'] === "000" ) {
            ...
            $item_list_html .= '<li><a href="' . $Dir.FrontDir."productlist.php?code=" . $row['cate_code'] . '">' . $row['code_name'] . '</a></li>';
        }
    }
    pmysql_free_result($result);
