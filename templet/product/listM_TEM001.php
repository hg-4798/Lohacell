	...
	<section class="page_local">
		<h2 class="page_title">
			...
			<script type="text/javascript">
				cate_name = "<?=$brand_idx?$brand_name:$thisCate [1]->code_name?>";
			</script>
		</h2>
...
function list_ajax(){
	$.ajax({
		...
		success: function(data) {
			...
			var pdtCode;
			for(var i = 1; i < 11; i++) {
				hrefData = $(".goods-list-ajax ul li:nth-child(1) a").attr("href");
				splData = hrefData.split("&");
				pdtCode = splData[0].split("=");
				fb_ids=fb_ids + "{" + pdtCode[1] + "},";
			}
			fbq('track', 'ViewCategory', {
			  content_name: "'" + cate_name + "'",
			  content_ids: "["+fb_ids.substring(0,fb_ids.length-1)+"]", // top 5-10 results
