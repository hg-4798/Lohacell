...
<?php 	if(is_file($Dir.DataDir."shopimages/product/".$_pdata->maximage)) { 	?>
			<img src="<?=$Dir.DataDir."shopimages/product/".$_pdata->maximage?>" alt="" style="width: 285px;" />
...
<?php	}	?>
		...
		<form name=form1 id = 'ID_goodsviewfrm' method=post action="<?=$Dir.FrontDir?>basket.php">
			...
			<table class="goods_info">
				...
				<tr>
					...
					<td><?=number_format($_pdata->consumer_reserve)?>원</td>
				</tr>
				<?}?>
				<tr>
							...
							<ul class="aList selectOption1">
								<?for($i=1;$i<$count;$i++) {?>
									...
									<?if(strlen($tok[$i]) > 0) {?>
										...
										<?}else{?>
											<a href="javascript:;" opt = '<?=$i?>' opt1='<?=$optprice[$i-1]?>'><?=$tok[$i]?>&nbsp;(<?=number_format($optprice[$i-1])?>원) </a><?//." (".number_format($optprice[$i-1])."원) (".$optcode[$i-1].")"?>
										<?}?>
									<?}?>
									</li>
								<?}?>
				...
				<tr>
					<td colspan="2">
						<ul class="opt_list">
							...
							<li>-화이트5호 <p>20,000원 <img src="../img/btn/btn_close03.gif" alt="삭제" class="btn_opt_del" /></p></li>
							<li>-화이트5호 <p>20,000원 <img src="../img/btn/btn_close03.gif" alt="삭제" class="btn_opt_del" /></p></li>
			...
			?>
							<?if (strlen($_ShopInfo->getMemid())>0 && $_ShopInfo->getMemid()!="deleted") {?>
								<a href="javascript:CheckForm('ordernow','<?=$opti?>');" class="first"><img src="../img/btn/btn_buy_now.gif" alt="바로구매" /></a><a href="javascript:CheckForm('','<?=$opti?>');" class="btn_cart"><img src="../img/btn/btn_cart.gif" alt="장바구니" /></a><a href="javascript:CheckForm('wishlist','<?=$opti?>')" class="btn_wishlist"><img src="../img/btn/btn_wishlist.gif" alt="관심상품" /></a>
							<?} else {?>
								<a href="javascript:CheckForm('ordernow','<?=$opti?>');" class="first"><img src="../img/btn/btn_buy_now.gif" alt="바로구매" /></a><a href="javascript:CheckForm('','<?=$opti?>');" class="btn_cart"><img src="../img/btn/btn_cart.gif" alt="장바구니" /></a><a href="javascript:check_login();" class="btn_wishlist"><img src="../img/btn/btn_wishlist.gif" alt="관심상품" /></a>
							<?}?>
			<?
						}
					}
		...
		<div class="right_info_wrap">
			...
			<div class="mini_review">
				...
				<?include("prreview_tem001_right.php")?>
			</div>
			<script>
				$(function(){
	...
	<div class="related_goods_wrap">
		<div class="title">
			...
			<p class="btn">
				<a href="javascript:vode(0);" data-target="prev" class="left"></a>
				<a href="javascript:vode(0);" data-target="next" class="right"></a>
			</p>
		</div>
		<div class="related_goods">
		<div class="related_goods_slider">
<?php
			$connectionCode = substr($productcode,0,12);
			$sql2 = "
			SELECT productcode,productname,sellprice,mdcomment,tinyimage 
			FROM tblproduct 
			WHERE productcode LIKE '{$connectionCode}%' 
			...
			while($row2 = pmysql_fetch_array($res2)){
				$connectionPr[] = $row2;
			}
...
?>
			<!--<ul class="list">-->
		<?foreach($connectionPr as $mv){?>
			<?if($pr_cnt%3==0){?>
			<ul class="list">		
			<?}?>
				<li>
					<p class="img">
						<a href="productdetail.php?productcode=<?=$mv["productcode"]?>">
					<?if(is_file($Dir.DataDir."shopimages/product/".$mv["tinyimage"])){?>
						<img src="<?=$Dir.DataDir."shopimages/product/".$mv["tinyimage"]?>" alt="" style="width: 125px" />
					<?}elseif(is_file($Dir.$mv["tinyimage"])){?>
						<img src="<?=$Dir.$mv["tinyimage"]?>" alt="" style="width: 125px" />
					<?}else{?>
						<img src="<?=$Dir?>images/no_img.gif" alt="" style="width: 125px" />
					<?}?>
						</a>
					</p>
					<div class="info">
						<p class="title"><a href=""><?=$mv['productname']?></a></p>
						<p class="price"><?=number_format($mv['sellprice'])?> 원</p>
					</div>
				</li>
		<?	$pr_cnt++;
			if($pr_cnt%3==0){?>
			</ul>
		<?	}elseif($pr_cnt==count($connectionPr)){?>
			</ul>
		<?	}?>
		<?}?>
			<!--</ul>-->
	...
	<div class="delivery_info_wrap">
		<div class="goods_view_tap" id="tap1">
			<ul class="tap_list">
				<li class="on"><a class="on" href="javascript:clickScoll(1);">제품상세정보<em class="on"></em></a></li>
				<li><a href="javascript:clickScoll(2);">배송/반품/교환<em></em></a></li>
				<li><a href="javascript:clickScoll(3);">상품후기<em></em></a></li>
				<li><a href="javascript:clickScoll(4);">상품Q&A<em></em></a></li>
			</ul>
		...
		<div class="delivery_info_wrap">
			...
			<div class="goods_view_tap" id="tap2" >
				<ul class="tap_list">
					<li><a href="javascript:clickScoll(1);">제품상세정보<em></em></a></li>
					<li class="on"><a class="on" href="javascript:clickScoll(2);">배송/반품/교환<em class="on"></em></a></li>
					<li><a href="javascript:clickScoll(3);">상품후기<em></em></a></li>
					<li><a href="javascript:clickScoll(4);">상품Q&A<em></em></a></li>
				</ul>
			</div><!-- //div.goods_view_tap -->
		...
		<div class="view_page_reivew_wrap">
			<div class="goods_view_tap" id="tap3">
				<ul class="tap_list">
					<li><a href="javascript:clickScoll(1);">제품상세정보<em></em></a></li>
					<li><a href="javascript:clickScoll(2);">배송/반품/교환<em></em></a></li>
					<li class="on"><a class="on" href="javascript:clickScoll(3);">상품후기<em class="on"></em></a></li>
					<li><a href="javascript:clickScoll(4);">상품Q&A<em></em></a></li>
				</ul>
			</div><!-- //div.goods_view_tap -->
		...
		<div class="view_page_qna_wrap">
			<div class="goods_view_tap" id="tap4">
				<ul class="tap_list">
					<li><a href="javascript:clickScoll(1);">제품상세정보<em></em></a></li>
					<li><a href="javascript:clickScoll(2);">배송/반품/교환<em></em></a></li>
					<li><a href="javascript:clickScoll(3);">상품후기<em></em></a></li>
					<li class="on"><a class="on" href="javascript:clickScoll(4);">상품Q&A<em class="on"></em></a></li>
				</ul>
			</div><!-- //div.goods_view_tap -->
