...
$sort=$_REQUEST["sort"]?$_REQUEST["sort"]:"recent";
...
foreach ( $arrSecondDepthCate as $arrCateInfo ) {
    ...
    if ($s_sort == $firstCateCode . $secondCateCode . "000000") {
        ...
        //exdebug($s_sort. " / ".$firstCateCode . $secondCateCode . "000000");
	}
    /*
	$sub_sql = "SELECT code_a,code_b,code_c,code_d, code_a||code_b||code_c||code_d as cate_code,code_name,idx FROM tblproductcode ";
	$sub_sql .= "WHERE code_a = '" . $firstCateCode . "' AND code_b = '" . $secondCateCode . "' AND code_c <> '000' AND code_d = '000' ";
				...
				<div class="sorting">
					...
					<section class="sorting-color on">
						...
						<?foreach($arrDbColorCode as $ck => $cv){?>
							<li id="color_<?=$cv->cno?>"><label><input type="checkbox"
						...
						<ul>
							...
							<li>
								<div>
									...
									<input type="radio" id="sorting_check05" name="sorting_check" value="price" <?=$checked['sorting_check']['price']?>>
								</div>
							</li>
							<li>
								<div>
									<label for="sorting_check04">높은가격순</label>
									<input type="radio" id="sorting_check06" name="sorting_check" value="price_desc" <?=$checked['sorting_check']['price_desc']?>>
								</div>
							</li>
						</ul>
					</section>
...
$(document).ready(function() {
	...
	$(".second_category").change(function() {
        ...
        var param = {"code":code, "color":color_name, "sort":sort, "soldout":soldout, "brand":brandCode, "size":sizeCode};
...
// 		$(location).attr('href',url);
	...
	$(".third_category").change(function() {
        ...
        var param = {"code":code, "color":colorCode, "sort":sort, "soldout":soldout, "brand":brandCode, "size":sizeCode};
		//var param = {"code":code};
// 		var url = "../m/productlist.php?code="+code;
// 		$("input[name=category_code]").val(code);
// 		$(location).attr('href',url);
	...
	$(".btn-submit").click(function() {
        ...
        //alert($("#s_sort").val());
        var code = oParams.code;
        if($("#s_sort").val()) code = $("#s_sort").val();
        if($("#t_sort").val()) code = $("#t_sort").val();
        
		//var param = {"code":oParams.code, "color":color_name, "sort":sort, "brand":brandCode, "size":sizeCode, "soldout":soldout};
        var param = {"code":code, "color":colorCode, "sort":sort, "brand":brandCode, "size":sizeCode, "soldout":soldout};
		$.ajax({
			type: "POST",
			url: "../m/ajax_product_search.php",
			data: param,
