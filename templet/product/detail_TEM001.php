...
if($_ShopInfo->cooper_yn == 'Y'){
	...
	if(strlen($tsale_num) > 11){
		$sale_num	= substr($tsale_num, -2);
	}else{
...
function getDBProduct(prodcode, productcode){
	...
	//임직원가
	if(staff_yn=='Y'){
		if(productcode == '001004001006000031'){
		}else{
	...
	if(cooper_yn=='Y'){
		...
		case '10':
			...
			break;
		case '11':
			var sale_conprice = p.pArr[productcode].sale_price11;
			break;
		case '12':
			var sale_conprice = p.pArr[productcode].sale_price12;
			break;
		case '13':
			var sale_conprice = p.pArr[productcode].sale_price13;
			break;
		case '14':
			var sale_conprice = p.pArr[productcode].sale_price14;
			break;
		case '15':
			var sale_conprice = p.pArr[productcode].sale_price15;
			break;
		default :
			alert('제휴사 회원이 아닙니다.');
			return false;
	...
	if(prop_option !='') {
		...
		jungbogosi += "		</tr>";
		jungbogosi += "		<tr>";
		jungbogosi += "			<th scope=\"row\" colspan=2>*착용된 모든 엑세서리는 코디용 아이템입니다.</td>";
		jungbogosi += "		</tr>";
...
<script>
	...
	value: '<?=$_pdata->sellprice?>',
	currency: 'KRW'
	});

function basket_fb_in(){

	price = '<?=$_pdata->sellprice?>';
	p_quan=$("#quantity").val();
	var total_sellprice = price*p_quan;
	
	fbq('track', 'AddToCart', {
		content_ids: ['<?=$productcode?>'],
		content_type: 'product',
		value: total_sellprice,
		currency: 'KRW'
	});
  }
</script>
				...
				<div class="goods-info-area clear">
					...
					<div class="specification">
						<section class="box-intro">
							...
							<div class="price">
								<?if($_REQUEST[productcode] == '001004001006000031' && $_ShopInfo->staff_yn=="Y"){?>
								<label>임직원가</label>
								<?}else{?>
								<label>판매가</label>
								<?}?>
						...
						<section class="box-price">
							...
							<div class="buy-btn clear">
								...
								<?
									...
									if($_REQUEST[productcode] == '001004001006000031'){
								?>
									<?if( $_pdata->quantity <= 0 || $_pdata->soldout == 'Y' ) {?>
									<a href="javascript:alert('품절된 상품입니다.');" class="btn-point w100-per">품절된 상품입니다.</a>
									<?php
										...
										if($mem_auth_type!='sns') {
										?>
									<a href="javascript:order_check('<?=strlen( $_ShopInfo->getMemid() )?>','N');" class="btn-point w100-per">임직원 구매</a>
										<?
										} else {
										?>
								...
								<ul class="mt-10">
									...
									<?} else {?>
										<li><button class="btn-line" type="button" onclick="javascript:order_check('basketI'); basket_fb_in();"  id="cartBtn"><span><i class="icon-cart mr-10" id="car_main"></i>장바구니</button></span></li>
									<?}?>
									<li>
