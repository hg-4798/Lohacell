			...
			<article class="mypage_content">
				<section class="mypage_main">
					<ul class="my-tab-menu clear">
						<li class="<?=$viewtab['online']?>"><a>온라인 쿠폰</a></li>
						<li class="<?=$viewtab['offline']?>"><a>오프라인 쿠폰</a></li>
					</ul>
...
<?
		...
		$sql = "SELECT
					a.*
					FROM
					tblcoupon_erp a LEFT JOIN tblmember b ON a.mem_seq = b.mem_seq
					WHERE   b.id = '".$_ShopInfo->getMemid()."'
					AND     (a.close_date >= '".date("Ymd")."' and a.useyn = '0')
					AND     ( 
						(a.issue_date <= '".str_replace( '-', '', $strDate1)."' and a.close_date >= '".str_replace( '-', '', $strDate1)."') 
						or
						(a.issue_date <= '".str_replace( '-', '', $strDate2)."' and a.close_date >= '".str_replace( '-', '', $strDate2)."') 
						or
						(a.issue_date >= '".str_replace( '-', '', $strDate1)."' and a.close_date <= '".str_replace( '-', '', $strDate2)."')  
					)
					ORDER BY a.close_date DESC, a.cno desc";
...
<?
		...
		if ($r_t_count > 0) {
			while($row=pmysql_fetch_object($result)) {
                ...
                $couponcheck="";
                if($row->issue_date>$coupondate || $row->close_date<$coupondate){
                    if($row->useyn=="1"){
                        $couponcheck="사용";
                    }else{
                        $couponcheck="사용불가";
               ...
               $t = sscanf($row->issue_date,'%4s%2s%2s');
               $s_time = strtotime("{$t[0]}-{$t[1]}-{$t[2]}");
               $t = sscanf($row->close_date,'%4s%2s%2s');
               $e_time = strtotime("{$t[0]}-{$t[1]}-{$t[2]}");

               $date=date("Y.m.d",$s_time)."시~".date("Y.m.d",$e_time)."시";
?>
								<tr>
									<td>-</td>
									<td class="bold"><?=number_format($row->amt).$dan.' '.$sale?></td>
                                    <td><?=number_format($row->amt).$dan.' '.$sale?></td>
									<td><?=$couponcheck?></td>
									<td>-</td>
									<td><?=$date?></td>
								</tr>
