...
<script type="text/javascript">
	...
	function nowtime(){
		...
		if(gaptime>0){
			...
			if(gapDate>0){
				$("div.sale_time02 p.day").html(gapDate);
			}else{
				$("div.sale_time02 p.day").html("0");
			}
			$("div.sale_time02 p.hour").html(gapH);
			$("div.sale_time02 p.min").html(gapM);
		}else{
			$("div.sale_time02 p.hour").html("00");
			$("div.sale_time02 p.min").html("00");
		}
...
<div class="main_wrap">
	<div class="container1100 pos_r">
		...
		<div class="line_map_r">
			<a href="#">홈</a> > <a href="#"><em>VIP ZONE</em></a>
		</div><!-- //라인맵 -->
				...
				<div class="right">
					...
					<div class="today_sale_table">
						...
						<table class="price_info" cellpadding=0 cellspacing=0 border=0 width="500">
							...
							<tr>
								<th>판매가</th>
								<td class="price"><span><?=number_format($_pdata->consumerprice)?></span> 원</td>
								<td rowspan=2></td>
							</tr>
							<tr>
								<th>특가</th>
								<td class="dc"><span><?=number_format($_odata['s_price'])?></span> 원</td>
							</tr>
						</table>
						</div>
					</div>
					...
					<ul class="time_sale_opt">
						<li class="ment">상세옵션을 선태해 주세요.(1인당 최대 5개 구매가능, <span>50,000원 이상 무료배송</span>)</li>
					...
					?>					
						...
						<li class="mt_10">
							<div class="select_type open" style="width:500px; z-index:70">
					...
					?>
						...
						<li class="mt_10">
							<div class="select_type open" style="width:500px; z-index:60">
								<span class="ctrl"><span class="arrow"></span></span>
					...
					<div class="mt_30">
						<a href="javascript:CheckForm('ordernow','<?=$opti?>');"><img src="../img/button/btn_buy_nowL.jpg" alt="즉시 구매하기" style="width: 100%" /></a>
						<!--<a href="javascript:CheckForm('','<?=$opti?>');"><img src="../img/button/btn_cart.gif" alt="장바구니" /></a>-->
						
					<?php if (strlen($_ShopInfo->getMemid())>0 && $_ShopInfo->getMemid()!="deleted"){ ?>
						<!--<a href="javascript:CheckForm('wishlist','<?=$opti?>')"><img src="../img/button/btn_wishlist.gif" alt="위시리스트" /></a>-->
					<?php }else{ ?>
						<!--<a href="javascript:check_login();"><img src="../img/button/btn_wishlist.gif" alt="위시리스트" /></a>-->
					<?php } ?>
					</div>
		...
		<!-- 다른공구 -->
		<?php
			$sql_other = "SELECT a.*,to_char(a.sdate,'YYYYMMDDHH24MISS') as s_date, to_char(a.edate,'YYYYMMDDHH24MISS') as e_date, b.maximage, b.minimage, b.tinyimage FROM tbl_timesale_list as a ";
			$sql_other.= "LEFT JOIN tblproduct as b ON a.productcode=b.productcode ";
			$sql_other.= "WHERE a.view_type='1' ";
			$sql_other.= "AND a.sdate < '{$today_date}' ";
			$res_other = pmysql_query($sql_other);
		?>
		<div class="other_gongu_wrap">
			<h3>다른 공동구매<a href="#">more+</a></h3>
			<div class="gongu_list_wrap">
				<a href="#" class="btn_left best_slider_btn" data-target="next"></a>
				<a href="#" class="btn_right best_slider_btn" data-target="prev"></a>
				<div class="gongu_list">
					<ul  style="z-index: 100;">
					<?php
						while($row_other = pmysql_fetch_array($res_other)){
							if($row_other['e_date']<date("YmdHis")){
					?>
						<li>
							<a href="<?=$Dir.FrontDir."timesale.php?sno=".$row_other['sno']?>"><img src="<?=$Dir.DataDir."shopimages/product/".$row_other[minimage]?>" alt="" style="width: 243px; height: 175px;"/></a>
							<p class="end"></p>
						</li>
					<?php			
							}else{
					?>
						<li><a href="<?=$Dir.FrontDir."timesale.php?sno=".$row_other['sno']?>"><img src="<?=$Dir.DataDir."shopimages/product/".$row_other[minimage]?>" alt="" style="width: 243px; height: 175px;"/></a></li>
					<?php
							}
						}
					?>
