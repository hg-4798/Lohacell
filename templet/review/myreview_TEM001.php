...
# 작성하지 않은 리뷰 리스트
# ====================================================================================================================================
$limit_date = date("Ymdhis",strtotime("-14 day") );
...
$sql .= " and a.order_conf_date >= '{$limit_date}' "; // 구매후 14일이 지난 상품에 대해서는 리뷰 쓰기 금지
$sql .= "       WHERE b.id = '" . $_ShopInfo->getMemid()  . "' and ( a.op_step = 3 OR a.op_step = 4 ) "; // 상태를 상품별로 변경 (2016.07.13 - 김재수 * 결제완료에서 리뷰쓰기를 했을경우 구매확정이 된 오류)
...
if ( $isMobile ) {
    ...
    $paging = new amg_Paging($sql, 10, $listnum, 'GoPageAjax', "1||{$whereDate1}||{$whereDate2}");
}
...
if ( !empty($whereDate1) && !empty($whereDate2) ) {
    $sql2 .= "AND ( tpr.date >= '{$whereDate1}' AND tpr.date <= '{$whereDate2}' ) ";
}
...
if ( $isMobile ) {
    ...
    $paging2 = new amg_Paging($sql2, 10, $listnum, 'GoPageAjax', "2||{$whereDate1}||{$whereDate2}");
}
    ...
    <div class="containerBody sub-page">
        ...
        <div class="right_section mypage-content-wrap">
            ...
            <div class="my-review-list">
                ...
                <table class="th-top util top-line-none">
                    ...
                    while ( $row = pmysql_fetch_object($result) ) {
                        ...
                        <tr>
                            <td><?=$order_date?></td>
                            <td><a href="../front/productdetail.php?productcode=<?=$row->productcode?>"><img class="img-size-mypage" src="<?=getProductImage($Dir.DataDir.'shopimages/product/',$sub_row->tinyimage)?>"></a></td>
                            <td class="ta-l">
                                <span class="brand-color"><?=$sub_row->brandname?></span><br>
                                <span><?=strip_tags($sub_row->productname)?></span><br>
            ...
            <div class="my-review-write-list">
                ...
                <div class="mypage-title">작성한 상품리뷰 <span id="product_writed_review_count">Total(<?=number_format($t_count2)?>)</span></div>
                <!-- 날짜 설정 -->
...
//            $marks .= '★';
                        ...
                        <tr class="my-write-review" ids="<?=$row->num?>">
                            ...
                            <td>
                                <a href="../front/productdetail.php?productcode=<?=$row->productcode?>">
                                    <?
                                    if(strlen($sub_row->tinyimage)!=0) {
                                        echo "<img class=\"img-size-mypage\" src=\"".getProductImage($Dir.DataDir.'shopimages/product/',$sub_row->tinyimage)."\" style='max-width:80px' border=0>";
                                    }else {
                                        echo "<img class=\"thumb-mypage\" src=\"".$Dir."images/no_img.gif\" width=\"\" border=\"0\">";
                                    }
                                    ?></a>
                            </td>
                            <td class="ta-l">
                                ...
                                <div class="list-tr-open">
                                    <div class="review_contents my-wirte">
                                        ...
                                        <div class="btn-place">
                                            ...
                                            <!--<button class="btn-dib-line " type="button" onClick="javascript:delete_review('<?=$row->num?>');"><span>삭제</span></button>--> <!-- 쿠폰지급 받고 삭제 할 경우를 예방하기위해 삭제 버튼 제거 20170927 bshan -->
                                        </div>
                                    </div>
                                </div>
                            </td>
                ...
                <div class="paging" id="product_writed_review_paging">
                    <?=$a_div_prev_page.$paging2->a_prev_page.$paging2->print_page.$paging2->a_next_page.$a_div_next_page?>
                </div>
    ...
    <!-- margin-left: 335px; 삭제 20170901 김광호대리 -->
    <div style="padding-top:30px;">

        <!-- PC 리뷰페이지 하단 배너 이미지 -->
        <?if(date("Ymd") >= '20180101'){?>
            <?php
            if( count( $review_banner ) > 0 ) { //리뷰베너
                ...
                <center><img src="<?=$review_banner[94][1]['banner_img']?>" alt=""></center>
                <?php
                if ( !empty($review_banner[94][1]['banner_link']) ) {
                    ?>
                    </a>
            ...
            ?>
            <?
        }else if(date("Ymd") < '20180101'){
            ?>
            <center><img src="PC_product detail bottom banner_review.jpg" style=""></center>
        <?}?>
    ...
    <SCRIPT LANGUAGE="JavaScript">
        ...
        function GoPageAjax(block,gotopage,mode,start_date,end_date) {
            var s_date = '';
            var e_date = '';

            if ( start_date == undefined && end_date == undefined ) {
                var arrTmp = mode.split("||");
    ...
    <script LANGUAGE="JavaScript">
        ...
        function CheckForm(frm_id) {
            ...
            var start_date = frm.s_year.value + frm.s_month.value + frm.s_day.value + "000000";
            var end_date = frm.e_year.value + frm.e_month.value + frm.e_day.value + "235959";

            GoPageAjax(0, 0, mode, start_date, end_date);
