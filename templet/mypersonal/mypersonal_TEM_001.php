...
$date_s = ($_POST['date_s'])?$_POST['date_s']:date('Y-m-d', strtotime('-1 month'));
$date_e = ($_POST['date_e'])?$_POST['date_s']:date('Y-m-d');
						...
						<header class="my-title mt-40">
							...
							<div class="date-sort clear">
								...
								<div class="type calendar">
									<div class="box">
										<div><input type="text" title="일자별 시작날짜" name="date_s" value="<?=$date_s?>" id="date_s" readonly class="datepicker"></div>
										<button type="button" class="btn_calen CLS_cal_btn">달력 열기</button>
									</div>
									<span class="dash"></span>
									<div class="box">
										<div><input type="text" title="일자별 시작날짜" name="date_e" value="<?=$date_e?>" id="date_e" readonly class="datepicker"></div>
										<button type="button" class="btn_calen CLS_cal_btn">달력 열기</button>
									</div>
								</div>
										...
										<td class="reset" colspan="4">
											...
											?>
												...
												<div class="answer editor-output">
													<div class="answer-user"><span>관리자 <em>|</em><?=date("Y.m.d", strtotime( $val['re_date']) )?></span></div>
													<p><?=$val['re_subject']?></p>
													<p><?=nl2br($val['re_content'])?></p>
												</div>
											<?
												}
											?>
						...
						<tr>
							...
							<td>
								<div class="input-cover">
									...
									<span class="txt">@</span>
									<input type="text" id="email2" name="email2" value="" title="도메인 직접 입력" class="mr-10" style="width:150px; display: none;" >
									<div class="select">
										<select style="width:170px" title="이메일 도메인 선택" onchange="customChk(this.value);" id="email_select">
											<option value="">선택</option>
											<option value="custom">직접입력</option>
											<? foreach($email_domain_arr as $v) {?>
						...
						<tr>
							...
							<td>
								<div class="input-cover">
									...
									<span class="txt">-</span>
									<input type="text" title="휴대폰 가운데 번호 입력" style="width:110px" id="phone_num2" maxlength="4" onkeyup="strnumkeyup(this);">
									<span class="txt">-</span>
									<input type="text" title="휴대폰 마지막 번호 입력" style="width:110px" id="phone_num3" maxlength="4" onkeyup="strnumkeyup(this);">
								</div>
							</td>
						</tr>
						<tr>
...
<input type="hidden" name="date_s" value="<?=$date_s?>">
<input type="hidden" name="date_e" value="<?=$date_e?>">
...
$(document).ready(function(){
	...
	$("#btnSubmit").click(function(){
		if(check_form()){
            ...
            if(document.getElementById("up_filename").value!=""){
                var file = document.getElementById("up_filename");
                var filename = document.getElementById("up_filename").value;
                ...
                if(fileSize > maxSize)
                {
                    alert("첨부파일 사이즈는 10MB 이하로 등록 가능합니다.");
                    return false;
                }
                ...
                if(_fileExt !=".jpg" && _fileExt !=".gif" && _fileExt !=".bmp" && _fileExt !=".png"){
                    alert('.jpg .gif .bmp .png만 등록 가능합니다.');
                    return false;
                }
            }
			document.getElementById("email").value = $('#email1').val() +'@'+ $('#email2').val();
			document.getElementById("mobile").value = document.getElementById("phone_num1").value +"-"+document.getElementById("phone_num2").value +"-"+document.getElementById("phone_num3").value;
			$("#write_form").submit();
		}
	});
...
function CheckForm() {
	...
	frm.date_s.value = $('#date_s').val();
	frm.date_e.value = $('#date_e').val();
...
function editForm(idx,table) {
	...
	$.ajax({
		...
		var phones = data[0]['mobile'].split("-");
		if(phones[0] != null){
