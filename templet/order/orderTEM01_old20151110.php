...
<?
	...
	while($vgrp=pmysql_fetch_object($res)) {
		...
		$sql = "SELECT a.opt1_idx,a.opt2_idx,a.optidxs,a.quantity,a.optionarr,a.quantityarr,a.pricearr,b.productcode,b.productname,b.sellprice,b.membergrpdc, b.option_reserve, ";		
		...
		$deli_productprice=0;
		$deli_init = false;
		$total_reserve = 0;	//즉시적립을 위한 전체 적립금
		...
		*/ 
		while($row = pmysql_fetch_object($result)) { 
			...
			//수량별 회원 판매금액 적용
			$chagePrice = exchagePrice($row->productcode,$row->quantity,$_ShopInfo->memgroup);
			if($chagePrice>0) $row->sellprice = $chagePrice;
			
			if (strlen($row->option_price)>0 && $row->opt1_idx=='') {
		...
		# 옵션가격 가져오기
		$optSql = "SELECT option_price FROM tblproduct_option WHERE option_code = '".trim($row->optionarr)."' ";
		$optRes = pmysql_query( $optSql );
		$optRow = pmysql_fetch_object( $optRes );
		$selectOptionPrice = $optRow->option_price;
		pmysql_free_result( $optRes );
		
		$price = $selectOptionPrice*$row->quantity;
		$total_price_ = $row_sellprice * $row->quantity;
		
		$tempreserve = getReserveConversion($row->reserve,$row->reservetype,$selectOptionPrice,"N");
		...
		if($timesale_sellprice > 0) {
			$total_price_ = $timesale_sellprice * $row->quantity;
			$salemoney_ = $row_sellprice - $timesale_sellprice;
		}
		//비회원이면 적립금 노출 X
...
?>
							...
							<?
								...
								if (strlen($row->option1)>0 || strlen($row->option2)>0 || strlen($optvalue)>0) {// 특징 및 선택사항이 있으면
									for ($i=0;$i<sizeof($ex_optionarr);$i++) {
										$_option_price = 0;
										// 옵션 가격을 불러오기위해 option table 조회
										$opPriceSql = "SELECT option_price FROM tblproduct_option WHERE productcode='".$row->productcode."' AND option_code ='".trim($ex_optionarr[$i])."' ";
										$opPriceRes = pmysql_query( $opPriceSql, get_db_conn() );
										$opPriceRow = pmysql_fetch_object( $opPriceRes );
										if ($opPriceRow->option_price>0){
											$_option_price = $opPriceRow->option_price*$ex_quantityarr[$i];
										}

										if ($ex_optionarr[$i]) {
											$tok1 = explode(",", $row->option1);
											$tok2 = explode(",", $row->option2);
											$optionarr = explode(chr(30),$ex_optionarr[$i]);
											$optionString = "옵션 : ".$tok1[0]." ".$optionarr[0];
											if( $optionarr[1] ){
												$optionString.= " / ".$tok2[0]." ".$optionarr[1];
											}
											echo "<span class=\"option\">".$optionString."</span>&nbsp;";	
										...
										if ((sizeof($ex_optionarr)-1) != $i) {
											echo "&nbsp;,&nbsp;";
										}
										$total_opotion_price = $total_opotion_price + $_option_price;
										$row_optionPrice += $_option_price;
									}
					...
					<? if($timesale_sellprice > 0) { ?>
						<td><strong><?=number_format($timesale_sellprice)?></strong></td>
					<? }else{ ?>
...
<?
	...
	if($_data->deli_basefee>0) {
		if($_data->deli_basefeetype=="Y") {
			# 상품가 + 옵션가
			$vender_delisumprice = $vender_sumprice + $row_optionPrice;
		}
						...
						<td colspan="7" bgcolor="#fafafa">
							<div class="result_box">
								...
								<span class="total">
									...
									<strong class="number" id="delivery_price2"><?=number_format($deli_price)?> 원</strong>
								</span>
								<img class="icon" src="../img/icon/cart_list_icon_equals.gif" alt="=" />
								<span class="total_payment">
									<span class="txt">총 결제 금액</span>
									<strong class="number" id="price_sum2"><?=number_format($bf_sumprice+$deli_price)?><span>원</span></strong>
								</span>
							</div>
						</td>
					</tr>
			...
			<div class="order_area">
						...
						<tbody>
							<tr>
								...
								<td class="name">
									...
									?>
										<input type=text  name="sender_name" value="<?=$userName?>" readonly style="font-weight:bold" style='border:0' required msgR="주문하시는분의 이름을 적어주세요">
									<?
										} else {
									?>
				...
				<div class="dc_area" style=''>
					...
					<table class="info_table" summary="할인받을 쿠폰 및 적립금을 입력할 수 있습니다.">
						...
						<tr>
							<th scope="row">즉시 적립</th>
							<td>
								<input type="checkbox" name="beforehand_reserve" id="beforehand_reserve" value="Y"/>
								* 적립될 적립금을 즉시 사용하는 기능입니다.
					...
					<table class="info_table" summary="결제수단을 선택할 수 있습니다.">
						...
						<tbody>
							<tr>
								...
								<td>
									<ul class="means CLS_paymentArea">
							...
							<tr>
								<th scope="row">배송방법 선택</th>
								<td>
									<ul class="means">
										<li><input type="radio" name="delivery" checked="checked" value="1">택배</li>
										<li>&nbsp;&nbsp;</li>
				...
				<div class="payment_area">
					...
					<div class="content">
						...
						<ul>
							...
							<li>
								<span class="txt">즉시적립</span>
								<span class="price CLS_beforehand_reserve">0원</span>
							</li>
							<li>
					...
					<div class="button_box">
						<div id="paybuttonlayer" name="paybuttonlayer" style="display:block;">
							<a href="javascript:CheckForm()" onmouseover="window.status='결제';return true;" target="_self" class="btn_A" style="width:144px">결제하기</a>
						...
						<div id="payinglayer" name="payinglayer" style="display:none;">
							<table border=0 cellpadding=0 cellspacing=0 width=100%>
								<tr>
									<td align=center><img src="<?=$Dir?>img/common/paying_wait.gif" border=0></td>
								</tr>
							</table>
						</div>
					</div>
...
$(document).ready(function(){
	var price = numberFormat('<?=$bf_sumprice?>');
	var deli_price = numberFormat('<?=$deli_price?>');
	var total_price = Number('<?=$bf_sumprice?>')+Number('<?=$deli_price?>');
