...
?>
			...
			<p class="save-ment">상품은 최대 30일 보관되며 상품 소진 시 삭제됩니다.</p>
			...
			<table class="th-top util CLS_basketTotalCount" summary="담은 상품의 정보, 판매가, 수량, 할인금액, 결제 예정가, 적립금을 확인할 수 있습니다.">
				...
				<thead>
					<tr>
						...
						<th scope="col">수량</th>
						<th scope="col" style="display:none;">옵션</th>
...
?>
					...
					<tr name='product_list' >
						<td><!-- <?=$product['soldout']?> -->
							<!--회원전용 상품을 비회원이 구매시에 구매못하게 막아주기위해 추가-->
							<input type="hidden" name="memberorder_check[<?=$product['basketidx']?>]" id="memberorder_check[<?=$product['basketidx']?>]" value="<?=$product["memberorder"]?>">
							<input class="checkbox-def" type="checkbox" name='checkBasket' id='' value='<?=$product['basketidx']?>' <?=$disabled?> data-productcode='<?=$product['productcode']?>' data-price='<?=$product_price?>' data-vender='<?=$vender?>' data-reserve='<?=$product['reserve']?>' style="display:none;" checked>
						</td>
						<td class="info">
							<a href="<?=$Dir.FrontDir?>productdetail.php?productcode=<?=$product['productcode']?>" <?if($product['chk']=='F'){?>class="no-sale"<?}?>>
								<!-- 상품 이미지 -->
						...
						<td class="ta-l">
							...
							<a href="<?=$Dir.FrontDir?>productdetail.php?productcode=<?=$product['productcode']?>">
								<?=$product['productname']?>
								<?if($product['chk']=='F'){?>
									<br><span style="color:red">구매불가상품입니다(<?=$Basket->return_code[$product['chk_msg']]?>)</span>
								<?}?>
							</a>
<?php
...
?>
							...
							<!--회원전용 상품을 구분해주기위해 추가-->
							<?//if($product['memberorder']=="M" && strlen($_ShopInfo->getMemid())==0){?>
						...
						<td class="cart-ea-choice">
							...
							<!-- <div style='margin-top: 3px;'>
								<a href="javascript:;" class="CLS_quantity_change"  data-productcode='<?=$product['productcode']?>'><img src="../static/img/btn/c_modify_btn.gif" type="image" ></a>
							</div> -->
						</td>
						
						<td name='NM_options' data-productcode='<?=$product['productcode']?>' style="display:none;">
						<!-- 옵션정보 시작 -->
<?php
...
?>
				<input type=hidden id="deli_<?=$vender?>" value='<?=$all_deli_price?>'>
				</tfoot>
			</table>
<?php
} // venderArr foreach
...
?>
			...
			<div class="total-price-sum">
				<ul>
					<li><span>총 주문금액</span><t id="view_sum1"><?//=number_format($bf_sumprice)?>0</t></li>
					<li class="minus"><span>총 할인금액</span><?=number_format(0)?></li>
					<li class="plus"><span>배송비</span><t id="view_sum2"><?//=number_format($all_deli_price)?>0</t></li>
					<li class="total"><span>총 결제금액</span><strong id="view_sum3"><?//=number_format($bf_sumprice+$all_deli_price)?>0</strong></li>
					<li><span>적립 마일리지</span><t id="view_sum4"><?//=number_format( $all_reserve )?>0</t>M</li>
				</ul>
			</div>
			...
			<div class="reply-list-wrap">
                    <div class="inner">
                            ...
                            <fieldset>
								...
								<button class="event_text">이벤트 참여 댓글</button>
								...
								<textarea cols="30" rows="10" id="up_comment"  style="float: right; width: 987px"></textarea>
                                <!-- <button class="btn-reply-reg" type="submit">댓글<br>입력</button> -->
                              
                            </fieldset>
                       
                        <table class="reply-list" style="display:none;">
                            ...
                            <colgroup><col style="width:185px"><col style="width:auto"><col style="width:90px"></colgroup>
								...
								<tr>
									<td>wonjja**</td>
									<td class="subject">  모바일에서도 댓글 써지나요??<button class="btn-delete" type="button" onclick="javascript:delete_comment('event', '112');"><img src="../static/img/btn/close.png" alt="삭제"></button></td>
									<td class="date">2017/05/23<span>16:56:57</span></td>
								</tr>
							   </tbody>
							   </table>
                    </div>
				...
				<a class="btn-dib-function" href="javascript:event_basket();" target="_self" style="height: 46px;width:300px;margin-top:20px;" id="event_basket_btn">
					<span style="height:auto;">장바구니 이벤트<br>참여 완료하기</span>
				</a>
				<!-- <a class=" btn-dib-function line" style="height:46px;" id="event_basket_btn"><span>장바구니 이벤트<br>참여 완료하기</span></a> -->
			</div>
