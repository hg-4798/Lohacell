...
class COUPON extends Common{
	...
	private $cog = array(
		'errormsg' => array(
			'E001'=>'해당하는 쿠폰이 없습니다',
			'E002'=>'쿠폰이 이미 발급되었습니다. MY PAGE>쿠폰에서 확인할 수 있습니다.',
			'E003'=>'쿠폰이 발급되지 않았습니다.\\n관리자에게 문의하세요.',
			'E004'=>'로그인 후 사용가능합니다.',
			'E005'=>'이미 중복 발급된 회원이 있습니다. 확인 후 재발급 해주세요'
		)
	);

	public function getPair() {
		$tbl = $this->tbls['point_config'];
		$sql = "SELECT point_id, point FROM {$tbl}";
 ...
 * @param  int  $limit 개수
 * @param  int  $offset
 * @return array	$list 	리스트 목록
 * @return array	$total
 */
	public function getCouponList($where, $limit, $offset) {
		$tbl = $this->tbls['coupon'];
		$cnt = $this->adodb->getOne("SELECT COUNT(*) FROM  {$tbl} {$where}");//전체개수

		$sql = "SELECT * FROM {$tbl} {$where} ORDER BY idx DESC LIMIT $limit OFFSET $offset";
		//echo $sql;
		$rs = $this->adodb->Execute($sql);
		//print_r($_ShopInfo);exit;
		if (!is_object($rs)) {
			return false;
		}
		$no=0;
		...
		while ($row = $rs->FetchRow()) {
			$list[$no]= $row;
			$list[$no]['part_cnt']= ($row['use_part']!='A' && $row['part_detail']!='ALL')? count( explode(',', $row['part_detail'])):'0';
			$list[$no]['url']="/front/couponissue.php?coupon=".encrypt_md5("COUPON|".$row['type_publish']."|".$row['idx']."|END","*ghkddnjsrl*");
			$no++;
		}
		$total = $cnt;
		//print_r($list);exit;
	 ...
	 * @return array	$total
	 */
	public function getDetailCoupon($where, $limit, $offset) {
		$cnt = $this->adodb->getOne("SELECT count(*) FROM tblcouponinfo info JOIN tblcouponissue issue ON issue.coupon_code::integer = info.idx {$where}");//전체개수

		$sql = "SELECT info.*,issue.* FROM tblcouponinfo info JOIN tblcouponissue issue ON issue.coupon_code::integer = info.idx {$where} LIMIT $limit offset $offset ";
		//echo $sql;
		$rs = $this->adodb->Execute($sql);
		//print_r($_ShopInfo);exit;
		if (!is_object($rs)) {
	...
	/**
	 * 쿠폰정보리턴 row
	 * @param int $coupon_idx  쿠폰번호
	 * @return void
	 */
	public function getCouponRow($coupon_idx) {
		$tbl = $this->tbls['coupon'];
		$row = $this->adodb->getRow("SELECT * FROM  {$tbl} WHERE idx = '{$coupon_idx}'");
		//print_r($row);exit;
		return $row;
	}
	/**
	 ...
	 * @return void
	 */
	public function getIssueCouponRow($ci_no) {
		$tbl = $this->tbls['coupon_issue'];
		$row = $this->adodb->getRow("SELECT * FROM  {$tbl} WHERE ci_no = '{$ci_no}'");
		//print_r($row);exit;
		return $row;
	}
	/**	
	 ...
	 * @return boolean
	 */
	public function issueDownload($coupon_idx,$id) {
		//로그인여부체크
		if(!MEMID){
			return $this->cog['errormsg']['E004'];
		}
		//쿠폰정보
		$coupon_info = $this->getCouponRow($coupon_idx);

		//print_r($coupon_info);exit;
		//다운로드 개수제한체크
		$cnt = $this->userCouponCnt(MEMID,$coupon_idx);
		if($coupon_info['limit_per'] <= $cnt){
			return $this->cog['errormsg']['E002'];
		}
		//@TODO 쿠폰설정에 따른 발급처리

		$rs = $this->_set($coupon_info,$id);
	 ...
	 * @return string msg
	 */
	public function issueInstant($coupon_info, $id) {


		//다운로드 개수제한체크
		//$cnt = $this->userCouponCnt($id,$coupon_info['idx']);
		//if($coupon_info['limit_per'] <= $cnt){
		//	return $this->cog['errormsg']['E005'];
		//}
		$rs = $this->_set($coupon_info,$id);
	 ...
	 * @return void
	 */
	private function _issue($record) {
		$tbl = $this->tbls['coupon_issue'];
		if($record['use_period_limit']){
			$record['date_start']= NOW;
			$record['date_end'] = date("Y-m-d H:i:s", strtotime($record['date']." +{$record['use_period_limit']} day"));
		}
		if($record['use_period_type']=='U'){
			$record['date_start']= NOW;
			$record['date_end'] = "2218-12-31 23:59:59";
		}
		$sql = sqlInsert($record,$tbl);
		//echo $sql;exit;
		$rs = $this->adodb->Execute($sql);
		if($rs){
			$this->adodb->Execute("UPDATE tblcouponinfo SET download_count =download_count+1 WHERE idx='{$record['coupon_code']}'");
			$issue = "success";

		}else{
			$issue = $this->cog['errormsg']['E003'];
		}
		return $issue;
	}

	private function _set($coupon_info,$id) {
		...
		foreach ($coupon_info AS $K=>$v){
			...
			switch ($K){
				...
				case 'use_device' :
					$record['use_device']=$v;
					break;
				case 'use_period_limit' :
					$record['use_period_limit']=$v;
				break;
				case 'part_detail' :
					$record['part_detail']=$v;
					break;
				case 'coupon_name' :
					$record['coupon_name']=$v;
					break;
				case 'sale_min_price' :
					$record['sale_min_price']=$v;
					break;
				case 'sale_price' :
					$record['sale_price']=$v;
					break;
				case 'sale_type' :
					...
					break;
				case 'sale_max_price' :
					$record['sale_max_price']=$v;
					break;
				case 'type_use' :
					$record['type_use']=$v;
				break;
				case 'use_period_type' :
					$record['use_period_type']=$v;
					break;
			}
		}

		$rs = $this->_issue($record);
	...
	private function _error($error_code) {
		return $this->cog['errormsg'][$error_code];
	}
	/**
	 * 사용자 쿠폰발급 개수
	 * @param string $id 사용자 아이디
	 * @return int $cnt
	 */
	public function userCouponCnt($id,$coupon_idx) {
		$tbl = $this->tbls['coupon_issue'];
		$cnt = $this->adodb->getOne("SELECT COUNT(*) FROM  {$tbl} WHERE id='{$id}' AND coupon_code='{$coupon_idx}'");//발급개수
 ...
 * @return array 쿠폰정보
 */
	public function getProductCoupon($productcode) {
		$tbl = $this->tbls['coupon'];

		$where = "use_part='P' AND issue_status='Y' AND part_detail !='ALL'";
		if(VIEWPORT == 'MOBILE') $where.=" AND use_device!='P'";
		else $where.=" AND use_device!='M'";
		...
		$no =0;
		while ($row = $rs->FetchRow()) {
			$productcode_arr = explode(',', $row['part_detail']);
	 ...
	 * @return array 쿠폰정보
	 */
	public function getUsableCoupon($productcode,$id) {
		$tbl = $this->tbls['coupon_issue'];
		$Product = new PRODUCT;
		$where = "id='{$id}' AND used='N' AND type_use='P' AND date_end >='".NOW."'";
		if(VIEWPORT == 'MOBILE') $where.=" AND use_device!='P'";
		else $where.=" AND use_device!='M'";
		$sql= "SELECT * from {$tbl} WHERE {$where}";
		//echo $sql;
		$rs = $this->adodb->Execute($sql);
		$tmp_product = $Product->getProductDetail($productcode);
		//pre($tmp_product);
		$list = Array();
		...
		while ($row = $rs->FetchRow()) {
			...
			//사용가능금액 필터
			if($tmp_product['endprice'] < $row['sale_min_price'] && $row['sale_min_price']!="") {
			  //상품금액과 비교
			  continue;
			}
			...
			//할인금액계산
			if($row['sale_type'] == 'R') {
				$row['discount'] = $tmp_product['endprice']*$row['sale_price']/100;
			}
			else {
				$row['discount'] = $row['sale_price'];
			}

			//사용기간 필터
			if($row['date_start'] >= NOW && $row['date_end'] <= NOW && $row['use_period_type']=="D"){
				continue;
			}else if($row['use_period_type']=="L"){
				if($row['date_end'] < NOW){
					continue;
				}
			}
			...
			switch($row['use_part']) {
				 case 'C': //특정카테고리
					 $part_detail = explode(',',$row['part_detail']);
					foreach ($part_detail AS $key=>$val){
						$tmp = rtrim($val,'0');
						$code_represent = substr($tmp_product['code_represent'],0, strlen($tmp));
						if($tmp==$code_represent){
							$list[] = $row; //$this->getCouponRow($row['coupon_code']);
						}
					}
					break;
				case 'P': //특정상품
					$part_detail = explode(',',$row['part_detail']);
					foreach ($part_detail AS $key=>$val){
						if($val==$tmp_product['productcode']){
							$list[] = $row;
						}
					}
					break;
	 ...
	 * @return array 쿠폰정보
	 */
	public function getUsableBasketCoupon($id,$price) {
		$tbl = $this->tbls['coupon_issue'];
		$where = "id='{$id}' AND used='N' AND type_use='B'";
		if(VIEWPORT == 'MOBILE') $where.=" AND use_device!='P'";
		else $where.=" AND use_device!='M'";
		$sql= "SELECT * from {$tbl} WHERE {$where}";
		//echo $sql;exit;
		...
		while ($row = $rs->FetchRow()) {
			//사용가능금액 필터
			if($price < $row['sale_min_price'] && $row['sale_min_price']!="") {
				//상품금액과 비교
				continue;
			}
			//할인금액계산
			if($row['sale_type'] == 'R') {
				$row['discount'] = $price*$row['sale_price']/100;
				if($row['discount'] > $row['sale_max_price'] && $row['sale_max_price'] !="") $row['discount'] =  $row['sale_max_price'];
			}
			else {
				$row['discount'] = $row['sale_price'];
				if($row['discount'] > $row['sale_max_price'] && $row['sale_max_price'] !="") $row['discount'] =  $row['sale_max_price'];
	 ...
	 * @return array 쿠폰정보
	 */
	public function getUsableDeliveryCoupon($id,$price) {
		$tbl = $this->tbls['coupon_issue'];
		$where = "id='{$id}' AND used='N' AND type_use='D'";
		if(VIEWPORT == 'MOBILE') $where.=" AND use_device!='P'";
		else $where.=" AND use_device!='M'";
		$sql= "SELECT * from {$tbl} WHERE {$where}";
		//echo $sql;exit;
	 ...
	 * 쿠폰 사용여부 체크
	 * @param string $id 사용자 아이디
	 * @param int $ci_no 쿠폰발급번호
	 * @return boolean
	 */
	public function getValid($id,$ci_no) {
		$tbl = $this->tbls['coupon_issue'];
		$where = "id='{$id}' AND ci_no='{$ci_no}'";

		$rs = $this->adodb->getOne("SELECT used FROM  {$tbl} WHERE {$where}");
		if($rs=="N"){
			return true;
		}else{
			return false;
	 ...
	 * @return boolean
	 */
	public function useProcessing($ci_no, $order_num, $order_product_no='') {
		$tbl = $this->tbls['coupon_issue'];
		$issue = $this->getIssueCouponRow($ci_no);

		$etc_record=array(
			'order_num'=>$order_num,
			'etc_type'=>'coupon',
			'etc_type_detail'=>$issue['type_use'],
			'etc_idx'=>$ci_no,
			'etc_limit'=>$issue['date_end'],
			'date_insert'=>NOW
		);
		if($issue['type_use']=='P'){ //상품쿠폰
			$etc_record['order_product_no'] = $order_product_no;
			$etc_price = $this->adodb->getOne("SELECT coupon_discount FROM ".$this->tbls['order_product']." WHERE order_num='{$order_num}' AND idx='{$order_product_no}' AND coupon_issue_no='{$ci_no}'");
		}
		else if($issue['type_use']=='B') { //장바구니쿠폰
			$etc_price = $this->adodb->getOne("SELECT coupon_basket_discount FROM ".$this->tbls['order']." WHERE order_num='{$order_num}'");
		}
		else { //무료배송쿠폰
			$etc_price = $this->adodb->getOne("SELECT coupon_delivery_discount FROM ".$this->tbls['order']." WHERE order_num='{$order_num}'");
		}

		$etc_record['etc_price'] = $etc_price; // 할인금액 by 2018-12-08
		$etc_record['etc_price_origin'] = $etc_price; // 최초할인금액 by 2018-12-08

		//pre($etc_record);
		$sql = sqlInsert($etc_record, 'tblorder_payment_etc');
		$rs = $this->adodb->Execute($sql);
		if(!$rs) return false; //사용이력정상처리체크

		$record=array('used'=>'Y','date_update'=>NOW);
		$where = array(
			'ci_no'=>$ci_no
		);
	...
	/**
		* 쿠폰 복원처리
		* @param string $ci_no 쿠폰 idx
		* @return boolean
	*/
	function restore($ci_no) {
		$tbl = $this->tbls['coupon_issue'];
		$cnt = $this->adodb->getOne("SELECT count(*) FROM {$tbl} WHERE ci_no ='{$ci_no}' AND date_end < '".NOW."'");
		
		if($cnt > 0 ) return false; // 쿠폰 사용날짜가 만료되는 복원 불가능
		...
		$sql = sqlUpdate($etc_record, 'tblorder_payment_etc', $where_etc);
		$rs = $this->adodb->Execute($sql);
		if(!$rs) return false;

		$record=array('used'=>'N','date_update'=>NOW);
		$where = array(
			'ci_no'=>$ci_no
		);
		$sql = sqlUpdate($record, $tbl, $where);
	...
	public function log($coupon_idx,$mode,$type,$member_data) {
		global $_ShopInfo;
		$tbl = $this->tbls['coupon_log'];
		$msg = array(
			'Instant'=>'수동발급'
		);
		$record = array(
			'coupon_code'=>$coupon_idx,
			'id'=>$_ShopInfo->getId(),
			'log_msg'=>$msg[$mode],
			'issue_type'=>$type,
			'date_insert'=>NOW,
			'ip'=>$_SERVER['REMOTE_ADDR'],
			'use_id'=>pg_escape_string($member_data)
	...
	public function useCoupon($id) {
		$tbl = $this->tbls['coupon_issue'];
		$cnt = $this->adodb->getOne("SELECT COUNT(*) FROM  {$tbl} WHERE id='{$id}' AND used='N' AND date_end > '".NOW."' ");//발급개수
		$sql= "SELECT * from {$tbl} WHERE id='{$id}' AND used='N' AND date_end > '".NOW."' ORDER BY date_end ASC";
		$rs = $this->adodb->Execute($sql);
		$list = Array();
		$no=0;
		while ($row = $rs->FetchRow()) {
			if($row['use_period_type']!='U'){
				$list[$no]['date_cnt'] = ceil((strtotime($row['date_end']) - time())/86400);
			}
			$list[$no]['issue'] = $row;
			$list[$no]['info'] = $this->getCouponRow($row['coupon_code']);
		   $no++;
		}
		$total = $cnt;
		//pre($list);
	...
	public function usedCoupon($id) {
		$Order = New ORDER();
		$tbl = $this->tbls['coupon_issue'];
		$cnt = $this->adodb->getOne("SELECT COUNT(*) FROM  {$tbl} WHERE id='{$id}' AND used='Y'");//발급개수
		$sql= "SELECT * from {$tbl} WHERE id='{$id}' AND used='Y' ORDER BY date_update ASC";
		$rs = $this->adodb->Execute($sql);
		$list = Array();
		$no=0;
		while ($row = $rs->FetchRow()) {
			...
			$list[$no]['issue'] = $row;
			$list[$no]['order'] = $Order->getPaymentEtc('coupon', $row['ci_no']);
			$no++;
		}
		$total = $cnt;
	 ...
	 * @return boolean
	*/
	public function birthCoupon() {
		$Sms = new SMS();
		$tbl_member = $this->tbls['member'];
		$tbl_issue = $this->tbls['coupon_issue'];
		$tbl = $this->tbls['coupon'];

		$member_list = $this->adodb->getArray("select id,group_code FROM {$tbl_member} where substr(birth, 5, 2) = to_char(now(),'MM')");

		foreach ($member_list AS $key =>$val){
			$coupon_info = $this->adodb->getRow("select * FROM {$tbl} where type_publish='A' AND group_code='".$val['group_code']."' ");
			$cnt = $this->adodb->getOne("select count(*) FROM {$tbl_issue} where used='N' AND coupon_code='".$coupon_info['idx']."' AND id='".$member_list[$key]['id']."' ");
			//pre("select count(*) FROM {$tbl_issue} where used='N' AND coupon_code='".$coupon_info['idx']."' AND id='".$member_list[$key]['id']."' ");
			if($cnt < 1){
				$rs = $this->_set($coupon_info,$member_list[$key]['id']);
			}
		}
		$Sms->send_sms('COUPON_002', 'birthday');
		return $rs;
	}
	 ...
	 * @return boolean
	 */
	public function expiresBatch() {
		$tbl = $this->tbls['coupon_issue'];
		$record=array('used'=>'Y','date_update'=>NOW);
		$where = "date_end < '".NOW."' AND used !='Y' ";
		$sql = sqlUpdate($record, $tbl, $where);
		//echo $sql;exit;
		$rs = $this->adodb->Execute($sql);
		return $rs;
	 ...
	 * @return boolean
	 */
	public function couponStatusBatch() {
		$tbl = $this->tbls['coupon'];
		$sql= "UPDATE {$tbl} SET issue_status='N' WHERE publish_date_type ='D' AND publish_date_end < '".NOW."' ";
	...
	public function search_member_coupon( $id, $type = 0, $is_mobile = 0 )
	{
		...
		$sql = "
			SELECT 
				...
				info.coupon_name, info.sale_type, info.mini_price, info.sale_money, info.amount_floor, 
