...
$arrSearchDate = array('1MONTH'=>'1개월', '3MONTH'=>'3개월', '6MONTH'=>'6개월');
...
$arrDeliveryType = array("0" => "택배발송", "1" => "매장픽업", "2" => "당일수령");
...
### 매장검색
$store_area = array(1=>"서울","경기","인천","강원","충남","대전","충북","부산","울산","대구","경북","경남","전남","광주","전북","제주");
//$store_category = array(1=>"백화점", "대리점","상설점");
$store_category = array(1=>"가맹점", "직영점"); // 매장구분은 이렇게 2개로 함.
...
$delivery_fee_type	= array(
		...
		2	=> "선불+2500원 동봉",
		4  => "핫티부담",
		3	=> "계좌이체 (5천원) [국민은행 : 978201-01-031084 (주)슈마커]"
);
...
$arr_admin_member_excel_info = array(
    ...
    array("휴대폰번호", "mobile"),
    array("활동 포인트", "to_char(act_point, '999,999,999')"),
...
function pmysql_query( $query, $connect=NULL ) {
	...
	if(($error = pg_last_error()) && strpos($error,'duplicate key value violates')===FALSE) {
		$bt = debug_backtrace();
		...
		if( $key !== null ) {
            ...
            */
			...
			$product_arr[$key] = array(
				'[CODE]'=>$row->productcode,                                                //상품코드
				'[NAME]'=>strip_tags($row->productname),                                    //상품명
...
function insert_order_point( $ordercode, $mem_id, $point, $body='', $rel_flag='', $rel_mem_id='', $rel_job='', $expire=0, $idx='' ) {
    ...
    } else if( $row->staff_order == 'N' ) {
        // 일반 포인트 처리
		$mem_auth_type	= getAuthType($mem_id);
		if ($mem_auth_type != 'sns') { // 정회원일 경우에만 지급
			$return_data = insert_point( $mem_id, $point, $body, $rel_flag, $rel_mem_id, $rel_job, $expire );
		}
    } else {
        $return_data = -2;
    }
...
function getAllBrandList() {
	...
	// ===============================================================================
	$sql  = "SELECT tblResult.bridx, tblResult.brandname, tblResult.brandname2, tblResult.vender, tblResult.logo_img, (case when tblResult.brandname = 'NIKE' then '1' else '2' end) as rno ";
	$sql .= "FROM ( ";
	$sql .= "   SELECT bridx, brandname, brandname2, vender,logo_img";
	$sql .= "   FROM tblproductbrand ";
	$sql .= "   WHERE display_yn = 1 ";
	$sql .= ") as tblResult ";
...
function Category_list($code_a, $code_b="000", $code_c="000", $code_d="000") {
	...
	$sql = "SELECT * FROM tblproductcode where ".$cate_array." and code_a not in ('".implode("', '",$not_cate)."')";
	$sql.= " ORDER BY cate_sort ASC";
