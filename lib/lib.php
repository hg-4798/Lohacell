...
$arrLimit = array(20,40,60,80,100,200,300,400,500);
$arrMobile = array('010','011','016','017','018','019');
$arrTel = array('02','031','032','033','041','042','043','044','051','052','053','054','055','061','062','063','064','070','0505');
...
$arrMemberOutReason = array('1'=>'ID 변경을 위한 재가입', '2'=>'개인정보 유출 우려', '3'=>'개인정보 관련 기타', '4'=>'광고성 메일, SMS', '5'=>'주문/취소/반품/환불 불만'
, '6'=>'사이트 속도 및 안정성 불만', '7'=>'이용 빈도 낮음', '8'=>'개인정보 관련 기타', '9'=>'상품 품질/가격 불만', '10'=>'배송지연', '11'=>'잦은 상품 품절'
, '12'=>'상품 관련 기타', '13'=>'고객센터 응대 불친절', '14'=>'회원 혜택 부족', '15'=>'서비스 관련 기타');

$arrMemberOutReason_m_cate = array('1'=>'개인정보 관련','2'=>'사이트 이용 관련','3'=>'상품관련','4'=>'서비스 관련');
$arrMemberOutReason_m = array('1'=>array('1'=>'ID 변경을 위한 재가입', '2'=>'개인정보 유출 우려', '3'=>'개인정보 관련 기타'),'2'=>array('4'=>'광고성 메일, SMS', '5'=>'주문/취소/반품/환불 불만'
, '6'=>'사이트 속도 및 안정성 불만', '7'=>'이용 빈도 낮음', '8'=>'사이트 이용 관련 기타'),'3'=>array('9'=>'상품 품질/가격 불만', '10'=>'배송지연', '11'=>'잦은 상품 품절'
, '12'=>'상품 관련 기타'),'4'=>array('13'=>'고객센터 응대 불친절', '14'=>'회원 혜택 부족', '15'=>'서비스 관련 기타'));
...
// 이메일 도메인 정보
$email_domain_arr	= array("naver.com","gmail.com","daum.net","hanmail.net","nate.com");
...
function adodb_connect() {
	...
	$f = file(DOC_ROOT."/conf/config.db.php") or error_msg("DB 설정 파일이 없습니다.<br>DB설정을 먼저 하십시요",$Dir."install.php");

	for($i=1;$i<=4;$i++) $f[$i]=trim($f[$i]);
	include(DOC_ROOT."/lib/adodb5/adodb.inc.php");
	$adodb = NewADOConnection('postgres9');
	$adodb->setFetchMode(ADODB_FETCH_ASSOC);
//	$adodb->Connect("host=".$f[1]." user=".$f[2]." password=".$f[3]);
	$adodb->Connect($f[1],$f[2],$f[3],$f[4]);
	if(!defined(DbName)) { define('DbName',$f[4]); } // AutoSql을 위해 상수 설정함
	return $adodb;
}
...
function pmysql_connect($hostname, $user_id, $password ) {
	...
	$dbconn->connect = @pg_connect($dbconn->con_str." dbname=".$f[4]);
	return $dbconn;
}
...
class UserSession {
	...
	function __construct($id, $authkey) {
		...
		if ($row) {
			...
			echo "	if (opener) {\n";
			echo "		opener.parent.location.href=\"/admin/logout.php\";\n";
			echo "		window.close();\n";
			echo "	} else {\n";
			echo "		parent.location.href=\"/admin/logout.php\";\n";
			echo "	}\n";
			echo "</script>\n";
			exit;
		}
...
class ShopInfo {
	...
	// SNS 로그인 관련 추가 - 2017.10.16 bshan
	var $checksnsdata	 = "";	 # sns종류||아이디
	// SNS 로그인 관련 추가 - 2017.10.16 bshan 끝
	...
	function __construct($_sinfo) {
		...
		if ($_sinfo) {
			...
			// SNS 로그인 관련 추가 - 2017.10.16 bshan
			$this->checksnsdata		= $savedata["checksnsdata"];
		}
	}
	...
	function Save() {
		...
		$savedata["gender"]		= $this->getGender();
		$savedata["birth"]		= $this->getBirth();
		...
		$savedata["checksnsdata"]	= $this->getCheckSnsData();
	...
	function SetMemNULL() {
		...
		// SNS 로그인 관련 함수 추가 - 2017.10.16 bshan
		$this->setCheckSnsData("");
	}
	...
	function setBirth($birth)			{$this->birth = $birth;}
	...
	function setCheckSnsData($checksnsdata)		{$this->checksnsdata = $checksnsdata;}
	...
	// SNS 로그인 관련 함수 추가 - 2017.10.16 bshan
	function getCheckSnsData()		{return $this->checksnsdata;}
...
*/
	...
	function adminLogin() {
		global $shopurl,$mem_id,$mem_pw,$ssllogintype,$sessid,$history,$shop_info;
		...
		if (ord($mem_id) && (ord($mem_pw) || ($ssllogintype=="ssl"))) {
			...
			if ($row = pmysql_fetch_object($result)) {
				...
				if ($flag) {
					...
					$authkey = md5(uniqid(""));
					$this->setShopurl($shop_info['shopurl']);
					$this->setId($id);
					$this->setName($name);
					$this->setEmail($email);
...
$_CFG = get_object_vars($_ShopInfo);
...
// AP포인트 부여 - deprecated
function insert_point_act($mem_id, $point, $body='', $rel_flag='', $rel_mem_id='', $rel_job='', $expire=0){return false;}

// 사용 AP포인트 입력 - deprecated
function insert_use_point_act($mem_id, $point, $pid=0){return false;}

// AP포인트 내역 합계 - deprecated
...
function getProductImage( $img_path, $img_name, $mobile_flag = null ) {
	...
	if(strpos($img_name, "https://") === false) {
...
*/
			...
			if ( $isMobile || ( $mobile_flag != null && $mobile_flag ) ) {
			   $img_name = "/admin/images/product/noimg.jpg";
			} else {
			   $img_name = "/admin/images/product/noimg.jpg";
			}
		}
...
function productlist_print( $sql, $type = 'W_001', $arrProdOrder = array(), $listnum = null, $icon_info = null, $cate_code = null) {
	...
	$prodcode_model = array("SUU29090","SUB24040","SUB24160","BUJ21030","BUB64580", "BUB24030","SUP22130","BUP22050");
	...
	while( $row = pmysql_fetch_object( $result ) ){
		// 모델컷 우선 나오게 하기
		$model_in_check=in_array($row->prodcode,$prodcode_model);
		if($model_in_check){
			$idlen = strlen($row->tinyimage);
			$model_Simg = substr($row->tinyimage,0,$idlen-4);
			$model_Limg = substr($row->tinyimage,-4);
			$model_Zimg = $model_Simg."Z".$model_Limg;
			if(remoteFileExist($model_Zimg) == 1){
				$row->minimage = $model_Zimg;
			}
		}
			...
			*/
			...
			if(strpos($row->over_minimage, "http://") === false) {
				if( strlen($row->over_minimage) == 0 || !file_exists( $img_path.$row->over_minimage ) ){
					$image_tags = '<img src="'.getProductImage( $img_path, $row->minimage ).'" alt="상품 썸네일11" >';
				}
			}
			//soldout icon 표시
	...
	for( $i=0; $i < $tag_cnt ; $i++ ){ //테그별
		...
		for( $j=0; $j < $content_length; $j++ ){ //컨텐츠별
			...
			if( ( $i == $tag_cnt -1 ) && ( $arr_lastcnt <= $j && $arr_lastcnt != 0 ) ){
				...
				#모델 컷이면 thumb-img-model 클래스로(2018.03.09)
				$tmpType = $type;
				if($type == 'S_001'|| $type == 'M_001') {
					$imgNm = array_pop(explode('/',$arr[$use_cnt]["[MINIMAGE]"]));
					$imgType = substr($imgNm, 0, strrpos($imgNm, "."));
					$imgType = array_pop(explode('_',$imgType));
					if(strtoupper($imgType) =="ZZ" || strtoupper($imgType) =="XX"){
						$tmpType = $type."_M";
					}
				}

				#----------------------------------------
				$temp_content = str_replace( $list_key, $arr[$array_key[$use_cnt]], $list_types[$tmpType]['content'] );	// 컨텐츠를 변환
...
/**
 * class libraries autoload
 * @author hjlee
 */
function lib_autoload($classname) {
	$classname = strtolower($classname); //소문자
	if($classname == 'categorylist') { //클래스명과 클래스파일명이 다름  TODO 통일
		$classname = 'category';
	}

    $doc_root = DOC_ROOT;
	$classpath = $doc_root."/lib/".$classname.".class.php";
...
function _render($path, $assign=array(), $dir='template', $fetch=false) {
	include_once(DOC_ROOT."/lib/Template_/Template_.class.php");
	$tpl = new Template_($dir);
	$tpl->define('CONT', $path);
	//$tpl->setScope('CONT');
	$tpl->assign($assign);
	//$tpl->print_('CONT');
	//echo $fetch;
	if($fetch) return $tpl->fetch('CONT');
	else $tpl->print_('CONT');
}


function _fetch($path, $assign=array(), $dir='template') {
	return _render($path, $assign, $dir, true);
}

function _layout($path, $assign, $layout='default', $dir='template') {
	$tpl = new Template_($dir);
	$tpl->define(
		array(
			'CONT'=>$path,
			'TOP'=>'/',
			'GNB'=>'/front/include/gnb.php',
			'BOTTOM'=>'/front/include/footer.php'
		)
	);
	$tpl->assign($assign);
	$tpl->print_('CONT');
...
function sqlInsert($record, $tbl) {
	
	$column = array_keys($record);
	$value = array_values($record);
	$sql = "INSERT INTO {$tbl} (".implode(',',$column).") VALUES('".implode("','",$value)."')";
	return $sql;
	/**/
}
 ...
 * @param array/string $where  [description]
 * @return [type]		 [description]
 */
function sqlUpdate($record, $tbl, $where) {
	$set = array();
	...
	if(is_array($where)) {
		...
		$where_query = implode(' AND ',$where_arr);
	}
	else $where_query = $where;

	$sql = "UPDATE {$tbl} SET {$set_query} WHERE {$where_query}";
...
//회원아이디
define('ADMINID', $_ShopInfo->getid());
define('MEMID', $_ShopInfo->getMemid());
define('GENDER', $_ShopInfo->getGender());
define('BIRTH', $_ShopInfo->getBirth());
define('MEMNAME', $_ShopInfo->getMemname());
define('STAFF_YN', $_ShopInfo->getStaffYn()); //임직원인경우:Y, 임직원이아닌경우:N

//쉘 실행일경우 (batch)
$current_shop_url = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'];
if(php_sapi_name() == 'cli'){
    //쉘 실행이며 개발서버일 경우
    if($_SERVER['USER']=='jayjun'){
        $current_shop_url = "https://www.iknowione.co.kr";
    }else {
        $current_shop_url = "https://dev-jayjun.ajashop.co.kr";
    }
}
define('SHOPURL', $current_shop_url);
?>
