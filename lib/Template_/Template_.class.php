...
/**/
	function __construct($template_dir='template') {
		if ( ! class_exists('Template_') ){
			//require_once($_SERVER['DOCUMENT_ROOT'].'/lib/Template_/Template_.class.php');
		}
		
		$this->template_dir=DOC_ROOT.'/'.$template_dir;

		if($template_dir == 'template') { //PC버전 컴파일경로
			$this->compile_dir=DOC_ROOT.'/lib/Template_/_compile';
			$this->cache_dir=DOC_ROOT.'/lib/Template_/_cache';
		}
		else { //MOBILE버전 컴파일경로
			$this->compile_dir=DOC_ROOT.'/lib/Template_/m/_compile';
			$this->cache_dir=DOC_ROOT.'/lib/Template_/m/_cache';
