...
class upload {
	...
	function process($server_path = null) {
		...
		/*
		if ($this->processed) {
			include $_SERVER['DOCUMENT_ROOT'].'/lib/file.class.php';
			$ftp_path = str_replace($_SERVER['DOCUMENT_ROOT'].'/','',$server_path);
			$imgfile = new FILE($Dir.$ftp_path);
			$imgfile->setUploadToFtp($this->file_dst_name);
		}
		*/
