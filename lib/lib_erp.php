...
function getErpProdStock($prodcd, $colorcd, $type='delivery') {
	...
	if($season_year){
		...
		$check_brand = substr($prodcd,0,1); //이자벨 
		$size_stock = array();
...
/*			if($check_season == 'R' && $season_year == '2017' && $list->SUMQTY > 1){
			...
			}else if($check_brand == 'V'){
				if($list->SUMQTY > 6){
					$res_size_stock[sumqty][] = trim($list->SUMQTY - 5);
				}else{
					$res_size_stock[sumqty][] = 0;
				}
			}else{*/
			if($prodcd == 'FKF45380' || $prodcd == 'FKF45390'){
//				if($prodcd == 'FKF45390' && $list->SIZECD == '74' && $colorcd == 'NA'){
//						$res_size_stock[sumqty][] = 1;
//				}else{
					if($list->SUMQTY > 71){
						$res_size_stock[sumqty][] = trim($list->SUMQTY - 70);
					}else{
						$res_size_stock[sumqty][] = 0;
					}
//				}
//추가 20180618 권정수 대리 요청
			}else if($prodcd == 'VQV27830' && $list->SIZECD == '77' && $colorcd == 'BK'){
					$res_size_stock[sumqty][] = 2;
////////////////////////////////////
			}else{
				if($check_brand == 'V'){
					if($list->SUMQTY > 1){
						$res_size_stock[sumqty][] = trim($list->SUMQTY);
					}else{
						$res_size_stock[sumqty][] = 0;
					}
				}else if($list->SUMQTY > 11){
...
// 상품 해당 사이즈의 매장별  재고 구하기(실시간)
function getErpProdO2oStock_Part($prodcd, $colorcd, $sizecd, $shopCd) {
	...
	if ($season_year) {
		...
		foreach($xml->NewDataSet->STOCK as $list) 
		{
			if($list->PART_CODE == $shopCd){
				$maket_stock[sumqty]	=	(($prodcd=='BMC21890' && $sizecd=='77') || ($list->STOCK_QTY < 0))?0:trim($list->STOCK_QTY);
			}
		}
	}
...
/*			if($check_season == 'R' && $season_year == '2017'){
				...
				if($list->SUMQTY > 6){
					$data['SUMQTY']	= $list->SUMQTY - 5;
				}else{
					$data['SUMQTY']	= 0;
				}
			}else{*/
			if($prodcd == 'FKF45380' || $prodcd == 'FKF45390'){
//				if($prodcd == 'FKF45390' && $sizecd == '74' && $colorcd == 'NA'){
//						$res_size_stock[sumqty][] = 0;
//				}else{
					if($list->SUMQTY > 71){
						$res_size_stock[sumqty][] = trim($list->SUMQTY - 70);
...
//				}
//추가 20180618 권정수 대리 요청
			}else if($prodcd == 'VQV27830' && $sizecd== '77' && $colorcd == 'BK'){
					$res_size_stock[sumqty][] = 2;
////////////////////////////////////
//			}else if($prodcd == 'FME41520' && $sizecd == '100'){
//					$res_size_stock[sumqty][] = 1;
			}else{
				if($check_brand == 'V'){
					if($list->SUMQTY > 1){
...
function sendErpOrderInfo($ordercode, $conn) {
    ...
    while($data = pmysql_fetch_array($result)){
		...
		if(substr(trim($ordercode), -1) == "X") {
            ...
            list($erp_shopmem_id, $erp_emp_id, $cooper_nm) = pmysql_fetch("Select erp_shopmem_id, erp_emp_id, office_name From tblmember Where id = '".$data[id]."'");
