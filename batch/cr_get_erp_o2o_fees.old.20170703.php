...
//$up_sql = "UPDATE tblstore SET pickup_yn='N', delivery_yn='N', day_delivery_yn='N'";
...
while($data = oci_fetch_array($smt, OCI_BOTH+OCI_RETURN_NULLS+OCI_RETURN_LOBS)) {
	...
	$sql = "
			WITH upsert as (
				...
				where store_code      = '".$store_code."' AND part_div = '".$data[PART_DIV]."' AND part_no = '".$data[PART_NO]."' AND brandcd = '".$data[BRANDCD]."' AND apply_fdate = '".$data[APPLY_FDATE]."' AND apply_tdate = '".$data[APPLY_TDATE]."' AND o2o_gb = '".$data[O2O_GB]."' 
				RETURNING * 
			)
	...
	if($o2o_gb=='3') $up_sql1 .= "day_delivery_yn='Y' ";
	...
	// 실서버에는 주석풀자..
	$sql = "update TA_OM004 set RCV_DATE=SYSDATE where RCV_DATE is NULL and PART_DIV = '".$data[PART_DIV]."' AND PART_NO = '".$data[PART_NO]."' AND BRAND = '".$data[BRANDCD]."' AND APPLY_FDATE = '".$data[APPLY_FDATE]."' AND APPLY_TDATE = '".$data[APPLY_TDATE]."' AND O2O_GB = '".$data[O2O_GB]."'  ";
