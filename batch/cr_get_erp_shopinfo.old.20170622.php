...
while($data = oci_fetch_array($smt, OCI_BOTH+OCI_RETURN_NULLS+OCI_RETURN_LOBS)) {
	...
	if ($vender != '') {
		...
		list($coordinate) = pmysql_fetch("Select coordinate From tblstore where part_div = '".$data[PART_DIV]."' AND part_no = '".$data[PART_NO]."' AND brandcd = '".$data[BRANDCD]."' ");
		...
		$sql = "
				WITH upsert as (
					update  tblstore 
					set 	name      = '".trim($data[PART_NAME])."', 
							address      = '".$address."', 
							phone      = '".$data[TEL_NO]."', 
							view      = '".$view."', 
							area_code      = '".$area_code."', 
							category = '".$erp_category_arr[$data[PART_DIV]]."', 
							vendor      = '".$vender."', 
							stime       = '".$stime."', 
							etime       = '".$etime."', 
