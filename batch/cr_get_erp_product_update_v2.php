...
//			   AND RCV_DATE IS NULL
    ...
    while($data = oci_fetch_array($smt_mr, OCI_BOTH+OCI_RETURN_NULLS+OCI_RETURN_LOBS)) {
		...
		# 중복제거 추가
		if($data[MIXRATE]){
			$mix_rate_arr[trim($data[COLORATION_GB])][]=trim($data[MATR_NAME])." ".$data[MIXRATE]."%";
		} else {
			$mix_rate_arr[trim($data[COLORATION_GB])][]=trim($data[MATR_NAME]);
		}
    }
    ...
    # 구분 중복제거
    $str = '';
    foreach($mix_rate_arr as $key => $val_arr) {
    	foreach($val_arr as $k => $value) {
    		//echo $key1." : ".$value1."<br>";
    		$str .= $value.' ,';
    	}
    	$str = substr($str, 0, -1);
