...
//		$(this).parents('tbody').find('tr:odd').addClass('hide');
	...
	jQuery(function($){
		...
		var select_a = $('div.select_custom>ul>li>a');
		var select_input = $('div.select_custom>ul>li>input[type=radio]');
		var select_label = $('div.select_custom>ul>li>label');
		...
		select_input.bind('focusin',function(){$(this).parents('div.select_custom').children('div.my_value').addClass('outLine')});
		select_input.bind('focusout',function(){$(this).parents('div.select_custom').children('div.my_value').removeClass('outLine')});
		
		// Show
		function show_option(){
			$(this).parents('div.select_custom:first').toggleClass('open');
		}
		...
		function hide_option(){
			var t = $(this);
			setTimeout(function(){
				t.parents('div.select_custom:first').removeClass('open');
			}, 1);
		}
		...
		function set_anchor(){
			var v = $(this).html();
			$(this).parents('ul:first').prev('.my_value').html('').append(v);
			$(this).parents('ul:first').prev('.my_value').addClass('selected');
		}

		// Anchor Focus Out
		$('*:not("div.select_custom a, div.select_custom")').focus(function(){//IE에서 스크롤바 클릭시 focus out되는 문제로 셀렉터 추가(2018-04-30)
			$('.a_list').parent('.select_custom').removeClass('open');
		});
		/*$('*:not("div.select_custom a")').focus(function(){
			$('.a_list').parent('.select_custom').removeClass('open');
		});*/
