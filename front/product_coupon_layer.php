...
/*				if( $prcouponVal->mini_quantity == 0 || ( $prcouponVal->mini_type == 'P' && $prcouponVal->mini_quantity <= $total_row->total_price_sum ) 
				...
				if( ( $prcouponVal->mini_type == 'P' && $prcouponVal->mini_price <= $total_row->total_price_sum )  || ( $prcouponVal->mini_type == 'Q' && $prcouponVal->mini_quantity <= $total_row->total_qty ) ){
...
?>
					<li>
						<div class="radio ml-20">
							<input type="radio" name="product_coupon" value="<?=$prcouponVal->ci_no?>"
								...
								<?=$dis_check?>
								>
							<label for="no_<?=$couponIndex?>"></label>
						</div>
						<div class="content w350 ">
							<p class="txt-toneB"><?=$prcouponVal->coupon_name?></p>
...
<div class="btnPlace mt-10">
	<button class="btn-line  h-large" onclick="javascript:prd_coupon_cancel(<?=$basketidxs?>);"type="button"><span>취소</span></button>
	<button class="btn-point h-large"  onclick="javascript:set_product_coupon();" type="button"><span>적용</span></button>
</div>