...
#---------------------------------------------------------------
	...
	if(strlen(MEMID)==0) {
        ...
        if(DIR_VIEW == '/m'){
            Header("Location:".DIR_M."/login.php?chUrl=".getUrl()."&ret_url=".$ret_url);
        }else{
	...
	$couponissue = $Coupon->issueDownload($exp_coupon[2],MEMID);


if($couponissue=="success"){
	    alert_go('쿠폰이 발급되었습니다',$ret_url);
    }else{
        alert_go($couponissue,$ret_url);
    }
