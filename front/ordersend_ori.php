...
while($vgrp=pmysql_fetch_object($res)) {
	...
	$sql = "SELECT a.opt1_idx,a.opt2_idx,a.optidxs,a.quantity,SUBSTR(a.date,1,8) date,b.productcode,b.productname,b.sellprice, ";
...
if(ord($_ShopInfo->getMemid())==0) {
	...
	$sql.= "receiver_addr, order_msg, ip, del_gbn, partner_id, loc, bank_sender) VALUES (";
	...
	$sql.= "'{$receiver_addr}', '{$order_msg}', '{$ip}', 'N', '".$_ShopInfo->getRefurl()."', '{$loc}', '{$bank_sender}')";
	pmysql_query($sql,get_db_conn());
	backup_save_sql($sql);
	if (pmysql_errno()) {
		sendmail(AdminMail,"[긴급!] INSERT ERROR",$_SERVER['HTTP_HOST']."<br>$sql - ".pmysql_error(),"Content-Type: text/plain\r\n");
