...
$sp_gubun = trim($_REQUEST[sp_gubun]);
...
$nods_str = file_get_contents("../js/json_adapter/json_adapter_query.arr");
if(empty($nods_str)) {
	$nods = array();
	...
	foreach($xml->children() as $node) {
	    ...
	    $arr = $node->attributes();
	    $node_id = $arr["id"];
	    $nods[(string)$node_id] = (string)$node;
	    if($node_id==$sp_name){
		$multi_query = (string)$node;
	    }
	}
	$nods_str = serialize($nods);
	file_put_contents("../js/json_adapter/json_adapter_query.arr",$nods_str);
} else {
  $nods = unserialize($nods_str);
  $multi_query = $nods[$sp_name];
}
...
list($code, $error_msg, $json_arr_sub, $total_count) = $db->$sp_type($sp_name, $sp_param, $multi_query, $sp_decode, $sp_gubun);
...
class Database {
	...
	public function getDBFunc($sp_name, $sp_param, $multi_query, $sp_decode, $sp_gubun){
		$code = true;
		...
		if($result){
			while ($row = pmysql_fetch_object($result)) {
				...
				foreach ($row as $key => $value) {
					//$keylower = strtoupper($key);
					$keylower = ($key);
					if($sp_gubun=='M' && strpos($value,'img.shinwonmall.com')>0) {
						$value = preg_replace('/\/(\w+\.\w+)$/','/M_\1',$value);
					}
					$values = $value;
