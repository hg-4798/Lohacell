...
while($vgrp=pmysql_fetch_object($res)) {
	...
	while($row = pmysql_fetch_object($result)) {
		...
		if($price>0){
			if(ord($group_type)>0 && $group_type!=NULL && ($group_payment=="N" || ($group_payment=="B" && strstr("BVOQ",$paymethod)) || ($group_payment=="C" && strstr("CP", $paymethod)))) {
			//if(strlen($group_type)>0 && $group_type!=NULL ) {
				$salemoney=0;
				$salereserve=0;
				if($dc_per>0){
					$salemoney=round($price*$dc_per/100,-1,PHP_ROUND_HALF_DOWN);
				}else{
					if($group_type=="SW" || $group_type=="SP") {
						if($group_type=="SW") {
							$salemoney=$group_addmoney;
						} else if($group_type=="SP") {
							$salemoney=round($price*$group_addmoney/100,-1,PHP_ROUND_HALF_DOWN);
				...
				if($group_type=="RW" || $group_type=="RP" || $group_type=="RQ") {
					if($group_type=="RW") {
						...
						$salereserve=round($price*$group_addmoney/100,-1,PHP_ROUND_HALF_DOWN);
...
for($orderi=0;$orderi<$count;$orderi++) {
	...
	if (pmysql_errno()) {
		...
		$tempoptcnt="";
		if(ord($option_quantity[$productcode[$orderi]])>0) {
		...
		$stock_quantity = $quantity[$orderi];

		#### 구매수량에 따른 재고차감 ####
		if($option_ea[$productcode[$orderi]] && !$option_quantity[$productcode[$orderi]]){
//			echo $option1num[$orderi]-1;
//			echo $option_ea[$productcode[$orderi]];
			$tmp_option_ea = explode(",",$option_ea[$productcode[$orderi]]);
			$stock_quantity  = $tmp_option_ea[($option1num[$orderi]-1)]*$quantity[$orderi];
//			echo "ddd";
		}

		if(ord($assemble_productcode[$orderi])>0) {
			...
			for($k=0; $k<count($assemble_productcode_exp); $k++) {
				...
				$sql.= "selldate		= now(), ";
				$sql.= "quantity		= quantity-{$stock_quantity} ";
		...
		if(ord($package_productcode[$orderi])>0) {
...
if(ord($_ShopInfo->getMemid())==0) {
	...
	$sql.= "receiver_addr, order_msg, ip, del_gbn, partner_id, loc, bank_sender, receipt_yn) VALUES (";
	...
	$sql.= "'{$receiver_addr}', '{$order_msg}', '{$ip}', 'N', '".$_ShopInfo->getRefurl()."', '{$loc}', '{$bank_sender}', '{$receipt_yn}')";
	pmysql_query($sql,get_db_conn());
	backup_save_sql($sql);
	if (pmysql_errno()) {
		sendmail(AdminMail,"[긴급!] INSERT ERROR",$_SERVER['HTTP_HOST']."<br>$sql - ".pmysql_error(),"Content-Type: text/plain\r\n");
