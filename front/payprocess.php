	...
	*/
	...
	if( strstr("VCPMY", $paymethod) ) {
			...
			//if ($staff_order!= "Y") {
				...
				while($pr_data=pmysql_fetch_object($pr_result)){
					...
					if($srtn != 'fail') {
						...
						$in_deli_sql = "UPDATE tblorderproduct SET  store_code='A1801B',delivery_type='0' WHERE ordercode='{$ordercode}' and idx={$pr_data->idx} ";
						pmysql_query($in_deli_sql, get_db_conn());
		...
		if(substr($_SERVER['REMOTE_ADDR'],0,10) == '218.234.32'){
			# erp 전송 20180302 - dereklee | CP[신용카드],M[휴대폰],Y[페이코]
			if( strstr("CPMY", $paymethod) ){
				sendErporder($ordercode);
				$f = fopen("../data/backup/send_erp_order_".date("Ymd").".txt","a+");
				fwrite($f,"[orderprocess] paymethod = ".$paymethod." \n");
				fwrite($f,"[orderprocess] sendErporder = ".$ordercode." \n");
				fclose($f);
