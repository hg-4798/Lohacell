...
$opt1_idx_ea = $_POST["opt1_idx_ea"];
$opt2_idx_ea = $_POST["opt2_idx_ea"];
		...
		if (strlen($productcode)==18) {
			...
			if($row->cnt>=200) {
				...
				if ($optionArr != "" && $priceArr != "" && $quantityArr != "" ) {
					$optionArr = explode("||",trim($optionArr));
					$priceArr = explode("||",trim($priceArr));
					$quantityArr = explode("||",trim($quantityArr));
					for ($i = 0; $i < sizeof($optionArr); $i++) {
						$multiOrderCount++;
						$ex_option = explode("_",$optionArr[$i]);
						$option1 = $ex_option[0];
						$option2 = $ex_option[1];
...
<!-- AceCounter eCommerce (Cart_Inout) v6.4 Start -->
<!-- Function and Variables Definition Block Start -->
<script language='javascript'>
var bizAllClearArray = new Array();
var i=0;
...
function AEC_B_A(){var i=0,_AEC_str='',_A_cart='';var ind = 0;_A_cart = HL_CRL+'?cuid='+HL_GCD+'&md=b';for( i = 0 ; i < _A_pl.length ; i ++ ){_AEC_str += ACE_REPL(_A_ct[i])+'@'+ACE_REPL(_A_pn[i])+'@'+ACE_REPL(_A_amt[i],1)+'@'+ACE_REPL(_A_nl[i],1)+'^';if(escape(_AEC_str).length > 800 ){if(ind > 4) ind = 0;_AEC_str = _RPS(escape(_AEC_str),'+','%2B')+'&cmd=on';AEC_S_F(_AEC_str,'b',ind); _AEC_str = '' ;ind++;};}; if( _AEC_str.length > 0 ){if(ind+1 > 4) ind = 0; AEC_S_F(_RPS(escape(_AEC_str),'+','%2B'),'b',ind+1);};};
...
function AEC_S_F(str,md,idx){ var i = 0,_A_cart = ''; var k = eval('_A_i'+idx); if(md == 'I' ) md = 'i' ; if(md == 'O' ) md = 'o' ; if(md == 'B' ) md = 'b' ; if( md == 'b' || md == 'i' || md == 'o'){ _A_cart = HL_CRL+'?cuid='+HL_GCD ; _A_cart += '&md='+md+'&ll='+(str)+'&'; k.src = _A_cart;window.setTimeout('',2000);};};
...
if($ordertype=="ordernow") {  //바로구매
	if($sumprice>=$_data->bank_miniprice) {
		...
		} else {			// if조건이 이상해서.   update 조건이 먹히지 않음   // optionarr quantity arr 값을 가져와서 update를 해야하나 안
...
$(function(){	
	...
	$('li div.goods_img').click(function(){			
		...
		$('div.popup_preview_warp').css({'top':$(window).scrollTop()+100,'z-index':'210'});	// 현재위치
		
	});
**/
/**
	$('div.layer_goods_info a.busket').click(function(){					
		var prcode = $(this).attr("alt");
		$.post("../front/ajax_preview_for_list.php",{productcode:prcode},function(data){
			if(data){
				$('div.popup_preview_warp').html(data);
			}
		});
