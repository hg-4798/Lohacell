...
function CheckForm(gbn,temp2) {
	...
	if(gbn!="wishlist") {
		...
		if (gbn != "ordernow"){
			...
			window.open("about:blank","confirmbasketlist","width=500,height=250,scrollbars=no,resizable=no, status=no,");
		...
		window.open("about:blank","confirmwishlist","width=500,height=250,resizable=no, status=no,scrollbars=no");
...
*/	
	if (strlen($_cdata->detail_type)==6 && $_cdata->detail_type[5]=="U") {
		...
		//include($Dir.TempletDir."product/detail_{$_cdata->detail_type}.php");
		include("../templet/product/detail_TEM001_test.php");
	}
?>
	</td>
</tr>
