	...
	if(($cert_type != 'email' && $_SESSION[ipin][dupinfo]) || ($cert_type == 'email')){
		...
		if($mode == "findid"){
			if ($qry_where) {
				...
				if(!$mem_id){
					list($mem_id, $mem_name, $mem_mail, $mem_date)=pmysql_fetch("select id, name, email, date from tblmember_sleep where 1=1 {$qry_where}");
				}
				if(!$mem_id){
					if ($cert_type == "mobile") {
					...
					if ($access_type == 'mobile') {
						...
						if ($access_type == 'pc_store') {
							$message	= "가입하신 아이디는 <span class='find_pw_memid'>".format($mem_id,'id')."</span>";
...
// 							$message	= "가입하신 아이디는<br> <span class='point-color'>".$mem_id."</span> 입니다.";
					...
					if ($cert_type == "mobile" || $cert_type == "ipin") {
						if($dupinfo_yn=='Y') {
							if ($mode == "findpw") {
								...
								$sql = "UPDATE tblmember SET passwd='".$shadata."', passwd_fail_count = 0 ";
								$sql.= "WHERE id='{$mem_id}' ";
						...
						$sql = "UPDATE tblmember SET passwd='".$shadata."', passwd_fail_count = 0, member_out = 'N' ";
						$sql.= "WHERE id='{$mem_id}' ";
			...
			if ($qry_where) {
				list($mem_id, $mem_name, $mem_mail, $mem_date, $dupinfo)=pmysql_fetch("select id, name, email, date, dupinfo from tblmember_sleep where 1=1 {$qry_where}");
				...
				if(!$mem_id){
					if ($cert_type == "mobile") {
						...
						$Member = new MEMBER();
						$rs = $Member->MemberWakeUp($dupinfo, 'N');
						if($rs) {
							$code = "1";
						}else{
							$message = "휴면해제실패";
