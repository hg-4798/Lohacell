...
$expire_mileage = $Order->adodb->getOne("SELECT COALESCE(SUM(etc_price),0) FROM tblorder_payment_etc WHERE order_num = '{$order_num}' AND etc_type='mileage' AND etc_limit < NOW() AND restore_yn='N'"); //소멸마일리지
...
$expire_point = $Order->adodb->getOne("SELECT COALESCE(SUM(etc_price),0) FROM tblorder_payment_etc WHERE order_num = '{$order_num}' AND etc_type='point' AND etc_limit < NOW() AND restore_yn='N'"); //소멸마일리지
...
$assign = array(
	...
	'expire'=>array(
		'point'=>$expire_point,
		'mileage'=>$expire_mileage
	),
	'bank'=>$bank
);

//pre($oder_giftlist);
_render('mypage/cancel_list.html', $assign);
