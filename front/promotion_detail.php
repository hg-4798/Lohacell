...
<script type="text/javascript">
    ...
    $(document).ready( function() {
        ...
        var display_seq = req.display_seq;
    ...
    function EventView(){
        ...
        this.getEventView = function (idx){
            ...
            var param = [event_type, req.display_seq, compare, req.idx, event_type, req.display_seq];
            var data = db.getDBFunc({sp_name: 'event_detail_before', sp_param : param});
            if(data.data){
                data = data.data[0];
                $('#prev').html('<span class="mr-20">PREV</span><a href="?idx='+data.idx+'&event_type='+data.event_type+'&display_seq='+data.display_seq+'&chkPage='+req.chkPage+'">'+data.title+'</a>');
            }
            ...
            if(data.data){
                data = data.data[0];
                $('#next').html('<span class="ml-20">NEXT</span><a href="?idx='+data.idx+'&event_type='+data.event_type+'&display_seq='+data.display_seq+'&chkPage='+req.chkPage+'">'+data.title+'</a>');
            }
    ...
    function setPhoto(responseText, statusText, xhr) {
        ...
        if(imgs){
			...
			if($('#photo_content').val() == ''){
				alert('내용을 넣어 주세요');
				return false;
			}

			if(user_img1 == '-'  && user_img2 == '-' && user_img3 == '-' && user_img4 == '-'){
				alert('포토를 하나 이상 등록 해야 합니다.');
				return false;
			}
                        ...
                        <div class="box">
                            <form>
                                <fieldset>
                                    ...
                                    <textarea placeholder="<?=$msg?>" id="comment_textarea" onkeydown="lengchk(this,'textarea_length');" onkeyup="lengchk(this,'textarea_length');"></textarea>
