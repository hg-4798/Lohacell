		...
		if(pmysql_query($sql,get_db_conn())) {
			...
			if( count( $spl_option_code ) > 0 ){
				foreach( $spl_option_code as $splKey=>$splVal ) {
					...
					$spl_optInsertSql.= "VALUES ( '".$splVal."', '".$prcode."', '".$spl_option_price[$splKey]."', '".$spl_option_quantity[$splKey]."', '".$spl_option_use[$splKey]."', '1' ) ";
...
?>
					...
					<tr>
						...
						<td style="padding:7px 7px"><input name=buyprice value="<?=$_data->buyprice?>" size=18 maxlength=10></td>
					</tr>
					<tr><td height=1 colspan=4 bgcolor=E7E7E7></td></tr>
					<tr>
...
<?php
						if (strlen($_data->maximage)>0 && file_exists($imagepath.$_data->maximage)) {
							echo "<br><img src='".$imagepath.$_data->maximage."?v".date("His")."' height=100 width=200 border=1 alt='URL : /".RootPath.DataDir."shopimages/product/".$_data->maximage."'>";
...
<?php
						if (strlen($_data->minimage)>0 && file_exists($imagepath.$_data->minimage)){
							echo "<br><img src='".$imagepath.$_data->minimage."?v".date("His")."' height=80 width=150 border=1 alt='URL : /".RootPath.DataDir."shopimages/product/".$row->minimage."'>";
...
<?php
						if (strlen($_data->tinyimage)>0 && file_exists($imagepath.$_data->tinyimage)){
							echo "<br><img src='".$imagepath.$_data->tinyimage."?v".date("His")."' height=70 width=120 border=1 alt='URL : /".RootPath.DataDir."shopimages/product/".$_data->tinyimage."'>";
							...
							<tr>
								<td class=lineleft style="PADDING-RIGHT: 5px; PADDING-LEFT: 5px; PADDING-BOTTOM: 5px; LINE-HEIGHT: 125%; PADDING-TOP: 5px" align=middle width="50%" bgcolor="#F9F9F9">
									<?php if(ord($mulimg_name[$gbn1])){?>
									 <img src="<?=$urlpath."s".$mulimg_name[$gbn1]."?v".date('His')?>" width="100" height="100" border="0"><A HREF="javascript:mulimgdel('<?=$gbn1?>');"><img src="images/icon_del1.gif" border="0"></a>
									<?php }else{echo"&nbsp;";}?>
								</td>
								<td class=line style="PADDING-RIGHT: 5px; PADDING-LEFT: 5px; PADDING-BOTTOM: 5px; LINE-HEIGHT: 125%; PADDING-TOP: 5px" align=middle width="50%" bgcolor="#F9F9F9">
									...
									<img src="<?=$urlpath."s".$mulimg_name[$gbn2]."?v".date('His')?>" width="100" height="100" border="0"><A HREF="javascript:mulimgdel('<?=$gbn2?>');"><img src="images/icon_del1.gif" border="0"></a>
									<?php }else{echo"&nbsp;";}?>
								</td>
							</tr>
		<?php }?>
...
<?php
	# 옵션이 존재할 경우
	$optionSql = "SELECT option_num, option_code, option_price, option_quantity, option_use FROM tblproduct_option WHERE productcode='".$prcode."' AND option_type = 0 ";
...
?>
					...
					<?}?>
<!-- 정보고시 등록/수정 안보이게 막음 2015 10 27 유동혁-->
					<tr style='display:none;'>
