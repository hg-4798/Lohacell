			...
			<tr>
				...
				<tr>
					...
					<tr>
						...
						<tr>
							<td>
							&nbsp;<U>기간선택</U>&nbsp; <input type=text name=search_start value="<?=$search_start?>" size=13  onfocus=this.blur(); onclick=Calendar(event) style="text-align:center;font-size:8pt"> ~ <input type=text name=search_end value="<?=$search_end?>" size=13  onfocus=this.blur(); onclick=Calendar(event) style="text-align:center;font-size:8pt">
							&nbsp;
...
?>
						...
						<tr>
							<td>
							&nbsp;<U>처리단계</U>&nbsp;
							<select name="deli_gbn"  style="font-size:8pt">
<?
...
<?
				$colspan=8;
				$sql = "SELECT a.ordercode,MAX(a.id) as id, MAX(a.paymethod) as paymethod, MAX(a.pay_data) as pay_data, MAX(a.bank_date) as bank_date, MAX(a.pay_flag) as pay_flag, MAX(a.pay_auth_no) as pay_auth_no, MAX(a.pay_admin_proc) as pay_admin_proc, MAX(a.escrow_result) as escrow_result, MAX(a.sender_name) as sender_name, MAX(a.del_gbn) as del_gbn ";
