...
#---------------------------------------------------------------
	$sql = "SELECT ";
	//오늘 주문건수 및 주문금액
	$sql.= "COUNT(DISTINCT(CASE WHEN a.ordercode LIKE '".$curdate."%' THEN a.ordercode ELSE NULL END)) as totordcnt, ";
	$sql.= "SUM(CASE WHEN a.ordercode LIKE '".$curdate."%' THEN b.price*b.quantity ELSE 0 END) as totordprice, ";
	//오늘 미배송 건수 및 미배송건 금액
	...
	$sql.= "SUM(CASE WHEN (a.ordercode LIKE '".$curdate."%') AND (b.deli_gbn IN('N','X','S')) THEN b.price*b.quantity ELSE 0 END) as totdelayprice, ";

	//1일전 주문건수 및 주문금액
	$sql.= "COUNT(DISTINCT(CASE WHEN a.ordercode LIKE '".$curdate_1."%' THEN a.ordercode ELSE NULL END)) as totordcnt1, ";
	$sql.= "SUM(CASE WHEN a.ordercode LIKE '".$curdate_1."%' THEN b.price*b.quantity ELSE NULL END) as totordprice1, ";
	...
	$sql.= "SUM(CASE WHEN (a.ordercode LIKE '".$curdate_1."%') AND (b.deli_gbn IN('N','X','S')) THEN b.price*b.quantity ELSE 0 END) as totdelayprice1, ";

	//이달 주문건수 및 매출
	$sql.= "COUNT(DISTINCT(CASE WHEN a.ordercode LIKE '".substr($curdate,0,6)."%' THEN a.ordercode ELSE NULL END)) as totmonordcnt, ";
	$sql.= "SUM(CASE WHEN (a.ordercode LIKE '".substr($curdate,0,6)."%' AND a.deli_gbn='Y') THEN b.price*b.quantity ELSE 0 END) as totmonordprice ";
	...
	<tr>
		...
		<tr>
			...
			<tr>
				...
				<tr>
					<td height=120>
					<table border=0 cellpadding=0 cellspacing=0 width=100% height=120 background="images/bg_center.gif" style="table-layout:fixed">
					<tr>
						...
						<tr>
							...
							<tr>
								...
								<tr>
									<td style="padding-left:5">방문수</td>
...
<?
					...
					while($row=pmysql_fetch_object($result)) {
						...
						echo "<tr height=20 bgcolor=#FFFFFF onmouseover=\"this.style.background='#FEFBD1'\" onmouseout=\"this.style.background='#FFFFFF'\">\n";
...
?>
					...
					<tr>
						...
						<tr>
							...
							<tr>
								...
								<A HREF="minishop_design.php">디자인관리</A><img src=images/main_center_quick_sel.gif>
								<br>
								<A HREF="main_design.php">메인화면관리</A><img src=images/main_center_quick_sel.gif>
								<A HREF="minishop_notice.php">미니샵공지사항</A><img src=images/main_center_quick_sel.gif>
								<br>
								<A HREF="product_register.php">상품등록</A><img src=images/main_center_quick_sel.gif>
								<A HREF="product_myprd.php">상품관리</A><img src=images/main_center_quick_sel.gif>
								<A HREF="order_list.php">주문조회</A><img src=images/main_center_quick_sel.gif>
								<A HREF="sellstat_list.php">정산조회</A><img src=images/main_center_quick_sel.gif>
								<A HREF="sellstat_sale.php">매출분석</A><img src=images/main_center_quick_sel.gif>
								<A HREF="coupon_list.php">쿠폰관리</A><img src=images/main_center_quick_sel.gif>
								</td>
							</tr>
							</table>
							</td>
					...
					<tr><td height=5></td></tr>
					<tr>
						<td bgcolor=#FCF7FD style="padding:7">
						<table border=0 cellpadding=0 cellspacing=0 width=100% style="table-layout:fixed">
						<tr><td height=7></td></tr>
						<tr>
							<td><img src=images/icon_dot07.gif border=0 width=5 height=13 align=absmiddle> 내 판매상품 현황</td>
						</tr>
						<tr><td height=5></td></tr>
						<tr>
						...
						<tr><td height=20></td></tr>
						<tr>
							<td><img src=images/icon_dot07.gif border=0 width=5 height=13 align=absmiddle> 내 미니샵 방문현황</td>
						</tr>
						<tr><td height=5></td></tr>
						<tr>
							<td bgcolor=#FFFFFF style="padding:10,15;border:1px solid #E7E7E7">
