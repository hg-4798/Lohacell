	...
	*/
	...
	if($file_size < $maxfilesize) {
		...
		*/
		...
		$sql.= "regdate			= now(), ";
		$sql.= "modifydate		= now(), ";
		$sql.= "content			= '".pmysql_escape_string($content)."' WHERE productcode='".$code.$productcode."' ";
...
*/
			...
			if( count( $opt_id ) > 0 && $opt_select == '1' && $opt_type == '0' ){
				foreach( $opt_id as $optKey=>$optVal ){
					...
					$optInsertSql.= "( option_code, productcode, option_price, option_quantity, option_quantity_noti, option_use, option_tf ) ";
					$optInsertSql.= "VALUES ( '".$optVal."', '".$code.$productcode."', '".$opt_price[$optKey]."', '".$opt_stock_qty[$optKey]."', '".$opt_noti_qty[$optKey]."', '".$opt_use[$optKey]."', 'T' ) ";
			...
			if( $opt_select == '1' && $opt_type == '1' && count( $spl_option_code ) > 0 ){
				foreach( $spl_option_code as $splKey=>$splVal ) {
					...
					$spl_optInsertSql.= "( option_code, productcode, option_price, option_quantity, option_quantity_noti, option_use, option_tf, option_type ) ";
					$spl_optInsertSql.= "VALUES ( '".$splVal."', '".$code.$productcode."', '".$spl_option_price[$splKey]."', '".$spl_option_quantity[$splKey]."', '".$spl_option_noti_qty[$splKey]."', '".$spl_option_use[$splKey]."', '".$spl_tf[$splKey]."', '1' ) ";
...
<table border=0 cellpadding=0 cellspacing=0 width=1400 style="table-layout:fixed">
<col width=175></col>
<col width=5></col>
<!-- <col width=740></col> -->
<col width=1140></col>
...
<!-- 카테고리 링크 선택 추가 2015 10 26 유동혁 -->
				...
				<tr>
                    ...
                    <tr style='display:none;' >
						...
						<td style="padding:7px 7px"><input name=reserve value="1" size=18 maxlength=6 colspan="2" onKeyUP="chkFieldMaxLenFunc(this.form,this.form.reservetype.value);"><select name="reservetype" style="width:77;font-size:8pt;margin-left:1px;" onchange="chkFieldMaxLenFunc(this.form,this.value);"><option value="N" >적립금(￦)</option><option value="Y" selected>적립률(%)</option></select><br><font style="color:#2A97A7;font-size:8pt;letter-spacing:-0.5pt;">* 적립률은 소수점 둘째자리까지 입력 가능합니다.<br>* 적립률에 대한 적립 금액 소수점 자리는 반올림.</span></td>
					</tr>
                    <tr><td height=1 colspan=4 bgcolor=E7E7E7></td></tr>
					<tr>
