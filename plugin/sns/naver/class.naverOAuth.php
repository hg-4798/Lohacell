...
class OAuthRequest{
    ...
    /*20171030 연동 삭제 start*/

    public function request_del(){
        header('Location: '. $this -> get_request_del_url() );
    }

    public function get_request_del_url(){

        return $this -> accesstoken_url . '?grant_type=delete&client_id=' . $this -> client_id . '&client_secret=' . $this -> client_secret . '&access_token=' . urlencode($_SESSION['access_token']) . '&service_provider=NAVER';
    ...
    public function call_accesstoken(){
        ...
        $_SESSION['access_token'] =  $data['access_token']; // 20171030 연동 삭제
    }
    public function get_user_profile(){
        ...
        $arrEmailExplode = explode("@", (string)$xml -> response -> email );
        $this -> userInfo = array(
            'userID' => $arrEmailExplode[0],
