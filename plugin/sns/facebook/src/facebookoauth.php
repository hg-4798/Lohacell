...
class FacebookOAuth {
  ...
  function getAccessToken($oauth_verifier = FALSE, $redirect_uri= FALSE) {
    ...
    else
    {
        // ssl 추가 2016-12-08 유동혁
        if( $_SERVER['HTTPS'] == "on" ) {
            $parameters['redirect_uri'] = 'https://' . $_SERVER['HTTP_HOST'] . $this->remove_query($_SERVER['REQUEST_URI']);
        } else {
    ...
    $token2['oauth_token'] = $token->access_token;
	$_SESSION['access_token'] =$token2['oauth_token'];
    return $token2;
  }
