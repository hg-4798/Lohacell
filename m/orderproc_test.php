<?
	...
	if (ord($paymethod)==0) {
		echo "<html></head><body onload=\"alert('결제방법이 선택되지 않았습니다.');location.replace('order.php');\"></body></html>";
		exit;
	}

	if (ord($usereserve)>0 && !IsNumeric($usereserve)) {
		echo "<html></head><body onload=\"alert('적립금은 숫자만 입력하시기 바랍니다.');location.replace('order.php');\"></body></html>";
		exit;
	}

	if(ord($_data->escrow_id)==0 && $paymethod=="Q") {
		echo "<html></head><body onload=\"alert('에스크로 정보가 존재하지 않습니다.');location.replace('order.php');\"></body></html>";
		exit;
	}
	...
	if ($paymethod!="B" && ord($pg_type)==0) {
		echo "<html></head><body onload=\"alert('선택하신 결제방법은 이용하실 수 없습니다.');location.replace('order.php');\"></body></html>";
		exit;
	}
	...
	if(ord($errmsg)>0) {
		echo "<html></head><body onload=\"alert('{$errmsg}');location.replace('".$Dir.FrontDir."basket.php');\"></body></html>";
		exit;
	}
	...
	while($vgrp=pmysql_fetch_object($res)) {
		...
		while($row = pmysql_fetch_object($result)) {
			...
			//######### 옵션에 따른 가격 변동 체크 끝 ############
			$bf_sumprice += $bf_price*$row->quantity;  //할인전 총 금액(총구매금액대별 할인률을 구하기 위해)
	...
	if($sumprice<$_data->bank_miniprice) {
		echo "<html></head><body onload=\"alert('주문 가능한 최소 금액은 ".number_format($_data->bank_miniprice)."원 입니다.');location.href='".$Dir.FrontDir."basket.php'\"></body></html>";
		exit;
	} else if($sumprice<=0) {
		echo "<html></head><body onload=\"alert('상품 총 가격이 0원일 경우 상품 주문이 되지 않습니다.');location.href='basket.php'\"></body></html>";
		exit;
	}

	if(strstr("CP", $paymethod)) {
		if($_data->card_miniprice>$sumprice) {
			echo "<html></head><body onload=\"alert('카드결제 최소 주문금액보다 결제금액이 작습니다.');location.href='".$Dir.FrontDir."basket.php'\"></body></html>";
			exit;
		}
	} else if(strstr("BVOQ",$paymethod) && $sumprice<$_data->bank_miniprice) {
		echo "<html></head><body onload=\"alert('최소 주문금액보다 결제금액이 작습니다.');location.href='".$Dir.FrontDir."basket.php'\"></body></html>";
		exit;
	}
	...
	for($orderi=0;$orderi<$count;$orderi++) {
		...
		if($beforehand_reserve == 'Y'){//즉시적립사용
	...
	if(count($coupon_code_goods) > 0 && $sumprice > 0){
		...
		foreach($coupon_code_goods as $couponGoods){
			...
			if($rowGoodsCou=pmysql_fetch_object($resultGoodsCou)) {
				...
				# 같은 쿠폰을 다른 상품에서 여러번 사용했을시 해당 쿠폰의 할인율/적립율을 더해줌
