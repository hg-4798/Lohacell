...
if(!$rs['list']) {
	alert_go('유효하지 않은 주문서입니다.','/m/basket.php');
}

//주문서유효시간체크(분)
if(!$Order->validOrder()) {
	alert_go('주문서 유효기간이 만료되었습니다.','/m/basket.php');
}
...
$privacy_buy = $Product->getConfig('privacy_buy');
...
$assign = array(
	...
	'bank_code' => $kcp_bank_code,
	'privacy_buy' => $privacy_buy
);

_render('order/order.html', $assign, DIR_M.'/template');
