...
class _MShopInfo {
	...
	function Save() {
        ...
        $savedata["mobile"]		= $this->getMobile();
...
function loginprocess($id='', $passwd='', $push_token ='', $push_os ='', $sns_id ='', $sns_type ='', $sns_login ='', $sns_login_id ='') {
	...
	if($row=pmysql_fetch_object($result)) {
        ...
        if($row->member_out == 'S') { // 휴면회원 이라면 실명인증 페이지로 이동
            return "SLEEP";
        }
    ...
    $_MShopInfo->setGender($gender); //회원 성별
    $_MShopInfo->setBirth($birth); //회원 생일
...
function response($status, $msg=NULL, $data=NULL) {
	if (strlen($msg)>0) {
		//$msg=rawurlencode(ajax_convert($msg)); //모바일 로그인페이지 한글깨짐으로 encoding 변경
