...
post_to_url(
	'<?=$Dir.MDir?>mypage_usermodify.php?my_passwd_check=<?=encrypt_md5($mud_arr[6] . '|Y')?>',
	{
		'type':'usermodify',
		'name':'<?=$mud_arr[1]?>',
		'birth1':'<?=$mud_arr[2]?>',
        ...
        'email2':'<?=$mud_arr[12]?>',
        'skintype':'<?=$mud_arr[13]?>',
        'home_tel1':'<?=$mud_arr[14]?>',
        'home_tel2':'<?=$mud_arr[15]?>',
        'home_tel3':'<?=$mud_arr[16]?>',
		'home_zonecode':'<?=$mud_arr[17]?>',
		'home_addr1':'<?=$mud_arr[18]?>',
		'home_addr2':'<?=$mud_arr[19]?>',
		'news_mail_yn':'<?=$mud_arr[20]?>',
		'news_sms_yn':'<?=$mud_arr[21]?>'
	}
);
...
<?} else {?>
    <?if(!$sns_type){?>
        <script>
        alert('<?=$returnMsg?>');
        document.location.href = "<?=$Dir.MDir?>member_chkid.php?auth_type=<?=$auth_type?>&mem_type=<?=$mem_type?>&find_type=<?=$find_type?>&cert_type=<?=$cert_type?>&join_type=<?=$join_type?>&birthdate=<?=$birthdate?>";
        </script>
    <?}else{?>
        <script>
            alert('<?=$returnMsg?>');
            document.location.href = "<?=$Dir.MDir?>member_sns_end.php?auth_type=<?=$auth_type?>&mem_type=<?=$mem_type?>&find_type=<?=$find_type?>&cert_type=<?=$cert_type?>&join_type=<?=$join_type?>";
        </script>
    <?}?>
<?}?>
<!--html>
