...
switch($action_mode) {
	case "LOGIN":
		...
		if($resdata=="NO") {
		...
		} else if($resdata=="SLEEP") {
			response(false, "회원님의 계정은 휴면상태입니다. 휴면 해제해 주세요.",'GOWAKEUP');
		}else if($resdata=="passwd_fail") {
			...
			if($fail_count >= 10){
				...
				$failMsg = "비밀번호가 ".number_format($fail_count)."회 틀렸습니다.";

			}
			response(false, $failMsg);
	...
	case "SNS_LOGIN":
		...
		$sns_email=$_REQUEST["sns_email"];	// S
