...
list($uri,$querystring) = explode('?',$chUrl);

if($querystring){
	$dirLocation = "order.php?".$querystring;
}else{
...
<main id="page" class="subpage with_bg">
	...
	<section class="loginpage sub_bdtop">
		...
		<div class="login_area">
			<input type="text" class="w100-per" title="아이디 입력자리" placeholder="아이디 입력" id="m_id" name="id"  maxlength="100" value="<?=($_COOKIE["save_id"]=="Y"?$_COOKIE['smart_id']:"")?>">
			...
			<?if(basename($uri)=="order.php" || basename($uri)=="basket.php") {?><a href="<?=$dirLocation?>" class="btn-line w100-per h-input mt-10">비회원구매</a><?}?>
		</div><!-- //.login_area -->
		</form>

		<!-- <div class="join_yet mt-20 pt-10">
			<p class="ment">- 로그인시 <?=$_data->shopname?> 기존회원의 비밀번호는 휴대폰번호(-제외)입니다.</p>
		</div> --><!-- //.join_yet -->
		...
		<div class="login_simple">
			...
			<div class="btn_area">
				...
				<?if($snsNvConfig["use"] == "nv"){?>
					<li><a href="/plugin/sns/sns_access.php?sns=<?=$snsNvConfig["use"]?>&sns_login=1&ac=m&churl=<?=urlencode($chUrl)?>" class="btn-line"><img src="/jayjun/m/static/img/icon/icon_naver.png" alt="네이버"><span class="naver">네이버</span></a></li>
				<?}?>
				<?if($snsKtConfig["use"] == "kt"){?>
					<li><a href="/plugin/sns/sns_access.php?sns=<?=$snsKtConfig["use"]?>&sns_login=1&ac=m&churl=<?=urlencode($chUrl)?>" class="btn-line"><img src="/jayjun/m/static/img/icon/icon_kakao.png" alt="카카오톡"><span class="kakao">카카오톡</span></a></li>
				<?}?>
				<?if($snsFbConfig["use"] == "fb"){?>
					 <li><a href="/plugin/sns/sns_access.php?sns=<?=$snsFbConfig["use"]?>&sns_login=1&ac=m&churl=<?=urlencode($chUrl)?>" class="btn-line"><img src="/jayjun/m/static/img/icon/icon_facebook.png" alt="페이스북"><span class="facebook">페이스북</span></a></li>
				<?}?>
				</ul>
			</div>
		</div><!-- //.login_simple -->
		...
		$(this).ajaxSubmit({
			...
			success: function(response) {
				if (response.success) {
					...
					if(data=="GOWAKEUP") {
						alert(msg);
						location.href = '/m/member_sleep.php';
					}else {
						alert(msg);
					}
				}
