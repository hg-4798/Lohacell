...
function JsonAdapter(){
    ...
    this.getDBFunc = function(sp){
        ...
        var getData = "sp_type="+sp_type+"&sp_name="+sp_name+"&sp_param="+sp_param+param_tail+"&sp_decode="+sp_decode+"&sp_gubun="+sp_gubun+"&nd="+new Date().getTime();
        //viewLog('JSONADAPTER:http://test-aja.ajashop.co.kr'+geturl+'?'+getData);
        $.ajax({
            url: geturl,
            type:'post',
            data:getData,
    ...
    this.setDBFunc = function(sp){
        ...
        //viewLog('JSONADAPTER:http://test-aja.ajashop.co.kr'+geturl+'?'+temp);
...
//				max_roundgrp = (total_cnt % (Number(roundpage)) +1);
                ...
                // 페이징 수정스크립트
                max_roundgrp = (total_cnt / Number(roundpage));
                max_roundgrp = Math.ceil(max_roundgrp);

                max_roundgrp = (max_roundgrp) +'';
                max_roundgrp = Number(max_roundgrp.substr(max_roundgrp.length -1 , 1));
        ...
        var before_currpage = Number(currpage)-1;
        var beforeG_currpage = 1;
        if(currgrp==1){
            if(currpage==1){
                before_currpage=0;
            }else {
                before_currpage = currpage - 1;
            }
        }

        //다음그룹페이지번호
        var after_currpage = Number(currpage) + 1;
        if(after_currpage > last_currnum)	after_currpage =0;

        var afterG_currpage = last_currnum;
        if(currpage==last_currnum)	afterG_currpage =0;
