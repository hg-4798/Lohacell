		...
		if(cmtArr){
			...
			for(var i = 0 ; i < cmtArr.length ; i++){
				...
				if(gubun=='M'){
					...
					rows += '		<button type="button" class="btn_like on" id="like_'+cmtArr[i].hno+'" title="선택됨">';
					rows += '			<span class="icon" onclick="like.clickLikeM(\''+cmtArr[i].section+'\','+cmtArr[i].hno+', \''+cmtArr[i].hott_code+'\');return false;">좋아요</span>';
                    rows += '			<span class="count" id="like_cnt_'+cmtArr[i].hno+'">'+cmtArr[i].cnt+'</span>';
					rows += '		</button>';
					rows += '	</div>';
					rows += '</figure>';
					rows += '</li>';
