...
function Product(req){
	...
	this.getProduct = function (prodcode, productcode,gubun){
		...
		if(prodcode || prodcode!=''){
			data = db.getDBFunc({sp_name: 'product_detail', sp_param : prodcode, sp_gubun :gubun});
		}else{
			data = db.getDBFunc({sp_name: 'product_detail2', sp_param : productcode, sp_gubun :gubun});
		}
		...
		var data = db.getDBFunc({sp_name: 'product_multiimages', sp_param : prodcode, sp_gubun :gubun });
		pmiArr = data.data;
		if(pmiArr) {
		}else{
	...
	this.mdChoise = function (gubun){
		...
		if(data.data){
			...
			var data = '';
			if(gubun=='M') {
				var param = [mdchoiseStr];
				data = db.getDBFunc({sp_name: 'product_md_choise_product_m', sp_param : param });
			} else {
				var param = [sessid, mdchoiseStr];
				data = db.getDBFunc({sp_name: 'product_md_choise_product', sp_param : param });
			}
			var list = data.data;
	...
	this.categorybest = function (gubun){
		...
		if(req.code){
			...
			if(gubun=='M'){
				var param = [category];
				var data = db.getDBFunc({sp_name: 'product_category_best_m', sp_param : param });
			} else {
