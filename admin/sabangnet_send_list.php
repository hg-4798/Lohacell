...
function registeradd(flag){
	if(flag == 'n'){
		$("input[name='mode']").val("sabangProductRegister");
	}else if(flag == 's'){
...
<!-- 라인맵 -->
<div class="admin_linemap"><div class="line"><p>현재위치 : 상품관리 &gt;제휴몰 상품관리 &gt; <span>제휴몰 연동 상품 등록</span></p></div></div>
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<td valign="top">
				<form name=form1 action="<?=$_SERVER['PHP_SELF']?>" method=post>
					...
					<tr>
						<td>
						<div class="title_depth3">제휴몰 연동 상품 등록</div>
		...
		//							$qry.=" AND option_quantity like '%,0,%' ";
								...
								$sql = "SELECT option_price,productcode,productname,production,sellprice,consumerprice, sabangnet_option_quantity, option_quantity, ";
								...
								$cnt=0;
								while($row=pmysql_fetch_object($result)) {
									list($sabangnet_set_count, $sabangnet_set_flag)=pmysql_fetch("SELECT sum(quantity), count(no) FROM tblproduct_sabangnet WHERE productcode = '".$row->productcode."'");
							
									$number = ($t_count-($setup['list_num'] * ($gotopage-1))-$cnt);
									$strSabangEa = "<font color='red'>재고 미등록</font>";
									$strSabangEaCheckDisabled = " disabled";
									if(($row->option_quantity && $row->sabangnet_option_quantity) || ($row->sabangnet_option_quantity) || $sabangnet_set_count > 0){
										$strSabangEa = "<font color='blue'>재고 등록</font>";
										$strSabangEaCheckDisabled = "";
									}else if($row->option_quantity && !$row->sabangnet_option_quantity){
										$strSabangEa = "<font color='red'>재고 미등록</font>";
										$strSabangEaCheckDisabled = " disabled";
									}
									
									echo "<tr>";
									echo "<td><input type = 'checkbox' name = 'sabang_val[]' value = '".$row->productcode."' alt = '".$sabangnet_set_flag."' ".$strSabangEaCheckDisabled."></td>";
									echo "<td>".$number."</td>";
									echo "<td>";
									...
									echo "<td>".$strSabangEa."</td>";
									if($sabangnet_set_flag){
										echo "<td style = 'color:green;'><b>묶음 상품</b></td>";
									}else{
										echo "<td style = 'color:green;'>일반 상품</td>";
									}
				...
				<?php					
								//페이징
									...
									echo "</div>";
									echo "</div>";
									echo "<div class=\"btn_right\"><a href=\"javascript:registeradd('n');\">[일반 상품 전송]</a>&nbsp;&nbsp;<a href=\"javascript:registeradd('s');\">[묶음 상품 전송]</a></div>";
				...
				?>
					...
					<div class="sub_manual_wrap">
						...
						<dl>
							...
							<dd>
								 [일반 상품 전송] 항목 : 1차 옵션만 가지고 있는 상품을 사방넷으로 연동 할 경우 해당 버튼 사용 <br>
								 [묶음 상품 전송] 항목 : 2차 옵션까지 가지고있는 묶음 상품을 상품을 사방넷으로 연동 할 경우 해당 버튼 사용
							</dd>
							<dt><span>주의 사항</span></dt>
							<dd>
								일반 상품은 <b>[일반 상품 전송]</b>으로 사방넷으로 전송하고, 묶음 상품은 <b>[묶음 상품 전송]</b>으로 사방넷으로 보내야 정상적으로 상품 등록이 됩니다.<br>
								상품을 잘못 보낼시 단품으로 등록 될 수 있으며, 잘못 된 상품은 연동 상품 리스트에서 <b>[상품 수정]</b>으로 전송 후 <b>[간략 수정]</b>으로 다시 보내시면 됩니다.
							</dd>
						</dl>
					</div>
					</td>
