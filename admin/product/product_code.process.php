...
if($mode=="movesave" && ord($codes)) {	//이동된 분류 순서 저장
	...
	if ($row=pmysql_fetch_object($result)) {
		//fkey로 잡혀있는 메인메뉴 리스트의 상품 삭제
		$fkey_sql = "DELETE FROM tblmainmenulist WHERE category_idx = '".$row->idx."' ";
		pmysql_query($fkey_sql,get_db_conn());
		//fkey로 잡혀있는 추천 리스트의 상품 삭제 
		$fkey_sql = "DELETE FROM tblrecommendlist WHERE category_idx = '".$row->idx."' ";
		pmysql_query($fkey_sql,get_db_conn());
		
		$sql = "DELETE FROM tblproductcode WHERE code_a = '{$code_a}' ";
		$likecode=$code_a;
		if($code_b!="000") {
	...
	/*
		...
		pmysql_query($sql,get_db_conn());
	*/
	/*오늘의 특가 추가 2014.06.16 by Park Tae Young*/
		//오늘의 특가
		//$sql = "DELETE FROM tblproductoneday WHERE productcode LIKE '{$likecode}%' ";
		//pmysql_query($sql,get_db_conn());
		//오늘의 특가 상세
		//$sql = "DELETE FROM tblproductonedaydetail WHERE productcode LIKE '{$likecode}%' ";
		//pmysql_query($sql,get_db_conn());
	/*오늘의 특가 끝*/
...
*/
	...
	$onload="<script>alert('선택하신 분류를 삭제하였습니다.');parent.document.location.href='product_code.php?category=".$likecode."'</script>";
}
echo $onload;
