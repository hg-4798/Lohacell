    ...
    if(count($oi_type_arr)) {
        foreach($oi_type_arr as $k => $v) {
            switch($v) {
                ...
                case 63 : $subWhere[] = " (b.redelivery_type = 'Y' And a.oi_step1 in (2,3,4) And (coalesce(b.opt1_change, '') = '' And coalesce(b.opt2_change, '') = '') And b.op_step = 41) "; break;    //반품접수
                case 64 : $subWhere[] = " (b.redelivery_type = 'Y' And a.oi_step1 in (2,3,4) And a.oi_step2 = 42) "; break;   //반품완료(배송중 이상이면서 환불접수단계)
                case 65 : $subWhere[] = " (b.redelivery_type != 'G' And a.bank_date is not null And ((a.oi_step1 in (1,2) and b.op_step = 41) OR b.op_step = 42) And ((coalesce(b.opt1_change, '') = '' And coalesce(b.opt2_change, '') = '')))"; break;  //환불접수
                case 66 : $subWhere[] = " (b.redelivery_type != 'G' And a.oi_step1 > 0 And b.op_step = 44 And ((coalesce(b.opt1_change, '') = '' And coalesce(b.opt2_change, '') = ''))) "; break;  //환불완료
            }
        }
    }
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 주문/매출  &gt; 주문조회 및 배송관리 &gt;<span>전체 주문 조회(주문별)</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">전체 주문 조회 (주문기준)</div>
					<!-- 소제목 -->
...
?>
			    <tr bgcolor=<?=$thiscolor?> onmouseover="this.style.background='#FEFBD1'" onmouseout="this.style.background='<?=$thiscolor?>'">
                    ...
                    <td align=center style="font-size:8pt;padding:3"><!-- <?=$row->oi_step1." / ".$row->oi_step2?><br> --><?=$o_step[$row->oi_step1][$row->oi_step2]?><?=$stock_status?></td>
