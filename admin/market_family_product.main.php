...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblfamily_product SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	...
	$sql = "INSERT INTO tblfamily_product(special,special_list) VALUES (
	'{$special}',
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 기획전상품에 추가하였습니다.');</script>\n";
	...
	if(ord($selcodes)==0) {
		$sql = "DELETE FROM tblfamily_product WHERE special='{$special}' ";
	} else {
		$sql = "UPDATE tblfamily_product SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	}
	pmysql_query($sql,get_db_conn());
...
<input type="hidden" name="pidx" value="<?=$pidx?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">
			<?
				$pqry="select * from tblfamily WHERE promo_idx = '{$pidx}' ORDER BY display_seq asc;";
			...
			?>
				...
				<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special<?=$prow->seq?> class="font_d12">
			...
			<div class="table_main_setup"  id="divscroll" style="height:400px;overflow-x:hidden;overflow-y:auto;">
				<table border=0 cellpadding=0 cellspacing=0 width=100%>
					...
					if(ord($special)) {
						$mode="insert";
						$sp_prcode="";
						$sql = "SELECT special_list FROM tblfamily_product WHERE special='{$special}' ";
						$result = pmysql_query($sql,get_db_conn());
