...
<div class="content-wrap">
	<table cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td>
				<!-- 페이지 타이틀 -->
				<div class="title_depth3">배송준비중 리스트
					<span>입금일별, 배송일자별, 주문일자별 주문현황 및 주문내역을 확인/처리하실 수 있습니다.</span>
				</div>
		...
		<tr>
			<td>
				<!-- 소제목 -->
				<div class="title_depth3_sub">주문현황 조회</span>
				</div>
			</td>
		</tr>
						...
						<tr>
							<td width="100%">
								...
								<input type=text name=search value="<?=$search?>" style="width:197" class="input">
								 -->
												<textarea rows="2" cols="10" class="w200" name="search" id="search" style="resize:none;vertical-align:middle;">
													<?=$search?>
												</textarea>
											</TD>
										</tr>
...
<? while($ref1_data=pmysql_fetch_object($ref1_result)){ ?>
											<option value="<?=$ref1_data->code?>" <?=$selected[dvcode][$ref1_data->code]?>>
												<?=$ref1_data->company_name?>
											</option>
											<? } ?>
												</select>&nbsp;
							</TD>
...
-->
						<TR>
							...
							<TD class="td_con1">
								<input type="radio" name="delivery_type" value="" <?if($selected[delivery_type]=='' ){?>checked
								<?}?>>전체
									<?foreach ($arrChainCode as $k=>$v){?>
										<input type="radio" name="delivery_type" value="<?=$k?>" <?if($selected[delivery_type]. "|"==$k. "|"){?>checked
										<?}?>>
											<?=$v?>
												<?}?>
							</TD>
...
-->
						<TR>
							<th>
								<span>결제타입</span>
								<font style="font-family: '굴림,굴림';font-weight: normal;color:#777777;padding-left:50px">
									<input type="checkbox" style="vertical-align: middle;" class='chk_all' chk='chk_paymethod' name="paymethod_all" value="<?=$k?>"
									    <?if(count($paymethod_arr)==6 ) { echo 'checked'; } else { echo ''; }?>>전체선택</input>
								</font>
							</th>
							<TD class="td_con1">
								<?php
                                                                                foreach($arpm as $k => $v) {
                                                                                    ...
                                                                                    ?>
									<input type="checkbox" class='chk_paymethod' name="paymethod[]" value="<?=$k?>" <?=$selPaymethod?>>
									<?=$v?>
										<?
                                                                                }
                                                                                ?>
						...
						<TR>
							<th>
								<span>유입경로</span>
								<font style="font-family: '굴림,굴림';font-weight: normal;color:#777777;padding-left:50px">
									<input type="checkbox" style="vertical-align: middle;" class='chk_all' chk='chk_ord_flag' name="ord_flag_all" value="<?=$k?>"
									    <?if(count($ord_flag_arr)==3 ) { echo 'checked'; } else { echo ''; }?>>전체선택</input>
								</font>
							</th>
							<TD class="td_con1">
			...
			<tr>
                ...
                ";
						...
						<table cellpadding="0" cellspacing="0" width="100%">
							<tr>
								<td width="372">
									<img src="images/icon_8a.gif" border="0">
									<B>정렬 :
										<?php if($orderby=="DESC"){?>
										<A HREF="javascript:GoOrderby('ASC');">
											<B>
												<FONT class=font_orange>주문일자순↑</FONT>
											</B>
										</A>
										<?php }else{?>
										<A HREF="javascript:GoOrderby('DESC');">
											<B>
												<FONT class=font_orange>주문일자순↓</FONT>
											</B>
										</A>
										<?php }?> &nbsp;주문번호(붉은색) :
										<B>
											<FONT class=font_orange>PG 주문번호</FONT>
										</B>
								</td>
								<td width="" align="right">
									...
									<img src="images/icon_8a.gif" border="0">현재
									<b>
										<?=$gotopage?>/
									...
									<?if($thisordcd2!=$row->ordercode) {?>
										<?$thisordcd2=$row->ordercode;?>
											...
											<td style="font-size:8pt;padding:3;line-height:11pt" rowspan="<?=$arrListOrderCount[$row->ordercode]?>">
												주문자:
												<A HREF="javascript:SenderSearch('<?=$name?>');">
													<FONT COLOR="blue">
														<?=$name.$member_text?>
													</font>
												</A>
												...
												<?=$stridM?>
													<br> 구매자 :
													<FONT class=font_orange>
														<?if($row->staff_order=="Y"){echo "임직원";}else if($row->cooper_order=="Y"){echo "협력사";}else{echo "일반";}?>
													</font>
											</td>
											<?}?>

												<td align="center" ids="<?=$row->ordercode?>">
													<input type=checkbox name=chkordercode value="<?=$row->idx?>">
													<br>
													<!-- <?=$number?> -->
												</td>
												<?
													...
													<td style='text-align:left'>
														<a href="javascript:viewVenderInfo(<?=$row->vender?>)">
															<?=$row->brandname?>
														</a>
													</td>
													<?
														...
														<td style='text-align:left'>
															<a href="JavaScript:OrderDetailView('<?=$row->ordercode?>')">
																<?=$row->productname?>
																	<img src="images/newwindow.gif" border=0 align=absmiddle>
															</a>
															<br>
															<?=$opt_name?>
														...
														<td align=right style="font-size:8pt;padding:3">
															<?=number_format((($row->price+$row->option_price)*$row->quantity)-$row->coupon_price-$row->use_point-$row->use_epoint)?>
														</td>
...
//							$res = getErpProdShopStock_New($prodcd, $colorcd, $sizecd, $delivery_type);
																				...
																				<?}?>
																					<?//if(($row->delivery_type==0 && ($row->op_step == 1 || $row->op_step == 2)) && $row->season!="K"){?>
																						<?if(($row->delivery_type==0 && ($row->op_step == 1 || $row->op_step == 2)) && $row->season!="K" && $sum > 0){?>
																							<br>
																							<input type="button" value="매장입찰" class="btn_blue" style="padding:2px 5px 1px" onclick="javascript:store_pass('<?=$row->idx?>')">
																							<?}?>
														...
														<?if($row->delivery_type==0){
															...
															<td align=right style="font-size:8pt;padding:3">
																<select name=chkdeli_com class="input" style="font-size:8pt;padding:3" id="chkdeli_com_<?=$row->idx?>">
																	...
																	<?
                                                                            foreach($delicomlist as $k=>$v){
                                                                                ?>
																		<option value="<?=$delicomlist[$k]->code?>" <?if(trim($delicomlist[$k]->code)=="01"){echo "selected";}?>>
																			<?=$delicomlist[$k]->company_name?>
																		</option>
																		<?
                                                                            }
                                                                            ?>
																...
																<a href="javascript:InsertDvcode(<?=$row->idx?>, '<?=$row->ordercode?>');">
																	<img style="padding-top: 3px; padding-bottom: 7px; vertical-align: middle;" src="images/btn_input.gif" border="0">
																</a>
															</td>
															<?}else{?>
																<td align=center style="font-size:8pt;padding:3">
							...
							<tr>
								<td align='left' valign=middle>
									<a href="javascript:OrderCheckExcel();">
										<img src="images/btn_excel_select.gif" border="0" hspace="1">
									</a>&nbsp;
									<a href="javascript:OrderDeliveryAll();">
										<img src="images/market_send.gif" border="0" hspace="1">
									</a>&nbsp;
								</td>
							</tr>
							<tr>
						...
						<tr>
							<td background="images/counter_blackline_bg.gif" class="font_white" align="center" height="40">
								...
								<form name="dv_all_form" action="dvcode_all_indb_v2.php" enctype="multipart/form-data" method="post" onSubmit="return submit_data();">
									<input type="hidden" name="mode" value="updatedvcode"> 송장정보 일괄 업데이트
									<input type="file" name="csv_file" id="csv_file" alt="csv파일" accept=".csv">
