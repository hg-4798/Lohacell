...
$menu = array('업무구분','일자','거래처코드','거래처존','품번','색상','사이즈','수량','실판매금액','비고','상품명','옵션');
...
while ($row=pmysql_fetch_object($result)) {
	...
	while($order_data = pmysql_fetch_object($order_result)){
		...
		$data_list = array($type, $date, str_pad($code, 5, "0", STR_PAD_LEFT), $zone, $itemno, $color, $size, $order_data->quantity, ($order_data->price * $order_data->quantity), $etc, $order_data->productname, $opt_names);

			
			for($i = 0 ; $i < count($data_list) ; $i++){
				if($i!=0) echo ",";
				echo '"' . doubleQuote($data_list[$i]) . '"';
			}
			echo "\n";
