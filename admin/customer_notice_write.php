...
if($mode=="delete") {
	...
	$qry = "DELETE FROM tblcustomer_notice WHERE no ='".$no."'";
    ...
    if($mode=="insert") {
        $sql = "INSERT INTO tblcustomer_notice (
        title,
        content,
        notice_type,
        notice_date, 
...
//         exit;
        ...
        $sql = "UPDATE tblcustomer_notice SET ";
...
if( $mode == 'modfiy_select' ){
	$no = $_POST['no'];
	if(!$no) $no = $_GET['no'];
	$mSelectSql = "SELECT * FROM tblcustomer_notice WHERE no =".$no."";
...
function goBackList(){
	location.href="customer_notice.php";
}
