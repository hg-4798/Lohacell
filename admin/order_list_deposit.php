...
// 기본 검색 조건
$qry_from = "(
					SELECT op.*, p.prodcode, p.colorcode, p.tag_style_no FROM tblorderproduct op LEFT JOIN tblproduct p 
...
$excel_sql = "SELECT  a.vender , a.ordercode, a.productcode, a.productname, a.opt1_name, a.opt2_name, a.quantity, 
					...
					b.paymethod, b.oi_step1, b.oi_step2, b.is_mobile, a.self_goods_code, b.order_msg2, a.delivery_type, a.store_code, a.reservation_date, a.prodcode, a.colorcode, a.staff_order, a.cooper_order, a.tag_style_no
                FROM {$qry_from} left join tblmember m on b.id = m.id {$qry} ";
$excel_sql_orderby = "
		...
		<div class="content-wrap">
			<table cellpadding="0" cellspacing="0" width="100%">
				...
				<tr>
					<td>
						<!-- 페이지 타이틀 -->
						<div class="title_depth3">결제완료 리스트
							<span>결제완료 주문내역을 확인/처리하실 수 있습니다.</span>
						</div>
						<!-- 소제목 -->
													...
													<TR>
														<th>
															...
															<font style="font-family: '굴림,굴림';font-weight: normal;color:#777777;padding-left:50px">
																<input type="checkbox" style="vertical-align: middle;" class='chk_all' chk='chk_paymethod' name="paymethod_all"
																    value="<?=$k?>" <?if(count($paymethod_arr)==6 ) { echo 'checked'; } else { echo ''; }?>>전체선택</input>
															</font>
														</th>
														<TD class="td_con1">
															<?php
...
?>
													...
													<TR>
														<th>
															<span>유입경로</span>
															<font style="font-family: '굴림,굴림';font-weight: normal;color:#777777;padding-left:50px">
																<input type="checkbox" style="vertical-align: middle;" class='chk_all' chk='chk_ord_flag' name="ord_flag_all"
...
?>
																...
																<input type=text name=brandname value="<?=$brandname?>" style="width:197" class="input">
															</TD>
										</td>
										</TR>
										<?
...
?>
							...
							?>
																	...
																	<td align=right style="font-size:8pt;padding:3">
																		<?=number_format($row->price+$row->option_price)?>
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
																		<?=number_format($row->quantity)?>
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
																		<?=number_format($row->coupon_price)?>
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
																		<?=number_format($row->use_point)?>
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
																		<?=number_format($row->use_epoint)?>
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
					...
					<?}?>-->
																	<td align=center style="font-size:8pt;padding:3">
																		<?=$row->oi_step1." / ".$row->oi_step2." / ".$row->op_step?>
																			<br>
																			<?=$status?>
																				<!-- <?=$op_step[$row->op_step]?> -->
																	</td>
																	<td align=right style="font-size:8pt;padding:3">
																		<?=$arr_mobile[$row->is_mobile]?>
																	</td>
																	<?
							...
							<?if($oistep1 =='1') {?>
								<tr>
									<td background="images/counter_blackline_bg.gif" class="font_white" align="center" height='40'>
										...
										<a href="javascript:CheckOrder();" style='font-weight:bold;color: #FFDB1A;'>[ 적용하기 ]</a>
									</td>
								</tr>
								<?} else {?>
