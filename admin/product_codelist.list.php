...
<?php
			if(strlen($code)==12) {
				...
				if (strpos($type,'T')===FALSE) {		//기본카테고리
					$sql0 = "SELECT COUNT(*) as t_count FROM tblproduct AS a ";
					$qry = "WHERE a.productcode LIKE '{$likecode}%' ";
				} else {
					$sql0 = "SELECT COUNT(*) as t_count FROM tblproduct AS a, tblproducttheme AS b ";
					$qry = "WHERE b.code LIKE '{$likecode}%' ";
				...
				if (strpos($type,'T')===FALSE) {		//기본카테고리
					...
					elseif ($sort=="productname")	$sql.= "ORDER BY a.productname ";
