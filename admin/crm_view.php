...
if ($memo_len < 1) {
    $memo = '<div class = "ta-c"> 입력된 메모가 없습니다. </div>';
}

$mem_name = $row->name;
$mem_email = Common::Dectypt_AES128CBC($row->email,JayjunKey,JayjunIvKey);
$mem_hp = $row->mobile;
...
$mem_reserve	= $row->reserve;
$mem_home_post = $row->home_post;
$mem_home_addr = str_replace("↑=↑", ", ", $row->home_addr);
$mobile_temp = Common::Dectypt_AES128CBC($row->mobile,JayjunKey,JayjunIvKey);
$home_tel_temp = Common::Dectypt_AES128CBC($row->home_tel,JayjunKey,JayjunIvKey);
$mobile=explode("-",$mobile_temp);
$home_tel=explode("-",$home_tel_temp);
$home_addr_temp=explode("↑=↑",$row->home_addr);
$home_addr1=$row->home_addr;
...
$tot_pay_price = $tot_pay_price-$tot_pay_dc_price-$tot_pay_reserve;
...
## 쿠폰수 (유효한 쿠폰수)
$sql = "select count(*) from tblcouponissue where id = '".$mem_id."' and used = 'N' and date_end >= '".NOW."'";
...
function OrderDetailView(ordercode) {
    document.detailform.orn.value = ordercode;
    window.open("","orderdetail","scrollbars=yes,width=1000,height=600,resizable=yes");
			...
			<div class="lnb-wrap">
				...
				<div class="inner">
					<div class="quick-message">
						...
						<ul class="mrk-icon">
<?//                            @TODO 퍼블팀에 admin/static/css/crm.css 54 width: 50%로 수정요청 ?>
							<li style="width: 50%"><a href="javascript:SendSMS('<?=str_replace("-", "", $mem_hp)?>')"><img src="static/img/icon/icon_marketing_phone.gif" alt=""></a></li>
							<li style="width: 50%"><a href="javascript:MemberMail('<?=$mem_email?>','<?=$news_mail_yn?>');"><img src="static/img/icon/icon_marketing_mail.gif" alt=""></a></li>
<!--							<li><a href="javascript:MemberMemo2('--><?//=$mem_id?><!--')"><img src="static/img/icon/icon_marketing_memo.gif" alt=""></a></li>-->
					...
					<ul class="nav-menu">
						...
						<li <?if($menu=="mileage_act") {?>class="on"<?}?>><a href="javascript:go_menu('mileage_act');">포인트 정보</a></li>
					...
					<dl class="quick-info-box">
						...
						<dd>포인트<strong><?=number_format($mem_act_point)?></strong></dd>
...
else if($menu == "mem_memo") include "./crm_view_mem_memo.php";
...
<form name=detailform method="get" action="/admin/order/order_view.php" target="orderdetail">
<input type=hidden name=orn>
</form>
