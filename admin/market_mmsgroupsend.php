...
if($type=="up" && ($group=="A" || $group=="B" || $group=="G")) {
	...
	if (($cnt*$minusCount) <= $maxcount && $cnt>0) {
		$temp=SendMMS($sms_id, $sms_authkey, $totellist, $tonamelist, $fromtel, 0, $msg, $etcmsg, $_FILES); 
		if($temp['result'] == 'true'){
			$onload = "<script>window.onload=function(){ alert('문자 전송이 성공했습니다.'); location.replace('/admin/market_mmsgroupsend.php'); }</script>";
		}else{
			$onload = "<script>window.onload=function(){ alert('문자 전송이 실패했습니다.'); location.replace('/admin/market_mmsgroupsend.php'); }</script>";
		}
	} else if ($cnt==0) {
		$onload="<script>window.onload=function(){ alert('MMS를 발송할 회원이 없습니다.'); location.replace('/admin/market_mmsgroupsend.php'); }</script>";
	} else {
		$onload="<script>window.onload=function(){ alert('SMS 머니가 부족합니다. 충전후 이용하세요.'); location.replace('/admin/market_mmsgroupsend.php'); }</script>";
	}

	$type="";$msg="";$from_tel1="";$from_tel2="";$from_tel3="";$clicknum="";	$b_month="";$b_day="";$group="";$group_code="";
}

#if($maxcount>0 && ord($onload)==0 && $type!="changegroup" && $type!="birthsearch") $onload="<script>window.onload=function(){ alert('현재 ".ceil($maxcount/3)."건의 MMS를 발송하실 수 있습니다.'); }</script>";
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 마케팅지원 &gt; MMS 발송/관리 &gt;<span>MMS 등급/단체 발송</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">MMS 등급/단체 발송</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>전체회원/등급회원/생일회원에게 단체 MMS 발송을 할 수 있습니다.</span></div>
				</td>
			</tr>
			<tr>
				<td>
				...
				<tr>
					...
					<tr>
						...
						<TR>
							<TD align=center height="26" background="images/sms_down_01.gif"><input type="text" name="len_msg" value="0" style="PADDING-RIGHT:5px; WIDTH:20px; TEXT-ALIGN:right" onfocus="this.blur();" class="input_hide">bytes (최대2000 bytes)<script>cal_pre2();</script></TD>
						</TR>
						<TR>
							<TD HEIGHT=6></TD>
						</TR>
