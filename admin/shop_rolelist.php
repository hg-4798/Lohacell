...
<div class="content-wrap">
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">그룹 및 권한 설정<span>관리페이지 메뉴별 접근권한 그룹을 관리합니다.</span></div>
				</td>
			</tr>
					...
					while($row = pmysql_fetch_object($result)) {
						...
						if($count == 1 || $ltaskgroup != $taskgroup) {
							...
							if($row2->taskgroupcode == $taskgroup) {
								echo chr(10).("<option value='' disabled style=\"background:#A2A2A2;color:#ffffff;\">-------------------[{$row2->taskgroupname}]-------------------");
							} else {
								continue;
							}
						}
