...
$period[3] = date("Y-m-d",strtotime('-3 month'));
$period[4] = date("Y-m-d",$CurrentTime-(60*60*24*10));
$period[4] = date("Y-m-d",strtotime('-3 day'));

$orderby=$_POST["orderby"];
if(ord($orderby)==0) $orderby="DESC";
//phpinfo();
//$paystate=$_REQUEST["paystate"] == "" ? "Y,B,":$_REQUEST["paystate"];
//$deli_gbn=$_REQUEST["deli_gbn"] == "" ? "S,N,":$_REQUEST["deli_gbn"];
//echo "searstate"+$_REQUEST['searstate'];
//
...
if(substr($search_s,0,8)==substr($search_e,0,8)) {
	$qry.= "WHERE sabangner_idx ='' AND a.ordercode LIKE '".substr($search_s,0,8)."%' ";
} else {
	$qry.= "WHERE sabangner_idx ='' AND a.ordercode>='{$search_s}' AND a.ordercode <='{$search_e}' ";
}
...
WHERE sabangner_idx ='' AND a.deli_gbn='N'
AND ((SUBSTR(a.paymethod,1,1) IN ('B','O','Q')  AND (a.bank_date IS NULL OR a.bank_date='')) 
OR ( SUBSTR(a.paymethod,1,1) IN ('C','P','M','V')
...
WHERE sabangner_idx ='' AND a.deli_gbn='S'
{$check_qry}"));

//발송중
$y_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangner_idx ='' AND a.deli_gbn='Y'
{$check_qry}"));
...
WHERE sabangner_idx ='' AND a.deli_gbn='C'
{$check_qry}"));
...
WHERE sabangner_idx ='' AND a.deli_gbn='R'
{$check_qry}"));

//취소요청
$d_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangner_idx ='' AND a.deli_gbn='D'
{$check_qry}"));

//환불대기
$e_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangner_idx ='' AND a.deli_gbn='E'
{$check_qry}"));

/**		안쓰고 있으므로 주석
//환불
$cns_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangner_idx ='' AND ((SUBSTR(a.paymethod,1,1) IN ('B','O','Q') AND LENGTH(a.bank_date)=9) OR (SUBSTR(a.paymethod,1,1) IN ('C','P','M','V') AND a.pay_flag='0000' AND a.pay_admin_proc='C'))
{$check_qry}"));
**/
...
function searchForm() {
	document.form1.searstate.value = '1';
...
function OrderCheckExcel4() {
	...
	document.checkexcelform.action="order_excel4.php";
	//document.checkexcelform.target="_blank";
	document.checkexcelform.submit();
}

function OrderCheckExcel5() {
	document.checkexcelform.ordercodes.value="";
	for(i=1;i<document.form2.chkordercode.length;i++) {
		if(document.form2.chkordercode[i].checked) {
			document.checkexcelform.ordercodes.value+=document.form2.chkordercode[i].value.substring(1)+",";
	...
	document.checkexcelform.action="order_excel5.php";
	//document.checkexcelform.target="_blank";
	document.checkexcelform.submit();
}
	...
	if($(".deliGbn_Y:checked").length > 0){
		...
		if(confirm("주문의 배송정보를 변경할까요?")){
			...
			if(num==0){
				...
				form.action="order_chg_deli_indb.php?ordcode="+ordcode;
				form.submit();
			}
		}
	}
...
$(document).ready(function(){
	...
	$(document).on("mouseover", ".CLS_orderOverColor", function(){
		$(this).next().css('background', $(this).css('background'));
	})
	$(document).on("mouseout", ".CLS_orderOverColor", function(){
		$(this).next().css('background', $(this).css('background'));
	})
	
	$(document).on("mouseover", ".CLS_ItemOverColor", function(){
		$(this).prev().css('background', $(this).css('background'));
	})
	$(document).on("mouseout", ".CLS_ItemOverColor", function(){
		$(this).prev().css('background', $(this).css('background'));
	})
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<input type="hidden" name="searstate" value=""/>
			<tr>
				<td>
...
?>
			...
			<tr>
				<td style="padding-top:4pt;" align="right"><a href="javascript:searchForm();"><img src="images/botteon_search.gif"  border="0"></a>&nbsp;<a href="javascript:OrderExcel();"><img src="images/btn_excel1.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:AddressPrint();"><img src="images/btn_adress.gif"  border="0"></a>&nbsp;<a href="javascript:OrderCheckExcel4();"><img src="img/btn/btn_company_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel3();"><img src="images/btn_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel5();"><img src="images/btn_distribution_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel2();"><img src="images/btn_erp_sale.gif"  border="0" hspace="1"></a></td>
			</tr>
			</form>
			<tr>
				<td height="20"></td>
			...
			<tr>
				...
				foreach($list as $row){
					...
					while($row2=pmysql_fetch_object($result2)) {
						...
						$vsql = "select vip_product, staff_product from tblproduct where productcode='{$row2->productcode}' ";
						$vres = pmysql_query($vsql); $vrow = pmysql_fetch_array($vres);
						...
						if($vendercnt>0) {
							...
							$prval.="	<td>";
							if($vrow[vip_product] == "1"){
							$prval.="<img src=\"img/icon/icon_vip.gif\" border=\"0\" style=\"margin-right:2px;\">";
							}
							if($vrow[staff_product] == "1"){
							$prval.="<img src=\"img/icon/icon_staff.gif\" border=\"0\" style=\"margin-right:2px;\">";
							}
							$prval.=titleCut(58,$row2->productname)."";
							if($over_product>0){
								$prval.=" 외 ".$over_product."개";
							}
						} else{
							$prval.="	<td><div class=\"ta_l\">";
							if($vrow[vip_product] == 1){
							$prval.="<img src=\"img/icon/icon_vip.gif\" border=\"0\" style=\"margin-right:2px;\">";
							}
							if($vrow[staff_product] == "1"){
					...
					echo "<tr bgcolor={$thiscolor} onmouseover=\"this.style.background='#FEFBD1'\" onmouseout=\"this.style.background='{$thiscolor}'\" class = 'CLS_orderOverColor'>\n";
					...
					if(ord($stridX)) {
						...
						echo "<br> 아이디: ".$stridM."(".$q_count.")";
					}
					*/
					// 주문자가 기업 회원일 경우 회사명,대표자 명으로 분기처리
					// 2015 01 20 유동혁
					if($row->mem_type == 1){
						echo "	회사명: <A HREF=\"javascript:SenderSearch('{$name}');\"><FONT COLOR=\"blue\">{$row->office_name}({$name})</font></A>".$blackImage;		
					}
					else{
					echo "	주문자: <A HREF=\"javascript:SenderSearch('{$name}');\"><FONT COLOR=\"blue\">{$name}</font></A>".$blackImage;
					}
					if(ord($stridX)) {
						...
						if($row->mem_type == 1 && $row->office_representative != ''){
							echo "<br> 대표자 명: ".$row->office_representative."(".$q_count.")";							
						}else if($row->mem_type == 1){					// 대표자 이름 없을때
							echo "<br> 주문자: ".$name."(".$q_count.")";								
						}
						else{
							echo "<br> 아이디: ".$stridM."(".$q_count.")";
						}
					...
					list($group_name)=pmysql_fetch_array(pmysql_query("select group_name from tblmembergroup where group_code='".$row->group_code."'"));
					echo "	<td height=100%>\n";
					if($group_name){
						echo $group_name;
					}else{
				...
				//	echo "	<col width=75 align=center></col>\n";
					...
					if($row->deli_type == "0"){					
						echo "	<td align=center>선불&nbsp;&nbsp;&nbsp;</td>\n";
					}else if($row->deli_type == "1"){
						echo "	<td align=center><font color=red>착불&nbsp;&nbsp;&nbsp;</font></td>\n";
					}
					...
					# 주문 리스트에서 상품 리스트 보이기 시작
					echo "<tr bgcolor='{$thiscolor}' onmouseover=\"this.style.background='#FEFBD1'\" onmouseout=\"this.style.background='{$thiscolor}'\" class = 'CLS_ItemOverColor' style = 'padding-top:40px;'>\n";
					echo "	<td colspan = '20'>\n";
					echo "		<table border='0' cellpadding='0' cellspacing='0' width='100%'>\n";
					echo "			<tr>\n";
					echo "				<td style = 'text-align:center;width:80px;'><b>이미지</b></td>\n";
					echo "				<td style = 'text-align:center;'><b>상품명</b></td>\n";
					echo "				<td style = 'text-align:center;'width:40px;'><b>수량</b></td>\n";
					echo "				<td style = 'text-align:center;'width:100px;'><b>판매가</b></td>\n";
					echo "				<td style = 'text-align:center;'width:180px;'><b>배송업체/송장번호</b></td>\n";
					echo "			</tr>\n";
						$countItems = 0;	//exdebug($row->items);
						foreach($row->items as $itemVal){
							$tdBorderColor = "";
							$countItems++;
							if(count($row->items) == $countItems){
								$tdBorderColor = "border-bottom:0px solid #FFFFFF;";
							}else{
								$tdBorderColor = "";
							}
							$product_img = $itemVal->tinyimage;
							if(file_exists($Dir.DataDir."shopimages/product/{$product_img}")) $file = $product_img;
							elseif(file_exists($Dir.DataDir."shopimages/product/{$product_img}")) $file = $product_img;
							else $file = "NO";

							if($file!="NO") {
								$fileUrls = "<img name='bigimgs' src='".$Dir.DataDir."shopimages/product/{$file}' style = 'width:50px;'>";
							}else if(file_exists($Dir.$product_img)){
								$fileUrls = "<img name='bigimgs' src='".$Dir.$product_img."' style = 'width:50px;'>";
							}else{
								$fileUrls = "-";
							}
							echo "<tr>\n";
							echo "	<td style = 'text-align:center;width:80px;".$tdBorderColor."'>
											<a href = \"javascript:ProductInfo('".substr($itemVal->productcode,0,12)."','".$itemVal->productcode."','YES')\">".$fileUrls."</a>
										</td>\n";
							//echo "	<td style = 'text-align:left;".$tdBorderColor."'>
							echo "	<td style = 'text-align:center;".$tdBorderColor."'>";
							echo "
							<a href = \"javascript:ProductInfo('".substr($itemVal->productcode,0,12)."','".$itemVal->productcode."','YES')\">".$itemVal->productname."</a>";

							$serial_arr = explode("|",$itemVal->product_serial);
							echo "<img onclick=* src=image/insert_icon.gif  style='margin: 0 0 0 100px';></img>";
/**
						//exdebug(explode("|",$itemVal->product_serial));
						$serial_arr = explode("|",$itemVal->product_serial);
				// 수량에따른 반복처리 
				for($i = 0 ; $i < $itemVal->quantity ; $i++){
							echo"<p>serial".($i+1)."  <input type=text name='product_serial[".$itemVal->ordercode."][".$itemVal->productcode."][".$i."]' value=".$serial_arr[$i]."></input>";
				}**/
				/////////////////////////////////////////////////////////////////////////	
/**
							echo "	<td style = 'text-align:center;".$tdBorderColor."'>
											<a href = \"javascript:ProductInfo('".substr($itemVal->productcode,0,12)."','".$itemVal->productcode."','YES')\">".$itemVal->productname."</a><p>serial :
											<input type=text name='product_serial[".$itemVal->ordercode."][".$itemVal->productcode."]' value=".$itemVal->product_serial."></input>	
										</td>\n";**/
							echo "	<td style = 'width:40px;".$tdBorderColor."'>".number_format($itemVal->quantity)."</td>\n";
							echo "	<td style = 'width:100px;".$tdBorderColor."'>".number_format($itemVal->price * $itemVal->quantity)."</td>\n";
							
							# 택배 시작
							echo "	<td style = 'width:180px;".$tdBorderColor."'>\n";
							echo "		배송업체 : ";
							echo "		<select name='chkdeli_com[".$itemVal->ordercode."][".$itemVal->productcode."]' style=\"width:100;height:18;font-size:8pt\">\n";
							echo "		<option value=\"\">없음</option>\n";
							$deli_url="";
							$trans_num="";
							$company_name="";
							for($yy=0;$yy<count($delicomlist);$yy++) {
								if($pg_type=="B" && strstr("QP", $itemVal->paymethod[0])) {
									if(ord($delicomlist[$yy]->dacom_code)) {
										echo "		<option value=\"{$delicomlist[$yy]->code}\"";
										if($itemVal->deli_com>0 && $itemVal->deli_com==$delicomlist[$yy]->code) {
											echo " selected";
							...
							echo "		</select><br>\n";
							echo "		송장번호 : ";
							echo "		<input type='text' name='chkdeli_num[".$itemVal->ordercode."][".$itemVal->productcode."]' value='{$itemVal->deli_num}' size='10' maxlength='20' onkeyup='strnumkeyup(this)' class='input' style = 'text-align:right;'>";
							if(ord($itemVal->deli_num) && ord($deli_url)) {
								if(ord($trans_num)) {
									$arrtransnum=explode(",",$trans_num);
									$pattern=array("[1]","[2]","[3]","[4]");
									$replace=array(substr($itemVal->deli_num,0,$arrtransnum[0]),substr($itemVal->deli_num,$arrtransnum[0],$arrtransnum[1]),substr($itemVal->deli_num,$arrtransnum[0]+$arrtransnum[1],$arrtransnum[2]),substr($itemVal->deli_num,$arrtransnum[0]+$arrtransnum[1]+$arrtransnum[2],$arrtransnum[3]));
									$deli_url=str_replace($pattern,$replace,$deli_url);
								} else {
									$deli_url.=$itemVal->deli_num;
								}
								echo "&nbsp;<a href = \"javascript:DeliSearch('{$deli_url}');\" style='color:#FFFFFF;border-color:#666666;background-color:#666666;padding:2px;'>추적</a>";
							} else {
								echo "&nbsp;<a href = 'javascript:;' style='color:#FFFFFF;border-color:#666666;background-color:#666666;padding:2px;'>추적</a>";
							}
							echo "	</td>\n";
							# 택배 종료
			...
			?>
			<tr>
				<td colspan = '17' style = 'text-align:right;padding:15px 5px 15px 5px;;'>
...
?>
				...
				<td>
					<ul style = 'list-style:none;width:100%;'>
						...
						<li style = 'width:50%;float:left;text-align:right;'>
							<a href = 'javascript:;'><img src="images/btn_deliinfomodify.gif" border="0" align="absmiddle" onClick = 'order_chg_deli()'></a>
						</li>
					</ul>
					<br><br>
				</td>
			...
			<tr>
				<td style="padding-top:4pt;"><a href="javascript:OrderDeliPrint();"><img src="images/btn_print.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckPrint();"><img src="images/btn_juprint.gif"  border="0" hspace="0"></a>&nbsp;<a href="javascript:OrderCheckExcel();"><img src="images/btn_excel1.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckDelete();"><img src="images/btn_judel.gif"  border="0"></a>&nbsp;<a href="javascript:OrderCheckExcel4();"><img src="img/btn/btn_company_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel3();"><img src="images/btn_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel5();"><img src="images/btn_distribution_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel2();"><img src="images/btn_erp_sale.gif"  border="0" hspace="1"></a></td>
			</tr>
			<!--tr>
				<td align="center">
				<table cellpadding="0" cellspacing="0" width="100%">
