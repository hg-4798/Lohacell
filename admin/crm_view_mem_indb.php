...
$home_addr2			= Common::Enctypt_AES128CBC(trim($_REQUEST["home_addr2"]),JayjunKey,JayjunIvKey);
$home_tel				= Common::Enctypt_AES128CBC(implode("-",$_REQUEST["home_tel"]),JayjunKey,JayjunIvKey);
//$email					= trim($_REQUEST["email1"])."@".trim($_REQUEST["email2"]);
$email					= Common::Enctypt_AES128CBC(trim($_REQUEST["email"]),JayjunKey,JayjunIvKey);
$skintype					= trim($_REQUEST['skintype']);
...
if($news_kko_yn!="Y") $news_kko_yn	= "N";
...
if($mode=="update"){
	...
	$sql.= "home_addr	= '{$home_addr1}', ";
    $sql.= "home_addr_detail	= '{$home_addr2}', ";
