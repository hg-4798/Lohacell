...
// 배너 상단 타이틀
$banner_up_title = $_POST['banner_up_title'];
...
$v_banner_img2_m = $_POST['v_banner_img2_m'];
//배너 노출순서
$banner_sort=$_POST["banner_sort"];
//배너 링크
$banner_link=pg_escape_string($_POST["banner_link"]);
//모바일배너 링크
...
# 메뉴카테고리
$banner_sub_title = $_POST['sub_title'];
# 카테고리 추가
$code_a = $_POST['code_a'];
$code_b = $_POST['code_b'];
$code_c = $_POST['code_c'];
...
//	alert_go('삭제가 완료되었습니다.', $_SERVER['REQUEST_URI']);
	...
	if(count($banner_img) > 1){
		...
		if( strlen( $banner_img["banner_img"][0]["v_file"] ) > 0 ){
			...
			$flg = true;
		}else if(strlen($v_banner_img) > 0 && strlen( $banner_img["banner_img"][0]["v_file"] ) == 0){
            $temp_img = $v_banner_img;
        }
		if( strlen( $banner_img["banner_img2"][0]["v_file"] ) > 0 ){
			if( is_file( $imagepath.strlen($temp_img) > 0 && strlen($temp_img2) > 0 )){
				$banner_file->removeFile( $v_banner_img2 );

			}

            $temp_img2 = $banner_img["banner_img2"][0]["v_file"];

			$flg = true;
		}

        if(strlen( $banner_img["banner_img2"][0]["v_file"] ) > 0){
            $temp_img = $temp_img."|".$temp_img2;
        }else if(strlen( $v_banner_img2 ) > 0 && strlen( $banner_img["banner_img2"][0]["v_file"] ) == 0){
            $temp_img = $temp_img."|".$v_banner_img2;
        }
          //  echo $temp_img;exit;
		if($flg){
...
// echo 'count=['.count($banner_img).'],'.$temp_img.',img1=['.$v_banner_img.'],img2=['.$v_banner_img2.']'; exit();
    ...
    if(count($banner_img) > 1){
       // print_r($banner_img);exit;
        ...
        if( strlen( $banner_img["banner_img"][1]["v_file"] ) > 0 ){
            ...
            $temp_img_m = $banner_img["banner_img"][1]["v_file"];

            $flg_m = true;
        }else if(strlen($v_banner_img_m) > 0 && strlen( $banner_img["banner_img"][1]["v_file"] ) == 0){
            $temp_img_m = $v_banner_img_m;
        }

        if( strlen( $banner_img["banner_img2"][1]["v_file"] ) > 0 ){
            if( is_file( $imagepath.strlen($temp_img_m) > 0 && strlen($temp_img2) > 0 )  ){
                $banner_file->removeFile( $v_banner_img2 );

            }
            $temp_img2_m = $banner_img["banner_img2"][1]["v_file"];
            $flg_m = true;
        }

        if(strlen( $banner_img["banner_img2"][1]["v_file"] ) > 0){
            $temp_img_m = $temp_img_m."|".$temp_img2_m;
        }else if(strlen( $v_banner_img2_m ) > 0 && strlen( $banner_img["banner_img2"][1]["v_file"] ) == 0){
            $temp_img_m = $temp_img_m."|".$v_banner_img2_m;
        }
        if($flg_m){
            $where[]="banner_img_m='".$temp_img_m."'";
            $flg_m = false;
    ...
    $where[]="banner_sub_title='".$banner_sub_title."'";
...
//		alert_go('수정이 완료되었습니다.', $_SERVER['REQUEST_URI']);
	...
	if(count($banner_img) > 1){
		$temp_img = $banner_img["banner_img"][0]["v_file"]."|".$banner_img["banner_img2"][0]["v_file"];
        $temp_img_m = $banner_img["banner_img"][1]["v_file"]."|".$banner_img["banner_img2"][1]["v_file"];
	} else {
		$temp_img = $banner_img["banner_img"][0]["v_file"];
        $temp_img_m = $banner_img["banner_img"][1]["v_file"];
	}
...
$arrUpTitleAdm = array('77','78', '79','85', '87', '118', '123', '125');
...
var bannerImgCate = [ '90', '93', '94', '99', '100', '101', '102', '103', '105', '106', '107', '111', '119', '120', '126','127','128','121','122','124']; // 이미지형 배너
var bannerProductTextCate = ['78', '79', '95', '96', '97', '98', '104', '110']; // 상품+텍스트형 배너
var bannerImgProductCate = ['80']; // 이미지+상품형 배너
var bannerImgProductTextCate1 = ['77','85','87', '118', '123', '125']; // 이미지+상품+텍스트형1 배너
...
function changeAction( mode , num ,obj){
	...
	if( mode == '0' ){
        if(num =='113'){
            if(!$("input[name='menu_category']:checked").val()){
...
function typeChange(){
	...
	if( bannerImgCate.indexOf( type ) > -1 ){
        ...
        if ( type == "85" || type=="118" || type=="77") {
            $("#ID_RelationProduct").hide();
            $("#ID_trTabSubTitle").show();
        }else{
            $("#ID_RelationProduct").show();
            $("#ID_trTabSubTitle").hide();
        }
        if (type=="87" ) {
            $("#ID_trTabSubTitle").show();
        }else{
...
<input type="hidden" name="search_up_title" value="<?=$search_up_title?>" />
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3"><?=$subPage_dept3_title?> 관리 <span><?=$subPage_dept3_title?> 등록/수정/삭제 처리를 할 수 있습니다.</span></div>
...
?>
        <label>
            <input type="radio" name="menu_category" value="BRAND"  class="hj" <?if($mSelect['menu_category']=="BRAND"){echo "checked";}?>>
            <span class="lbl">BRAND</span>
        </label>
        <label>
            <input type="radio" name="menu_category" value="SHOPPING" class="hj" <?if($mSelect['menu_category']=="SHOPPING"){echo "checked";}?>>
            <span class="lbl">SHOPPING</span>
        </label>
        <label>
            <input type="radio" name="menu_category" value="EVENT"  class="hj" <?if($mSelect['menu_category']=="EVENT"){echo "checked";}?>>
            <span class="lbl">EVENT</span>
        </label>
        <label>
            <input type="radio" name="menu_category" value="REVIEW" class="hj" <?if($mSelect['menu_category']=="REVIEW"){echo "checked";}?>>
...
?>
                        ...
                        <tr id='ID_trTabSubTitle' style='display: none;' >
                            <th><span>서브 타이틀</span></th>
                            <TD><INPUT maxLength=80 size=80 id='sub_title' name='sub_title' value="<?=$mSelect['banner_sub_title']?>"></TD>
                        </tr>
...
<!--							--><?//if ($form_listing_type == "bannerImgProductTextCate1" || $form_listing_type == "bannerImgProductTextCate2") {?>
							<th><span>배너 이미지(PC)</span>
                                <?if($no==113){?><div class="font_orange">(권장사이즈:270X145)</div><?}?>
                                <?if($no==123){?><div class="font_orange">(이미지 : 580X696)</div><?}?>
                                <?if($no==125){?><div class="font_orange">(이미지 : 1010X480)</div><?}?>
                                <?if($no==87){?><div class="font_orange">(권장사이즈:620X840)</div><?}?>
                                <?if($no==88){?><div class="font_orange">(이미지 : 1200x650)</div><?}?>
                                <?if($no==77){?><div class="font_orange">(1번 배너:644X720)<br>(2번 배너:644X720)</div><?}?>
                                <?if($no==85){?><div class="font_orange">(권장사이즈:434X460)</div><?}?>
                                <?if($no==118){?><div class="font_orange">(권장사이즈:1000X565)</div><?}?>
                                <?if($no==127){?><div class="font_orange">(권장사이즈:1200X440)</div><?}?>
							</th>
							<?/*}ELSE{*/?><!--
							<th><span>배너 이미지(PC)</span>
...
<?if($no!=87){?><!-- BEST SELLER 관리일 경우 비노출 처리 시작 -->
...
<!--							--><?//if ($form_listing_type == "bannerImgProductTextCate1" || $form_listing_type == "bannerImgProductTextCate2") {?>
							<th><span>배너 이미지(MOBILE)</span>
                                ...
                                <?if($no==77){?><div class="font_orange">(1번 배너:720X714)<br>(2번 배너:600X600)</div><?}?>
                                <?if($no==85){?><div class="font_orange">(권장사이즈:434X460)</div><?}?>
                                <?if($no==118){?><div class="font_orange">(권장사이즈:1000X565)</div><?}?>
                                <?if($no==127){?><div class="font_orange">(권장사이즈:750X822)</div><?}?>
							</th>
							<?/*}ELSE{*/?><!--
							<th><span>배너 이미지(MOBILE)</span>
							...
							<td class="td_con1" colspan="3" style="position:relative">
                                <?
                                if($no==123 || $no==77){
                                    $t_img_m = explode ("|", $mSelect['banner_img_m']);
                                    ?>

                                    <input type=file name="banner_img[]" style="WIDTH: 400px"><br>
                                    <!--<span class="font_orange">(권장이미지 : )</span>-->
                                    <input type=hidden name="v_banner_img_m" value="<?=$t_img_m[0]?>" >
                                    <br>
                                    <?
                                    if(is_file($imagepath.$t_img_m[0])){
                                        ?>
                                        <div style='margin-top:5px' >
                                            <img src='<?=$imagepath.$t_img_m[0]?>' style='max-width: 125px;' />
                                        </div>
                                        <?
                                    }
                                    ?>
                                    <input type=file name="banner_img2[]" style="WIDTH: 400px"><br>
                                    <!--<span class="font_orange">(권장이미지 : )</span>-->
                                    <input type=hidden name="v_banner_img2_m" value="<?=$t_img_m[1]?>" >
                                    <?
                                    if(is_file($imagepath.$t_img_m[1])){
                                        ?>
                                        <div style='margin-top:5px' >
                                            <img src='<?=$imagepath.$t_img_m[1]?>' style='max-width: 125px;' />
                                        </div>
                                        <?
                                    }
                                    ?>
...
<?}?><!-- BEST SELLER 관리일 경우 비노출 처리 끝 -->
						<TR id='ID_RelationProduct' style='display: none;'>
							<th>
								...
								<?if($no==125){?> <span>상품 3개 진열</span>&nbsp;&nbsp;<?}?>
                                    <span>관련상품</span>
								<a href="javascript:ChoiceProduct();"><img src="./images/btn_search2.gif" style='vertical-align:middle;padding-top:3px;padding-bottom:7px;'/></a>
								<?}else{?>
								<span>관련상품</span>&nbsp;&nbsp;
							...
							<td align="left">
									<div style="margin-top:0px; margin-bottom: 0px;">							
										<table cellpadding=0 cellspacing=0 style='border:0px; width:100%;' name="prList" id="check_relationProduct">
										<input type="hidden" name="limit_relationProduct" id="limit_relationProduct" value=""/>
                                            ...
                                            <tbody id="best_tbody">
                                            <?php if(!empty($thisBannerProduct)){?>

                                                <?foreach($thisBannerProduct as $bannerProductKey=>$bannerProduct){?>
                                                <tr data-productcode="<?=$bannerProduct[productcode]?>">
                                                    <td><img style="width: 40px; height:40px;" src="<?=getProductImage($Dir.DataDir.'shopimages/product/', $bannerProduct['tinyimage'] );?>" border="1"/></td>
                                                    <td class="ta_l"><?=$bannerProduct[productname]?></td>
                                                    <td><?=$bannerProduct[productcode]?><input type="hidden" name="relationProduct[]" value="<?=$bannerProduct[productcode]?>"></td>
						...
						<tr id='ID_trLink' style='display: none;' >
                            <?php if($no==118){?>
							<th><span>동영상 URL</span></th>
							<?php }else{?>
							<th><span>링크</span></th>
							<?php }?>
							<TD><INPUT  size=80 id='banner_link' name='banner_link' value="<?=htmlspecialchars($mSelect['banner_link'])?>" ></TD>
						</tr>
						<tr id='ID_trmLink' style='display: none;' >
							<th><span>모바일 링크</span></th>
							<TD><INPUT  size=80 id='banner_mlink' name='banner_mlink' value="<?=$mSelect['banner_mlink']?>" ></TD>
						</tr>
...
?>
			...
			<tr>
				...
				<?} else if ($form_listing_type == "bannerImgProductTextCate1") { // 이미지+상품+텍스트형1 배너?>
				    ...
				    <th>배경 이미지 PC</th>
					<th>배경 이미지 MOBILE</th>
                    <?php if ( $mSelect['banner_no'] != "85" && $mSelect['banner_no'] != "118" && $mSelect['banner_no'] != "77") { ?>
					<th>관련상품수</th>
					<?php }?>
					<th>링크</th>
				<?} else if ($form_listing_type == "bannerImgProductTextCate2" ) { // 이미지+상품+텍스트형2 배너?>
...
?>
	...
	<?} elseif ($form_listing_type == "bannerProductTextCate" || $form_listing_cate_type == "on") { ?>
					...
					<td>
<?php
			if( $bVal['menu_category'] ){
				echo $bVal['menu_category'];
...
?>	
					...
					<td>
						<div id='img_display2' >
                            <?php
                            $t_img_m = explode ("|", $bVal['banner_img_m']);
                            if(count($t_img_m) > 1){
                                if( is_file($imagepath.$t_img_m[0]) ){
                                    ?>
                                    <img src='<?=$imagepath.$t_img_m[0]?>' style='max-width : 70px;' >
                                    <?php
                                }
                            } else {
//echo count($t_img);
...
<?	} ?>
    ...
    <tr data-productcode="${productcode}">
        ...
        <td>${productcode}<input type="hidden" name="relationProduct[]" value="${productcode}"></td>
        <td><button type="button" class="btn_type1" onclick="ProductBest.remove(this)">삭제</td>
    </tr>
    </textarea>
...
*/
    ...
    var ChoiceCallback = function(tr_new){
        ...
        $.each(tr_new, function(i,e) {
            ...
            if (idx >= ProductBest.max) {
                alert('추천상품은 최대 ' + ProductBest.max + '개까지 등록가능합니다.');
                return false;
            }
            ...
            target.append(html);
            $("input[name='productcode']").val("GOODS");
