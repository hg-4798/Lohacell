...
$sabangnet_flag=$_POST["sabangnet_flag"];
...
$checked["sabangnet_flag"][$sabangnet_flag] = "checked";
...
function registeraddSoldout(){
	var checkNumber = 0;
	$("input[name='mode']").val("sabangProductSoldout");
	$("input[name='sabang_val[]']").each(function(){
		if($(this).is(":checked")){
			checkNumber++;
		}
	})
	if(checkNumber > 0){
		$.ajax({
			type: "POST", 
			url: "sabangnet_indb.php", 
			data: $("form[name='form1']").serialize(), 
			beforeSend: function () {
				popupJquery('sabangnet_process_popup.php', 397, 200);
			}
		}).done(function ( data ) {
			$('#createIfrm').contents().find('.sabangnet_pop_msg').html(data);
		});
		/*
...
function registeradd(){
	var checkNumber = 0;
	$("input[name='mode']").val("sabangProductModify");
	$("input[name='sabang_val[]']").each(function(){
		if($(this).is(":checked")){
			checkNumber++;
		}
...
function registeraddAll(){
	var checkNumber = 0;
	$("input[name='mode']").val("sabangProductSetRegister");
	$("input[name='sabang_val[]']").each(function(){
		if($(this).is(":checked")){
			checkNumber++;
		}
	...
	if(checkNumber > 0){
		...
		/*
		document.form1.action="sabangnet_indb.php";
		document.form1.mode="sabangProductRegister";
		document.form1.target="";
		document.form1.submit();
		*/
	}else{
...
function popupStock(pdcode){
	window.open("../admin/sabangnet_send_stock_popup.php?chk=1&productcode="+pdcode, "registerStock","width=400,height=300,scrollbars=yes,status=no");
	//sabangnet_send_stock_popup.php
}
</script>
<!-- 라인맵 -->
<div class="admin_linemap"><div class="line"><p>현재위치 : 상품관리 &gt;제휴몰 상품관리 &gt; <span>제휴몰 연동 상품리스트</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<form name=form1 action="<?=$_SERVER['PHP_SELF']?>" method=post>
				...
				<tr>
					<td>
					<div class="title_depth3">제휴몰 연동 상품리스트</div>
					...
					?>
							...
							<tr>
								<th><span>연동상태(사방넷)</span></th>
								<td><input type="radio" name="sabangnet_flag" value="" <?=$checked["sabangnet_flag"]['']?>/>전체 <input type="radio" name="sabangnet_flag" value="Y" <?=$checked["sabangnet_flag"]['Y']?>/> 수정성공&nbsp;&nbsp; <input type="radio" name="sabangnet_flag" value="R" <?=$checked["sabangnet_flag"]['R']?>/> 수정실패</td>
							</tr>
						</table>
			...
			<?php
							...
							$qry.= "AND (SELECT count(no) FROM tblproduct_sabangnet WHERE productcode = a.productcode) > 0 ";
							...
							if($sabangnet_flag == 'Y')	 $qry.="AND sabangnet_flag='Y' ";
							elseif($sabangnet_flag == 'R') $qry.="AND sabangnet_flag='R'";
							...
							$arraySabangStatus = array('Y'=>"<font color = 'blue'>수정성공</font>", 'R'=>"<font color = 'red'>수정실패</font>");
								
							echo "<tr>";
							echo "<td><input type = 'checkbox' name = 'sabang_val[]' value = '".$row->productcode."'></td>";
							echo "<td>".$number."</td>";
							echo "<td>";
							...
							echo "<td>".substr($row->modifydate,0,10)."</td>";
							echo "<td>".$arraySabangStatus[$row->sabangnet_flag]."</td>";
			...
			<?php					
							//페이징
								...
								echo "</div>";
								echo "</div>";
								echo "<div class=\"btn_right\"><a href=\"javascript:registeraddAll();\">[상품수정]</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript:registeradd();\">[간략수정]</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript:registeraddSoldout();\">[품절처리]</a></div>";
			...
			?>
					...
					<div class="sub_manual_wrap">
						...
						<dl>
							<dt><span>수정 항목 내역</span></dt>
							<dd>
								 [상품수정] 항목 : 처음 등록 할 때 등록 되었던 모든 내용이 수정 가능함. (옵션 등록 / 수정 가능)<br>
								 [요약수정] 항목 : 상품명, 자체코드, 상품상태, 원가, 판매가, TAG(소비자가), 옵션(등록 할때 옵션을 등록하지 않았다면 옵션이 추가/수정되지 않음)<br>
								 [품절처리] 항목 : 해당 상품의 판매 상태를 품절로 변경.
							</dd>
						</dl>
					</div>
					</td>
