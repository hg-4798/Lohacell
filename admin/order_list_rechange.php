...
#---------------------------------------------------------------
	...
	if($vendercnt>0) {
		...
		$qry_from .= "join (Select p.oc_no, p.vender, pb.brandname from tblorderproduct p left join tblvenderinfo v on p.vender = v.vender left join tblproductbrand pb on p.vender=pb.vender where p.oc_no > 0 and p.redelivery_type = 'G' and p.op_step=41 group by p.oc_no, p.vender, pb.brandname) b on toc.oc_no=b.oc_no {$subqry} ";
	} else {
		$qry_from .= "join (Select oc_no from tblorderproduct p where oc_no > 0 and redelivery_type = 'G' and op_step=41 group by oc_no) b on toc.oc_no=b.oc_no ";
	}
...
function searchForm() {
	document.form1.action="order_list_rechange.php";
...
function OrderExcel() {
    //alert("excel");
	document.form1.action="order_excel_rechange.php";
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 주문/매출  &gt; 주문조회 및 배송관리 &gt;<span>교환접수 내역 관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">교환접수 내역 관리</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>교환접수 내역을 확인/처리하실 수 있습니다.</span></div>
				</td>
			</tr>
...
?>
			...
			<form name=checkexcelform action="order_excel_rechange.php" method=post>
