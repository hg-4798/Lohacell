...
function DynamicTree(id) {
    ...
    this.path = "http://i.ajashop.co.kr/ex/DynamicTree/img/";
    this.img = {
    ...
    this.parse = function(nodes, tree) {
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].nodeType == 1) {
				...
				//alert(nodes[i].getAttribute('vtype'));
				...
				node.vtype = nodes[i].getAttribute('vtype');
    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
        ...
        this.toHtml = function() {
            ...
            if (this.isFolder) {
                ...
                if (this.childNodes.length) { s += '<a href="javascript:void(0)" onclick="?.nodeClick(\'?\');openTree(this,1);?.clk_chk(this);">'.format(self.id, this.id,self.id ); }
                s += '<img id="?-node" src="?" width="18" height="18" alt="" />'.format(this.id, nodeIcon);
               
                //s += '<img id="?-icon" src="?" width="18" height="18" alt="" />'.format(this.id, icon);  폴더 이미지 픽스
				s += '<img id="?-icon" src="/admin/images/directory_folder1.gif" alt="" />'.format(this.id);
                if (self.foldersAsLinks) {
                    s += '<a href="#1"??>?'.format( (this.title ? ' title="?"'.format(this.title) : ""), (this.target ? ' target="?"'.format(this.target) : ""), this.text);
					s += '<input type=hidden name=cate?[] value="?">.....</a>'.format((this.id.length / 3), this.id);
                } else {
                    s += this.text;
					s += '<input type=hidden name=cate?[] value="?">'.format((this.id.length / 3), this.id);
				}
				
				if (this.vtype=='1') { 
                    s += " <font color='#CCCCCC'> (사용안함)</font>"; 
                } else if (this.vtype=='2') { 
                    s += " <font color='#CCCCCC'> (숨김)</font>"; 
                }
            ...
            if (this.isDoc) {
                s += '<img src="?" width="18" height="18" alt="" /><img src="/admin/images/directory_folder3.gif" alt="" />'.format((this.isLast() ? self.img.leafEnd : self.img.leaf));
                s += '<a href="?"?? onclick=openTree(this,1);?.clk_chk(this);>'.format((this.href?this.href :"#1"), (this.title ? ' title="?"'.format(this.title) : ""), (this.target ? ' target="?"'.format(this.target) : ""),self.id);
//				s += '?<input type=hidden name=cate?[] value="?">?</a>'.format(this.text, (this.id.length / 3), this.id,((this.vtype=='1')?"<font color='#CCCCCC'> (숨김)</font>":""));


                statusHtml = '';
				if (this.vtype=='1') { 
                    statusHtml = " <font color='#CCCCCC'> (사용안함)</font>"; 
                } else if (this.vtype=='2') { 
                    statusHtml = " <font color='#CCCCCC'> (숨김)</font>"; 
                }

				s += '?<input type=hidden name=cate?[] value="?">?</a>'.format(this.text, (this.id.length / 3), this.id, statusHtml);
