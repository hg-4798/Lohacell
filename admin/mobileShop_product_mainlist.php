...
function ProductInfo(prcode,popuptype) {
	...
	if (popup=="YES") {
		document.form_register.action="product_register.set.php?p_code=popup";
		document.form_register.target="register";
		window.open("about:blank","register","width=1300,height=700,scrollbars=yes,status=no");
	} else {
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 모바일샵 &gt; 모바일샵 관리 &gt;<span>메인상품 진열관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">메인상품 진열관리</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>쇼핑몰 메인페이지에 신상품, 인기상품, 추천상품, 특별상품에 진열할 상품을 등록할 수 있습니다.</span></div>
				</td>
			</tr>
			<tr>
				<td>					
			...
			<tr>
				...
				<?php
									...
									if ($likecode){
										...
										$qry.= "AND productcode in ('".implode("','",$linkcode)."') ";
...
//									if ($likecode) $qry= "AND productcode LIKE '{$likecode}%' ";
									...
									if (ord($row->tinyimage) && file_exists($imagepath.$row->tinyimage)){
										echo "<img src='".$imagepath.$row->tinyimage."' border=1 onMouseOver=\"ProductMouseOver('primage{$cnt}')\" onMouseOut=\"ProductMouseOut('primage{$cnt}');\">";
									} else {
...
<?php					
		...
		?>
							...
							<td valign=top>
								<!-- 소제목 -->
								<div class="title_depth3_sub">메인 진열 리스트</div>
								
								<IFRAME name="MainPrdtFrame" src="mobileShop_product_mainlist.main.php" width=100% height=100% frameborder=0 align=TOP scrolling="no" marginheight="0" marginwidth="0"></IFRAME>
								<!--
								<div class="main_view_setup_wrap">
									<div class="group">
										<input type="radio" name="" id="" />신상품 <input type="radio" name="" id="" />인기상품 <input type="radio" name="" id="" />추천상품 <input type="radio" name="" id="" />특별상품
									</div>
									<div class="list">

										<div class="table_main_setup">
											<table border=0 cellpadding=0 cellspacing=0 width=100%>
											<tr>
												<td colspan="12"></td>
											</tr>
												<colgroup>
													<col width="30" /><col width="" /><col width="" /><col width="" /><col width="" /><col width="" /><col width="" />
												</colgroup>
												<tr>
													<th>순번</th>	
													<th>상품명</th>	
												...
												<?
												$qry="select * from tblproduct where productcode like '004%' limit 10";
												$result=pmysql_query($qry);
												while($row=pmysql_fetch_object($result)){
																								?>
												<tr>
													<td>1</td>
													<td><div class="ta_l"><?=$row->productname?></div></td>
													<td><?=$row->sellprice?></td>
													<td>무제한</td>
													<td>판매중</td>
													<td><a href="#"><img src="images/icon_newwin1.gif" alt="수정" /></a></td>
													<td><a href="#"><img src="images/icon_del1.gif" alt="삭제" /></a></td>
												</tr>
												<?}?>
			...
			<tr>
				...
				<div class="sub_manual_wrap">
					...
					<dl>
						<dt><span>메인 섹션별 진열상품 등록하기</span></dt>
						<dd>
					...
					<dl>
						<dt><span>메인 섹션별 진열시 주의사항</span></dt>
						<dd>
					...
					<dl>
						<dt><span>메인 상품진열 섹션 순서 입니다.</span></dt>
						<dd>
