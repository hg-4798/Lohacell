...
if(!$linkerData['customer_id'] || !$linkerData['shoplinker_id']){
	$onload="<script>window.onload=function(){alert(\"연동설정이 되지 않았습니다.\");location.replace('linker_settings.php');}</script>";
	echo $onload;
	exit;
}
...
function registeradd(flag){
	if(flag == 'n'){
		$("input[name='mode']").val("linkerProductRegister");
	}
	$.ajax({
		type: "POST", 
		url: "linker_indb.php", 
		...
		beforeSend: function () {
			popupJquery('linker_process_popup.php', 430, 200);
		}
	}).done(function ( data ) {
		$('#createIfrm').contents().find('.linker_pop_msg').html(data);
	});
	/*
	document.form1.action="linker_indb.php";
...
<!-- 라인맵 -->
<div class="admin_linemap"><div class="line"><p>현재위치 : 상품관리 &gt;샵링커 상품관리 &gt; <span>샵링커 연동 상품등록</span></p></div></div>
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<td valign="top">
				<form name=form1 action="<?=$_SERVER['PHP_SELF']?>" method=post>
					<table cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed">
					<input type=hidden name=mode value = 'linkerProductRegister'>
					<input type=hidden name=prcode>
					<input type=hidden name=block value="<?=$block?>">
					<input type=hidden name=gotopage value="<?=$gotopage?>">			
					<tr>
						<td>
						<div class="title_depth3">샵링커 연동 상품 등록</div>
		...
		//							$qry.=" AND option_quantity like '%,0,%' ";
								...
								$sql0.= $qry;
								$paging = new newPaging($sql0,10,10);
								$t_count = $paging->t_count;
								$gotopage = $paging->gotopage;

								$sql = "SELECT option_price,productcode,productname,production,sellprice,consumerprice, option_quantity, ";
								...
								while($row=pmysql_fetch_object($result)) {
									...
									if(count($option_quantity)!=0){
										$oq = 0;
										foreach($option_quantity as $arr_oq){
											$oq += $arr_oq;
										}
										$quantityOption = $oq;
									}else{
									...
									echo "<tr>";
									echo "<td><input type = 'checkbox' name = 'linker_val[]' value = '".$row->productcode."'></td>";
									echo "<td>".$number."</td>";
									echo "<td>";
									if (ord($row->tinyimage) && file_exists($imagepath.$row->tinyimage)){
										echo "<a href=\"/front/productdetail.php?productcode=".$row->productcode."\" target=\"_blank\"><img src='".$imagepath.$row->tinyimage."' style=\"width:50px\" border=1></a>";
									} else {
										echo "<img src='images/space01.gif' style=\"width:50px\">";
									}
									echo "</td>\n";
									...
									echo "<td>".number_format($quantityOption)."</td>";
				...
				<?php					
								//페이징
									...
									echo "</div>";
									echo "</div>";
									echo "<div class=\"btn_right\"><a href=\"javascript:registeradd('n');\">[상품 연동]</a></div>";
