...
$todayY = $_POST['todayY'];
$todayM = str_pad($_POST['todayM'],2,"0",STR_PAD_LEFT);
$todayD = str_pad($_POST['todayD'],2,"0",STR_PAD_LEFT);
...
function MainInsert(prcode) {
	...
	document.prodDetail.prcode.value=prcode;
	document.prodDetail.submit();
	
	
	/*
	$.post("market_onedayprice_ajax.php",{prcode:prcode},function(data){
...
function OndayApply(){
	var mainform = document.form_reg;
	var subform = document.MainPrdtFrame.form1;

	mainform.mode.value = "regist";
	mainform.prcode.value = subform.prcode.value;
	mainform.dcprice.value = subform.dcprice.value;
	mainform.memo.value = subform.memo.value;
	mainform.applydate.value = "<?=$todayY?>-<?=$todayM?>-<?=$todayD?>";
	
	mainform.submit();
	
}
function GoList(){
	document.form1.action="market_onedayprice.php";
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">오늘의 특가 상품 등록</div>
					<!-- 소제목 -->
			...
			<tr>
				...
				<?php
									...
									$sql0.= "AND sabangnet_flag='N' ";	//제휴몰 상품이 아닌것들만..
									$sql0.= $qry;
									...
									$sql.= "AND a.sabangnet_flag='N' ";	//제휴몰 상품이 아닌것들만..
									$sql.= $qry." ";
...
<?php					
		...
		?>
							...
							<td valign=top>
								<!-- 소제목 -->
								<div class="title_depth3_sub">오늘의 특가 상품 등록</div>
								
								<IFRAME name="MainPrdtFrame" src="market_onedayprice_iframe.php" width=100% height=100% frameborder=0 align=TOP scrolling="no" marginheight="0" marginwidth="0"></IFRAME>
...
<form name="prodDetail" action="market_onedayprice_iframe.php" target="MainPrdtFrame" method="POST">
...
<form name="form_reg" action="market_onedayprice_indb.php" method="POST">
	...
	<input type="hidden" name="prcode" value="">
	<input type="hidden" name="dcprice" value="">
