...
if ($type=="up") {
	...
	if($cnt <=$maxcount){
		...
		if($temp['result'] == 'true'){
			$onload = "<script>window.onload=function(){ alert('문자 전송이 성공했습니다.'); location.replace('/admin/market_smssinglesend.php'); }</script>";
		}else{
			$onload = "<script>window.onload=function(){ alert('문자 전송이 실패했습니다.'); location.replace('/admin/market_smssinglesend.php'); }</script>";
		}
	} else {
		$onload="<script>window.onload=function(){ alert('SMS 머니가 부족합니다. 충전후 이용하시기 바랍니다.'); location.replace('/admin/market_smssinglesend.php'); }</script>";
	}
}

if($maxcount>0 && ord($onload)==0) $onload="<script>window.onload=function(){ alert('현재 {$maxcount}건의 SMS를 발송하실 수 있습니다.'); }</script>";
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">SMS 개별 발송</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>특정 고객에게 SMS를 발송할 수 있습니다.</span></div>
				</td>
			</tr>
...
?>
					...
					<tr>
						...
						<tr>
							...
							<TR>
								<th><span>발송 선택</span></th>
								<TD width="596" class="td_con1"><input type=radio id="idx_rsend1" name=rsend value="0" checked onclick="change_rsend(this.value);"><label style='cursor:hand;' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_rsend1>즉시</label>
...
<?php
				for($i=1;$i<=24;$i++) {
					$i0=sprintf("%02d",$i);
					if($i0==date("H"))
						echo "<option value=\"{$i0}\" selected>{$i0}시</option>\n";
					else
						echo "<option value=\"{$i0}\">{$i0}시</option>\n";
				}
?>
									</select>
...
<?php
				for($i=0;$i<=59;$i++) {
					$i0=sprintf("%02d",$i);
					if($i0==date("i"))
						echo "<option value=\"{$i0}\" selected>{$i0}분</option>\n";
					else
						echo "<option value=\"{$i0}\">{$i0}분</option>\n";
				}
?>
									</select>
								</TD>
							...
							<div class="table_none">
								<table cellpadding="0" cellspacing="0" width="100%">
									...
									<tr>
										<td align=center><a href="javascript:CheckForm();"><img src="images/btn_sms3.gif" border="0" <?php if($isdisabled!="1") echo "style=\"filter:Alpha(Opacity=60) Gray\"";?>></a>&nbsp;&nbsp;<a href="market_smsfill.php"><img src="images/btn_sms4.gif" border="0" hspace="2"></a></td>
									</tr>
								</table>
							</div>
