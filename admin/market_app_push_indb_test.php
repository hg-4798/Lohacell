<?php
// 테스트용
// shinwonmall.com/admin/market_app_push_indb_test.php?test=아이디^아이디^아이디
$Dir="../";
...
if($testid!= '') {
    $testidarr = explode("^", $testid);
    $testid = "'".implode("','", $testidarr)."'";
    exdebug("테스트 계정 리스트");
    exdebug($testidarr);
...
function sendPushCurl($pushNum, $os = ''){
    ...
    if($memSearchType == 'a'){
        ...
        elseif($_SERVER['REMOTE_ADDR'] == '218.234.32.68' || $testid!= "" ) $sql .= " and id in (".$testid.") ";
...
switch ($mode){
    case "memLoad":
        $mem_search_type = $_GET['mem_search_type'];
    ...
    case "insertData":
        ...
        if($_FILES['push_img']['name'] && $_GET['sendType'] == "m"){
        ...
        if($succFlag){
            ...
            if($_GET['sendType'] == "t"){
                $pushContents = pg_escape_string($_GET['push_content_text']);
            }else{
                $pushContents = pg_escape_string($_GET['push_content_mix']);
            }
            $pushTitle = pg_escape_string($_GET['push_title']);

            if($_GET['mem_search_type'] == 'a'){
                $_GET['search_start'] = "";
                $_GET['search_end'] = "";
            }

            $push_os = $_GET['push_os'];
            ...
            $push_insert_sql = "
													...
													VALUES
														(
															'".$_GET['sendType']."', '".$pushTitle."', '".$pushContents."', 
															'".$fullFileUrl."', '".$_GET['push_url']."', '".$_GET['push_send_type']."', 
															'".$_GET['push_send_day']."', '".$_GET['push_send_hour']."', 0, 
															now(), '".$_GET['search_start']."', '".$_GET['search_end']."',
															'".$arrSendFlag[$_GET['push_send_type']]."', '".$_GET['mem_search_type']."', '".$push_os."'
														)";
            pmysql_query($push_insert_sql,get_db_conn());
            ...
            //sendPushCurl($row_seq->currval_seq, $_GET['search_start'], $_GET['search_end']);
    ...
    case "sendAjaxPush":
        ...
        $result_seq = pmysql_query("SELECT push_send_type, push_os FROM tblpushlist WHERE no = '".$no."'", get_db_conn());   // 테이블 시퀀스
        $row_seq = pmysql_fetch_object($result_seq);
        if($row_seq->push_send_type == 'n'){
            $returnData = sendPushCurl($no, $row_seq->push_os);
