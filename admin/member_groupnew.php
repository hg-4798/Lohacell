...
if ($type=="insert") {
	...
	if (ord($groupimg["name"])) {
		...
		if (file_exists($imagepath."groupimg_{$group_code}.{$ext}")) {
			unlink ($imagepath."groupimg_{$group_code}.{$ext}");
		}
		$uploaded_img="groupimg_{$group_code}.{$ext}";
		move_uploaded_file ($groupimg["tmp_name"], $imagepath.$uploaded_img);
		chmod($imagepath.$uploaded_img,0777);
	}
	...
	*/
    ...
    if (file_exists($imagepath."groupimg_{$group_code}.png")) {
        unlink ($imagepath."groupimg_{$group_code}.png");
    }
	/*
...
<form name=form3 action="<?=$_SERVER['PHP_SELF']?>" method=post>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">회원등급 관리<span>회원등급 신규등록/수정/삭제를 하실 수 있으며 등급별 권한설정이 가능합니다.</span></div>
					<!-- 소제목 -->
...
<?php
				...
				while($row=pmysql_fetch_object($result)) {
					...
					*/
					$group_img = $imagepath."groupimg_".$row->group_code.".png";
					echo "<tr>\n";
					echo "	<td>{$i}</td>\n";
					...
					echo "	<td><NOBR>&nbsp;".number_format($row->group_ap_s)." 원 ~ ".number_format($row->group_ap_e)." 원</td>\n";
...
?>
			...
			<tr>
				...
				<TR>
					...
					<td><input type=text name=group_description value="<?=$group_description?>" maxlength=100 style="width:450px" class="input"> <span class="helper">50자 이내</span></td>
				</TR>
				<TR>
					<th><span>등급조건</span></th>
					<td>
						<span class="font_orange"><B>구매누적금액</B></span></label>
						<input type=text name=group_ap_s value="<?=$group_ap_s?>" style="text-align:right" maxlength=8 size=8 class="input"> 원 ~ 
						<input type=text name=group_ap_e value="<?=$group_ap_e?>" style="text-align:right" maxlength=8 size=8 class="input"> 원
					</td>
				</TR>
				<!-- <TR>
				...
				<TR>
					...
					* 권장크기 : 80*40 픽셀 [가로*세로])<br><span class="font_orange">* 150KB 이하의 PNG(png)이미지만 가능합니다.</span>
					<?php if(file_exists($imagepath."groupimg_{$group_code}.png")){?>
					<BR><BR><img src="<?=$imagepath?>groupimg_<?=$group_code?>.png" align=absmiddle>&nbsp;&nbsp; | &nbsp;&nbsp;<A HREF="javascript:GroupSend('imgdel','<?=$group_code?>');"><img src="images/icon_del1.gif" border=0 align=absmiddle></A>
					<?php }?></td>
				</TR>
				</TABLE>
				</div>
