...
if($type=="update" && strlen($design)==3) {
	if($_shopdata->design_bmap!=$design) {
		$sql = "UPDATE tblshopinfo SET design_bmap='{$design}' ";
		pmysql_query($sql,get_db_conn());
		DeleteCache("tblshopinfo.cache");

		$_shopdata->design_bmap=$design;
	}
	$onload="<script>window.onload=function(){alert(\"브랜드맵 화면 템플릿 설정이 완료되었습니다.\");}</script>";
}
...
function design_preview(design) {
	document.all["preview_img"].src="images/sample/bmap"+design+".gif";
}
...
<!-- 라인맵 -->
<div class="admin_linemap"><div class="line"><p>현재위치 : 디자인관리 &gt; 템플릿-페이지 본문 &gt;<span>브랜드맵 화면 템플릿</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 타이틀 -->
					<div class="title_depth3">브랜드맵 화면 템플릿</div>
					<div class="title_depth3_sub"><span>쇼핑몰 브랜드맵 화면 디자인을 선택하여 사용하실 수 있습니다.</span></div>
                </td>
            </tr>
            <tr>
            	<td>
			...
			<tr>
				<td align=center><img id="preview_img" src="images/sample/bmap<?=$_shopdata->design_bmap?>.gif" border=0 border="0" class="imgline"></td>
			</tr>
			<tr>
				<td height="20"></td>
			</tr>
...
<?php
		for($i=0;$i<count($templet_list);$i++) {
			...
			echo "<img src=\"images/sample/bmap{$templet_list[$i]}.gif\" border=\"0\" class=\"imgline1\" onMouseOver='changeMouseOver(this);' onMouseOut=\"changeMouseOut(this,'dotted #FFFFFF');\" style='cursor:hand;' onclick='ChangeDesign({$i});'>";
			echo "<br><input type=radio id=\"idx_design{$i}\" name=design value=\"{$templet_list[$i]}\" ";
			if($_shopdata->design_bmap==$templet_list[$i]) echo "checked";
...
?>
			...
			<tr>
				...
				<div class="sub_manual_wrap">
					...
					<dl>
						<dt><span>템플릿 디자인</span></dt>
						<dd>- 브랜드맵 페이지 노출 설정은 <a href="javascript:parent.topframe.GoMenu(4,'product_brand.php');"><span class="font_blue">상품관리 > 카테고리/상품관리 > 상품 브랜드 설정 관리</span></a> 에서 설정해 주세요.</dd>
                    </dl>
                    <dl>
                    	<dt><span>개별 디자인</span></dt>
                        <dd>- <a href="javascript:parent.topframe.GoMenu(2,'design_eachbmap.php');"><span class="font_blue">디자인관리 > 개별디자인 - 페이지 본문 > 브랜드맵 화면 꾸미기</span></a> 에서 개별 디자인을 할 수 있습니다.<br />- 개별 디자인 사용시 템플릿 적용되지 않습니다.</dd>
                    </dl>
                    <dl>
