...
if( $basefeetype_select == '1' ){
	...
	$minprice_select_staff = $_POST['minprice_select_staff']; // 배송료 지불 기준값 ( 미만 )
} else {
	$deli_select = 0; // 지불방법 0 - 선불, 1 - 착불, 2 - 구매자(선불/착불) 선택
	$basefee_select = 0; // 배송료
	$minprice_select = 0; // 배송료 지불 기준값 ( 미만 )
	$minprice_select_staff = 0; // 배송료 지불 기준값 ( 미만 )
}
...
$up_deli_miniprice_staff   = $minprice_select_staff; // 배송료 지불 기준값(임직원)
...
if ($type=="up") {
	...
	*/
	...
	$sql.= "deli_miniprice_staff		= '{$up_deli_miniprice_staff}', ";
...
$sql = "SELECT deli_type,deli_basefee,deli_basefeetype,deli_miniprice,deli_miniprice_staff, deli_setperiod,deli_limit, deli_select, ";
...
if ($data=pmysql_fetch_object($result)) {
	...
	$deli_miniprice_staff = $data->deli_miniprice_staff;
...
<div class="content-wrap">
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">상품 배송관련 기능설정 <span>상품 배송관련 조건을 쇼핑몰 성격에 맞게 설정하실 수 있습니다.</span></div>
				</td>
			</tr>
			<tr>
				<td>
...
-->
			...
			<tr>
				...
				<tr>
					...
					<td class="td_con1">
						<label><input type='radio' name='basefeetype_select'  value='0' <? if( $deli_basefeetype == '0' || is_null($deli_basefeetype) ) { echo 'checked'; } ?> class="hj" ><span class="lbl">배송료 <b>무료</b></span></label>
						<label><input type='radio' name='basefeetype_select'  value='1' <? if( $deli_basefeetype == '1' ) { echo 'checked'; } ?> class="hj"> <span class="lbl">배송료 <b>유료</b></span></label>
					</td>
				</tr>
				<tr style="display:none">
				...
				<tr>
					<th><span>배송료</span></th>
					<td>
						배송료 <input type='text' name='basefee_select' data-group="basefee" value='<?=$deli_basefee?>' style='text-align: right;' class="input"  <?=($deli_basefeetype<1)?'disabled':''?>> 원
					</td>
				</tr>
				<tr>
					<th rowspan="2">무료배송기준</th>
					<td>
						<b>일반상품</b>: 
						구매금액 <input type='text' name='minprice_select' data-group="basefee" size='10' maxlength='10' value="<?=$deli_miniprice?>"  class="input" style="text-align:right;" <?=($deli_basefeetype<1)?'disabled':''?>> 원 미만일 경우 배송비가 청구됩니다.&nbsp;
						<span class="helper">
				...
				<tr>
					<td>
						<b>임직원상품</b>: 
							구매금액 <input type='text' name='minprice_select_staff' data-group="basefee" size='10' maxlength='10' value="<?=$deli_miniprice_staff?>"   <?=($deli_basefeetype<1)?'disabled':''?> class="input" style="text-align:right;">
							원 미만일 경우 배송비가 청구됩니다.&nbsp;
							<span class="helper">
							* 구매금액 0 원 입력시 모든 금액에 배송비가 부과됩니다.
						</span>
				...
				<script>
					...
					$(document).ready( function(){
						$('input[name="basefeetype_select"]').on('click',function() {
							if(this.value == 1) {
								$('input[name="deli_select"]').each( function( deli_idx, deli_obj ){
									$(this).removeAttr( 'disabled' );
								});

								$('[data-group="basefee"]').removeAttr('disabled');
							}
							else {
								$('input[name="deli_select"]').each( function( deli_idx, deli_obj ){
									$(this).attr( 'disabled', 'true' );
								});
								$('[data-group="basefee"]').attr('disabled', true);
							}
						});
					});
				</script>
