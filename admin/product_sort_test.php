...
if ($mode=="sequence" && $change=="Y" && ord($prcodes)) {
	...
	for($i=0;$i<$cnt;$i++){
		...
		if(strpos($type,'T')===FALSE) {
			...
			$arr_procode=substr($productcode[$i],-18);
			//$sql = "UPDATE tblproducttheme SET date = '".$date1.$date."' ";
			$sql = "UPDATE tblproducttheme SET date = '".$date1.$date."' ";
			if($_POST["start_no_{$arr_procode}"])$sql.= ", start_no=1 ";
			else $sql.= ", start_no=".$i." ";
...
function takeChange(argObj)
{
	...
	document.all["idx_inner_"+argObj.sort].innerHTML="<div class=\"table_none\"><TABLE onclick=\"ChangeList('"+argObj.num+"');\" border=\"0\" cellSpacing=\"0\" cellPadding=\"0\" width=\"100%\" style=\"table-layout:fixed\"><col width=40></col><col width=12></col><?=($vendercnt>0?"<col width=70></col>":"")?><col width=100></col><col width=></col><col width=90></col><col width=70></col><col width=70></col><col width=70></col><col width=70></col><tr align=\"center\">"+innerHtmlStr+"</tr></table></div>";
}
						...
						<td width="100%" height="100%" valign="top" bgcolor="#FFFFFF" style="padding-left:5px;padding-right:5px;">
							...
							<?php
									...
									if(strlen($code)==12) {
										...
										if ($likecode){
										$link_qry="select c_productcode, c_date_{$sort_num} from tblproductlink where c_category like '{$likecode}%' group by c_productcode,  c_date_{$sort_num} order by c_date_{$sort_num} desc";
										...
										while($link_data=pmysql_fetch_object($link_result)){
											...
											$casewhen[]=" '".$link_data->c_productcode."' then ".$lk;
											$lk++;
										}
										...
										if ($keyword) $qry.= "AND (productname || b.c_category)LIKE '%{$keyword}%' ";
										...
										$sql.= "buyprice,quantity,reserve,reservetype,addcode,display,vender,tinyimage,assembleuse,assembleproduct, date, modifydate,start_no ";
										$sql.= "FROM tblproduct a WHERE 1=1  AND sabangnet_flag='N'";
										$sql.= $qry." ";
										//$sql.= "ORDER BY case productcode when ".implode(" when ",$casewhen)." end, date desc";
										$sql.= "ORDER BY start_no asc ";
										...
										if($cnt>0)
										{
											...
											while($row=pmysql_fetch_object($result)) {
												...
												echo "	<col width=35></col><col width=15></col>".($vendercnt>0?"<col width=70></col>":"")."<col width=62></col><col width=95 ></col><col width=50></col><col width=90></col><col width=70></col><col width=70></col><col width=70></col>\n";
												...
												/// 달러												
												echo "		<TD style=\"text-align:right; padding-right:20px\"><span class=\"font_orange\">".$row->sellprice." $</span></TD>";
												...
												echo "		<TD  style=\"text-align:right;\">".($row->display=="Y"?"<font color=\"#0000FF\">판매중</font>":"<font color=\"#FF4C00\">보류중</font>")."</td>";
