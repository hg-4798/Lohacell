...
$asale_price10=(array)$_POST["sale_price11"];
$asale_price102=(array)$_POST["sale_price112"];
$asale_price10=(array)$_POST["sale_price12"];
$asale_price102=(array)$_POST["sale_price122"];
$asale_price10=(array)$_POST["sale_price13"];
$asale_price102=(array)$_POST["sale_price132"];
$asale_price10=(array)$_POST["sale_price14"];
$asale_price102=(array)$_POST["sale_price142"];
$asale_price10=(array)$_POST["sale_price15"];
$asale_price102=(array)$_POST["sale_price152"];
...
if($_SERVER["REMOTE_ADDR"] == "218.234.32.36"){
		...
		if (
			...
			|| $asale_price9[$i]!=$asale_price92[$i] || $asale_price10[$i]!=$asale_price102[$i] || $asale_price11[$i]!=$asale_price112[$i] || $asale_price12[$i]!=$asale_price122[$i] || $asale_price13[$i]!=$asale_price132[$i] || $asale_price14[$i]!=$asale_price142[$i] || $asale_price15[$i]!=$asale_price152[$i])  
			)
			{
				...
				if($_SERVER["REMOTE_ADDR"] == "218.234.32.36"){
					...
					if (ord($asale_price10[$i])==0) 
						$asale_price10[$i]=0;
					if (ord($asale_price11[$i])==0) 
						$asale_price11[$i]=0;
					if (ord($asale_price12[$i])==0) 
						$asale_price12[$i]=0;
					if (ord($asale_price13[$i])==0) 
						$asale_price13[$i]=0;
					if (ord($asale_price14[$i])==0) 
						$asale_price14[$i]=0;
					if (ord($asale_price15[$i])==0) 
						$asale_price15[$i]=0;

				if (is_numeric($asale_price1[$i]) && is_numeric($asale_price2[$i]) && is_numeric($asale_price3[$i]) && is_numeric($asale_price4[$i]) && is_numeric($asale_price5[$i]) && is_numeric($asale_price6[$i]) && is_numeric($asale_price7[$i]) && is_numeric($asale_price8[$i]) && is_numeric($asale_price9[$i]) && is_numeric($asale_price10[$i]) && is_numeric($asale_price11[$i]) && is_numeric($asale_price12[$i]) && is_numeric($asale_price13[$i]) && is_numeric($asale_price14[$i]) && is_numeric($asale_price15[$i])) {   #숫자인지 검사
					...
					$sql.= "sale_price11		= '{$asale_price11[$i]}', ";
					$sql.= "sale_price12		= '{$asale_price12[$i]}', ";
					$sql.= "sale_price13		= '{$asale_price13[$i]}', ";
					$sql.= "sale_price14		= '{$asale_price14[$i]}', ";
					$sql.= "sale_price15		= '{$asale_price15[$i]}', ";
					$sql.= "modifydate		=	now()";
	...
	if($_SERVER["REMOTE_ADDR"] == "218.234.32.36"){
					...
					$log_content = "## 제휴사사격 일괄수정 ## - 상품코드: {$aproductcode[$i]} 제휴가 가격: {$sale_price1[$i]}  {$sale_price2[$i]}  {$sale_price3[$i]} {$sale_price4[$i]} {$sale_price5[$i]} {$sale_price6[$i]} {$sale_price7[$i]} {$sale_price8[$i]} {$sale_price9[$i]} {$sale_price10[$i]}{$sale_price11[$i]}{$sale_price12[$i]}{$sale_price13[$i]}{$sale_price14[$i]}{$sale_price15[$i]}";
...
function ProductCheckExcel(){
	...
	document.product_excel.action = 'product_allupdate_company_excel_v2.php';
	//document.form1.target = "HiddenFrame";
	document.product_excel.submit();
}
function ProductAllExcel(){
	document.product_excel.action = 'product_allupdate_company_allexcel_v2.php';
	//document.form1.target = "HiddenFrame";
	document.product_excel.submit();
}
...
//echo  count($group_productcode);
						$sql.= "sale_price1,sale_price2,sale_price3,sale_price4,sale_price5,sale_price6,sale_price7,sale_price8,sale_price9,sale_price10,sale_price11,sale_price12,sale_price13,sale_price14,sale_price15,";
		...
		?>		
			...
			<tr>
				...
				<!--td align="center" height="40"-->
					<form name="dv_all_form" action="product_allupdate_company_indb_v2.php" enctype="multipart/form-data" method="post" onSubmit="return submit_data();">
