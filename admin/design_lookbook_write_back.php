...
?>
	...
	<?for($j=0;$j<5;$j++){?>
				<TR>
					...
					<TD class="td_con1"><INPUT maxLength=60 size=60 name='places[<?=$j?>]' value=""><input type="hidden" name="productcodes[<?=$j?>]" class='procode productcodes<?=$j?>'></TD>
					<th><span>관련상품</span>&nbsp;&nbsp;<a href="javascript:T_layer_open('layer_product_sel','relationProduct<?=$j?>');"><img src="./images/btn_search2.gif" style='vertical-align:middle;padding-top:3px;padding-bottom:7px;'/></a></th>
					<td align="left">
							<div style="margin-top:0px; margin-bottom: 0px;height:50px">							
								<table border=0 cellpadding=0 cellspacing=0 style='border:0px' name="prList" class="prList" id="check_relationProduct<?=$j?>" alt='<?=$j?>'>	
								<input type="hidden" name="limit_relationProduct<?=$j?>" id="limit_relationProduct<?=$j?>" value="1"/>								
									<colgroup>
										<col width=50></col>
										<col width=></col>
									</colgroup>
