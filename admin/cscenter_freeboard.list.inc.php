...
if($search_site) {
    $qry .= "AND a.site_name like '%{$search_site}%' ";
}

$qry .= "AND to_char(a.writetime::abstime, 'YYYY-MM-DD') BETWEEN '{$search_start}'  AND  '{$search_end}'";
...
if (ord($s_check) && ord($search)) {
    ...
    switch ($s_check) {
        ...
        case "c":
            $qry .= "AND (";
            $qry.= " a.customer LIKE any ( array[".implode(",", $search_arr)."] ) ";
            $qry .= ") ";
            break;
        case "p":
            $qry .= "AND (";
            $qry.= " a.productcode LIKE any ( array[".implode(",", $search_arr)."] ) ";
            $qry .= ") ";
            break;
    }
}
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
    <form name=form1 action="<?=$_SERVER['PHP_SELF']?>" method=get>
       ...
       <col>
        ...
        <tr >
            <td colspan="<?=$colspan?>" width="100%" style="border: none">
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        <!--
                        <td class="font_size">
                        ...
                        <td align="right" class="font_size ta-r" style="border: none">
    ...
    <TR align="center">
        ...
        <th class="th_style01">작성자(아이디)</th>
    ...
    <form name=form2 action="<?=$_SERVER['PHP_SELF']?>" method=post>
        ...
        <input type="hidden" name="site_name" value="<?=$search_site?>">
    </form>
</table>
</div>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
                ...
                <TD>
                    <table border="0" cellpadding="0" cellspacing="0" width="100%">
                        <tr>
                            ...
                            <TD align="right">
                                <button type="button" id="btn_register" class="btn-point" onclick="boardWrite()">등록</button>
                            </TD>
                        </tr>
                    </table>
                </td>
...
<SCRIPT LANGUAGE="JavaScript">
    <!--
    function boardWrite() {
       location.href="<?=$_SERVER['PHP_SELF']?>?exec=write";
    }
    function schecked(){
        if (frm.search.value == ''){
            alert('검색어를 입력해주세요.');
    ...
    $(document).ready(function() {
        $("select[name='site_name']").val("<?=$search_site?>");
