...
<?php if($print!="Y"){?>
<div class="admin_linemap"><div class="line"><p>현재위치 : 통계분석 &gt; 트래픽 분석 &gt;<span>시간별 페이지뷰</span></p></div></div>
...
<?php }?>
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">시간별 페이지뷰</div>
				</td>
			</tr>
...
<?php
				if($type=="d") {
					$sql= "SELECT SUBSTR(date,9,2) as hour,pagecnt FROM tblcounter 
					WHERE date LIKE '{$searchdate}%' ";
				} elseif($type=="w") {
					$prevdate=date("Ymd00",strtotime('-7 day'));
					$nextdate=date("Ymd99");
					$sql ="SELECT SUM(pagecnt) as pagecnt,SUBSTR(date,9,2) as hour FROM tblcounter 
...
<?php if($print!="Y"){?>
			<tr>
				...
				<div class="sub_manual_wrap">
					...
					<dl>
					  <dt><span>시간 흐름(일/주간/월간)에 따른 쇼핑몰 페이지뷰 숫자를 보여 드리고 있습니다.</span></dt>
                    </dl>
                    <dl>
                    	<dt><span>방문자가 쇼핑몰은 방문하여 얼마나 많은 페이지를 검색했는 지 알 수 있습니다.<br />
순방문자 대비 페이지뷰를 계산하면, 고객의 사이트 관심도를 알 수 있습니다. </span></dt>
                    </dl>
                    <dl>
                    	<dt><span>방문자가 시간기준으로 1,000명이고, 이 때 사이트 페이지뷰가 5,000이면, 방문자의 사이트 관심도는 500포인트라는 것을 알 수 있습니다.<br />
사이트 관심도가 높을 수록 메인(첫)화면의 쇼핑몰 화면 기획이 우수하다는 것을 보여 주고 있습니다.<br />
이유는 방문자가 사이트를 방문하여 관심을 보이는 컨텐츠가 많음을 알려주고 있습니다. </span></dt>
                    </dl>
                </div>				
                </td>
			</tr>
